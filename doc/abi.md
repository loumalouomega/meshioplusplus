...
| 17 | v16.14.0 | **Tier B**: three inline bodies in `ndarray.hpp`. The zeroing constructor and `MakeOwned()` no longer call `memset`/`memcpy` on an empty buffer's null pointer (undefined behaviour UBSan found), and the two allocating constructors size their buffer through a new private `AllocBytes`, which throws `std::length_error` when a shape's byte count overflows `size_t` instead of wrapping to a small allocation. `sizeof(NDArray)` is unchanged. Also Tier B: `detail::read_int` (`detail/value_io.hpp`) reads a NaN float as 0 and saturates one outside int64 instead of casting it (undefined behaviour), and `detail::parse_double` (`detail/fast_number.hpp`) bounds its `from_chars` call to the number instead of `strlen` of the rest of the buffer, which made every reader parsing a whole file through it quadratic. The fuzz campaign's findings and the performance audit are why. The same release adds, additively: `Unsupported` in `exceptions.hpp`, `detail::check_vtk_cell_arrays` and `detail::vtk_xml_header_bytes` in `detail/vtk_cells.hpp`, a `vtu_encode_binary` overload taking the `header_type` width and `detail::vtu_header_bytes_for` in `detail/vtu_binary.hpp`, a `vtu_write_field_array` overload taking it in `detail/vtk_xml.hpp`, and the new headers `detail/read_guard.hpp` and `detail/parse_guard.hpp`; `detail/polyhedron.hpp` changed only in a comment. |
| 18 | v16.16.0 | **Tier A, layouts** (roadmap §3). The `MESHIO` backend's `CellBlock` stores ragged blocks as CSR -- `mFlat`, `mRowOffsets`, `mFaceOffsets`, the `NATIVE` layout -- instead of the nested `mPolygonRows`/`mPolyhedronRows`; `detail::FaceLookup` and `detail::FacetIndex` hold sorted vectors instead of `unordered_map`s; `detail::DistanceQuery` replaces its `mEdgeNormal` map with `mEdgeOfCorner` and `mEdgeNormals`; and `detail::SurfaceEdgeMap` (the return type of the exported `build_surface_edges`) becomes a sorted `std::vector` of (edge, record) pairs. All four are now pinned in `test_abi_layout.cpp` (`CellBlock` on the `MESHIO` configuration). Added in the same release: CSR overloads of `AddPolygonBlock`/`AddPolyhedronBlock` on every backend and the new header `detail/ragged_csr.hpp`; `parallel_sort`, `parallel_reduce` and `parallel_exclusive_scan` in `parallel.hpp` (new inline templates; `parallel_for`'s body is unchanged); `detail::ProvenanceSlotlessWrite` in `detail/provenance.hpp`. |
...
if(NOT MESHIOPLUSPLUS_ABI_VERSION EQUAL 18)
  message(FATAL_ERROR "meshio++ ABI 18 required, found ${MESHIOPLUSPLUS_ABI_VERSION}")
endif()
```
