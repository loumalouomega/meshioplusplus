...
| 17 | v16.14.0 | **Tier B**: three inline bodies in `ndarray.hpp`. The zeroing constructor and `MakeOwned()` no longer call `memset`/`memcpy` on an empty buffer's null pointer (undefined behaviour UBSan found), and the two allocating constructors size their buffer through a new private `AllocBytes`, which throws `std::length_error` when a shape's byte count overflows `size_t` instead of wrapping to a small allocation. `sizeof(NDArray)` is unchanged. Also Tier B: `detail::read_int` (`detail/value_io.hpp`) reads a NaN float as 0 and saturates one outside int64 instead of casting it (undefined behaviour). The fuzz campaign's findings are why. The same release adds, additively: `Unsupported` in `exceptions.hpp`, `detail::check_vtk_cell_arrays` in `detail/vtk_cells.hpp`, and the new headers `detail/read_guard.hpp` and `detail/parse_guard.hpp`; `detail/polyhedron.hpp` changed only in a comment. |
