...
Determinism follows the repo's standard phase split: signed distances and warps are computed in parallel into disjoint per-vertex slots; since v16.19.0 the cut, weld and compaction passes run in parallel too, deduplicating cut points over the shared sort-based facet table by ascending edge key (the `refine.cpp`/`surface.cpp` idiom) instead of a serial pass, so two lattice tets sharing a face still agree bit-for-bit on that face's cut points.
