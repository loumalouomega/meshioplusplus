...
| **libmesh** | 📖 | 📖 | 📖 | ✅ | **Read only**: a subdomain is a `cell` region named by the file's subdomain map (else `subdomain_<id>`) and tagged with its id; a side set is a `side` region (a side of a refined element carried to its active children), a node set a `point` region, both named by their maps (else `boundary_<id>`/`nodeset_<id>`) and tagged. See [`doc/formats/libmesh.md`](./formats/libmesh.md#boundaries). |
...
| **radioss** | 📖 | 📖 | 📖 | ✅ | **Read only**: a `/PART` is a `cell` region named by its title and tagged with its id, a `/SUBSET` the `cell` region of its parts, `/GRNOD` a `point` region, `/GRBRIC`/`/GRSHEL`/… `cell` regions and `/SURF/SEG` a `side` region, each tagged with its id. See [`doc/formats/radioss.md`](./formats/radioss.md#regions). |
...
| **z88** | ❌ | ❌ | ❌ | ❌ | A Z88 structure file has no groups: regions are dropped on write with a warning. |
