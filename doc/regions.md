...
Deferred to Phase 2, and listed in `tests/python/test_region_roundtrip.py` so the gap stays on the record: **UNV** and **Ansys** (absorbing `UnvInfo`/`AnsysInfo`), **XDMF** (Sets), and **VTU/VTP** — which have no native set concept at all, so a convention has to be chosen and documented rather than invented silently. Exodus's **writer** belongs on this list too.
