...
| **radioss** | 📖 | 📖 | 📖 | ✅ | **Read only**: a `/PART` is a `cell` region named by its title and tagged with its id, a `/SUBSET` the `cell` region of its parts, `/GRNOD` a `point` region, `/GRBRIC`/`/GRSHEL`/… `cell` regions (since v16.11.0 also by `BOX`, `BOX2`, `GENE` and `GEN_INCR`) and `/SURF/SEG` a `side` region (since v16.11.0 also `/SURF/PART`, `SUBSET`, `MAT`, `PROP`, `GRBRIC`, `GRSHEL`, `GRSH3N`, `GRTRIA` and `SURF`), each tagged with its id. See [`doc/formats/radioss.md`](./formats/radioss.md#regions). |
...
| **marc** | 📖 | 📖 | ❌ | ❌ | **Read only**: a deck's `DEFINE ELEMENT SET` is a `cell` region and `DEFINE NODE SET` a `point` region, named as the deck names them (ranges, set names and `AND`/`EXCEPT`/`INTERSECT` resolved); edge and face sets are skipped with a warning. The post file's element and node sets (`marc_t19`) alike. See [`doc/formats/marc.md`](./formats/marc.md#sets). |
