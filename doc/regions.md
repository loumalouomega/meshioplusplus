...
| **xplt** | 📖 | 📖 | 📖 | ✅ | **Read only**: FEBio plot-file domains → `cell` (tagged with the part id), node sets → `point`, element sets → `cell`, surfaces → `side` as for `.feb`; each data surface is also a facet block with a `cell` region of its name (v16.10.0). See [`doc/formats/xplt.md`](./formats/xplt.md#mapping). |
| **ansys** (Fluent) | ❌ | ✅¹ | ❌ | ✅ | Each Fluent zone, a cell zone or a boundary face zone, is a `cell` region named from its `(39 ...)`/`(45 ...)` declaration (`zone_<id>` otherwise), tagged with the zone id, over the cells (or boundary facet cells) `cell_data["ansys:zone"]` puts in it. On write (face-based since v16.10.0), the zones come from `ansys:zone` (else one per block) and a zone takes the name of the region with its tag and dimension, else of the region with exactly its cells. ¹ Only regions shaped like zones name anything: other cell regions, point regions and side regions are not written. See [`doc/formats/ansys.md`](./formats/ansys.md#writing). |
...
| **ansys_rst** | 📖 | 📖 | ❌ | ❌ | **Read only**: the result file's node and element components → `point` and `cell` regions, as for `.cdb`; `ansys_rst_cyclic` keeps a region's members in every sector of the full rotor. See [`doc/formats/ansys_rst.md`](formats/ansys_rst.md). |
...
| **patran** | ✅ | ✅ | ❌ | ✅ | A packet 21 named component lists nodes and elements: its node entries are a `point` region and its element entries a `cell` region of the same name, both tagged with the component number. Elements no component names are grouped by property into `property_<pid>` regions (tag = pid). On write, a point and a cell region sharing a name are one component, numbered by a positive unused `tag`; component names are cut to 12 characters. No facet form. See [`doc/formats/patran.md`](./formats/patran.md). |
| **femap** | ✅ | ✅ | ❌ | ✅ | A 408 group's node list is a `point` region and its element list a `cell` region of the group's title, tagged with the group id; each property is a `cell` region named by its 402 title (else `property_<id>`) and tagged with its id. On write, properties come from `femap:property` and their regions, other point and cell regions become groups (one per name, numbered by a positive unused `tag`). No facet form. See [`doc/formats/femap.md`](./formats/femap.md). |
| **mfem** | ❌ | ✅ | ↪️ | ✅ | An element attribute is an `attribute_<n>` and a boundary attribute a `boundary_<n>` `cell` region tagged `n`; a v1.3 attribute set is a `cell` region of its name. On write, cells take `mfem:attribute`, else their region's tag (or a fresh number), and named regions become attribute sets. ↪️ A `side` region's facets are written as boundary elements, so it reads back as a `cell` region over a new block. No node sets, so point regions are dropped. See [`doc/formats/mfem.md`](./formats/mfem.md). |
| **libmesh** | ✅ | ✅ | ✅ | ✅ | A subdomain is a `cell` region named by the file's subdomain map (else `subdomain_<id>`) and tagged with its id; a side set is a `side` region (a side of a refined element carried to its active children), a node set a `point` region, both named by their maps (else `boundary_<id>`/`nodeset_<id>`) and tagged. Since v16.11.0 an edge set is a `cell` region `<name>:edge` over `line` cells made from the edges, and a shell-face set a `cell` region `<name>:shellface0`/`1`. The writer (v16.11.0) maps each of these back, other cell regions to subdomains. See [`doc/formats/libmesh.md`](./formats/libmesh.md#boundaries). |
| **abaqus_fil** | 📖 | 📖 | ❌ | ❌ | **Read only**: the results file's node sets (1931) and element sets (1933) → `point` and `cell` regions; a name longer than 8 characters is resolved through the 1940 labels. Surfaces are not in the `.fil` file. See [`doc/formats/abaqus_fil.md`](./formats/abaqus_fil.md#records). |
| **radioss** | ✅ | ✅ | ✅ | ✅ | A `/PART` is a `cell` region named by its title and tagged with its id, a `/SUBSET` the `cell` region of its parts, `/GRNOD` a `point` region, `/GRBRIC`/`/GRSHEL`/… `cell` regions (since v16.11.0 also by `BOX`, `BOX2`, `GENE` and `GEN_INCR`) and `/SURF/SEG` a `side` region (since v16.11.0 also `/SURF/PART`, `SUBSET`, `MAT`, `PROP`, `GRBRIC`, `GRSHEL`, `GRSH3N`, `GRTRIA` and `SURF`), each tagged with its id. Written since v16.17.0: parts from `radioss:part` (else a cell region per part), a cell region that is a union of whole parts as a `/SUBSET`, any other as one group per element family, point regions as `/GRNOD/NODE` and side regions as `/SURF/SEG`, ids from a positive free `tag`. See [`doc/formats/radioss.md`](./formats/radioss.md#regions). |
| **radioss_anim** | ❌ | 📖 | ❌ | ✅ | **Read only** (v16.11.0): each part of an animation file is a `cell` region named by its title and tagged with its id. See [`doc/formats/radioss_anim.md`](./formats/radioss_anim.md#the-mesh). |
| **marc** | ✅ | ✅ | ❌ | ❌ | A deck's `DEFINE ELEMENT SET` is a `cell` region and `DEFINE NODE SET` a `point` region, named as the deck names them (ranges, set names and `AND`/`EXCEPT`/`INTERSECT` resolved); edge and face sets are kept as field data (v16.12.0). The post file's element and node sets (`marc_t19`, read only) alike. Written since v16.17.0 as `DEFINE NODE SET`/`DEFINE ELEMENT SET` (names made one token); side regions are dropped with a warning. See [`doc/formats/marc.md`](./formats/marc.md#sets). |
| **lsdyna_d3plot** | ❌ | 📖 | ❌ | ✅ | **Read only**: every part with elements is a `cell` region named by its title (else `Part <id>`) and tagged with the user part id; the d3plot has no sets. See [`doc/formats/lsdyna_d3plot.md`](./formats/lsdyna_d3plot.md#the-model). |
| **nastran_op2** / **nastran_h5** | ❌ | 📖 | ❌ | ✅ | **Read only**: one `cell` region per property id, named `<PTYPE>_<pid>` after the property card (`PID_<pid>` when none defines it) and tagged with the pid; an OP2 read through its sibling deck has the deck's regions instead. See [`doc/formats/nastran_op2.md`](./formats/nastran_op2.md#the-model). |
| **z88** | ✅ | ✅ | ❌ | ✅ | A Z88OS structure file has no groups. Z88Aurora's `z88sets.txt` (read since v16.11.0, written since v16.12.0): element sets are `cell` regions and node sets `point` regions, named as in Aurora and tagged with the set id (surface sets are skipped). On write, cell regions become element sets (purpose `MATERIAL`) and point regions node sets (`CONSTRAINT`), the id a positive unused tag or the next free one; side regions are dropped with a warning. See [`doc/formats/z88.md`](./formats/z88.md#writing). |
...
| **tecplot** | ❌ | ✅ | ❌ | ❌ | Reading a non-transient multi-`ZONE` file (v15.5.0, roadmap §1.1; ASCII and, since v16.10.0, binary `.plt`) gives one `cell` region per zone, named from the zone's own title (de-duplicated) or `zone_<i>` when it has none; the region's `tag` is the zone's own position in the file, but that position is not meaningful on write, so `tag` does not round-trip. On write, a `cell` region whose entries exactly match one cell block's global range supplies that block's zone title; a block with no matching region gets `block_<i>`. No node-set and no facet-group concept, so point and side regions are dropped. See [`doc/formats/tecplot.md`](./formats/tecplot.md#multiple-zones). |
