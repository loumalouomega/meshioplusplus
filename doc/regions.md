...
| **z88** | 📖 | 📖 | ❌ | ✅ | A Z88OS structure file has no groups. Z88Aurora's `z88sets.txt` (v16.11.0) is read: element sets are `cell` regions and node sets `point` regions, named as in Aurora and tagged with the set id (surface sets are skipped). Regions are dropped on write with a warning. |
