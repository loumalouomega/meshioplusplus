...
`abaqus`, `ansys`, `ansys_rst` (Ansys MAPDL results, **read-only**; every result set is a step, see [Ansys results](./formats/ansys_rst.md)), `ansysinp` (read/write; `CMBLOCK` components become regions, see [Ansys coded database](./formats/ansysinp.md)), `avsucd`, `cgns`, `code_aster` (Code_Aster `.mail`; `GROUP_MA`/`GROUP_NO` become regions, see [Code_Aster](./formats/code_aster.md)), `dex`, `dolfin`, `elmer` (Elmer mesh directories; written into MEMFS, read back with no format by sniffing the directory — see [Elmer](./formats/elmer.md)), `ensight` (EnSight Gold geometry, `.case`/`.geo`, ASCII + C-binary), `exodus`, `febio` (FEBio `.feb` meshes; surfaces become side regions, see [FEBio](./formats/febio.md)), `flac3d` (including `ZGROUP`/`FGROUP` cell groups, which ride on the mesh object as named regions — see [FLAC3D](./formats/flac3d.md#data-mapping)), `flux`, `frd` (CalculiX results, **read-only**; every increment is a step, see [CalculiX results](./formats/frd.md)), `freefem`, `gid` (GiD postprocess; *writing* rides a vendored gidpost and needs zlib, *reading* is meshio++'s own code and needs nothing for the ascii flavour), `gltf` (**write-only**, `.glb`/`.gltf` — the surface as glTF 2.0 with the default options, see [glTF](./formats/gltf.md)), `gmsh`, `h5m`, `hmf`, `ip`, `lsdyna` (LS-DYNA keyword decks; `*PART`/`*SET_*` become regions, see [LS-DYNA](./formats/lsdyna.md)), `mdpa` (Kratos; mesh-level blocks only — see [MDPA](./formats/mdpa.md#c-core)), `med`, `medit`, `mff`, `mfm`, `mphbin`, `mphtxt` (COMSOL meshes, binary and text; Selections become regions, see [COMSOL](./formats/mphtxt.md)), `nastran` (any bulk-data deck; HyperMesh components and OptiStruct `SET`s become regions, see [Nastran](./formats/nastran.md)), `nastran_h5` (MSC Nastran HDF5 results, **read-only**; every result domain is a step, see [MSC Nastran HDF5](./formats/nastran_h5.md)), `netgen`, `obj`, `off`, `openfoam` (read/write; writing creates a `constant/polyMesh` directory in MEMFS), `pcd` (PCL point clouds; reads `ascii`/`binary`/`binary_compressed`, writes `ascii`/`binary`), `permas`, `ply`, `pvd` (a time-indexed collection of `.vtu` steps), `pvtp`, `pvtu` (parallel indices over per-part pieces; index and pieces sit side by side in MEMFS), `stl`, `su2`, `svg` (**write-only**, 2D visualization), `tecplot`, `tetgen`, `tikz` (**write-only**, 2D LaTeX visualization), `triangle` (`.poly` by default; see the ambiguous-extensions table for `.node`/`.ele`), `ugrid`, `unv` (`.unv`/`.uff`; groups become regions, results are steps — see [UNV](./formats/unv.md)), `vti` (VTK XML ImageData: a regular lattice, so writing needs one — see [VTI](./formats/vti.md)), `vtk`, `vtkhdf` (HDF5-backed: UnstructuredGrid, PolyData and the two composite types, including polyhedra, partitions and time; `piece`/`timeStep` select what to read), `vtp`, `vtu` (zlib compression works via Emscripten's built-in port), `wkt`, `xdmf` (XML, Binary **and** HDF), `xplt` (FEBio plot files, **read-only**; every state is a step, see [FEBio plot files](./formats/xplt.md)), `xyz` (headerless ASCII point clouds, see [XYZ](./formats/xyz.md)). The three field-only formats (`dex`, `ip`, `mff`) read/write geometry-less meshes (field values in `point_data`).
