...
**As of v8.0.0 the WASM build ships every format the C++ core has**, including the five that need HDF5 or netCDF — there is no longer a WASM-specific format gap. That is 76 readable and 68 writable format keys (`svg`, `tikz`, `gltf` and `gmsh22` are write-only; `frd`, `nastran_h5`, `nastran_op2`, `xplt`, `ansys_rst`, `ansys_rst_cyclic`, `abaqus_fil`, `radioss_anim`, `radioss_th`, `marc_t19`, `lsdyna_d3plot` and `lsdyna_binout` read-only, each for the reason the [conformance page](./conformance.md#read-only-formats) gives; `libmesh` gained its writer and `radioss_anim` joined in v16.11.0, `lsdyna_binout` and `radioss_th` in v16.12.0, and `radioss` and `marc` gained writers in v16.17.0). The two formats of v16.13.0 are not in it: `vtx` needs ADIOS2 and `szplt` TecIO, neither of which the Emscripten build links, so both report the missing library by name. `gid` gained a reader in v10.19.0 and is now both. `.vts`/`.vtr`/`.vtm` (v11.6.0, tier B4) added three of each, none needing a `js_bindings.cpp` change at all -- see the format list above. `.pvd`/`.pvtu`/`.pvtp` (v15.0.0) added three more of each, again with no `js_bindings.cpp` change: they are registered in `registry.cpp` and reached through the existing read options. `.pcd` and `.xyz` (v15.1.0, point clouds) added one of each the same way; `.k`/`.key`/`.dyn` (v15.2.0, LS-DYNA keyword decks) did too, with `*INCLUDE` files resolved next to the deck in MEMFS; `.frd` (v15.3.0, CalculiX results) added one **read-only** key the same way, its increments reachable through the existing `timeStep` read option; `.h5` (v15.7.0, MSC Nastran HDF5 results) added another **read-only** key, on the wasm32 HDF5 already linked, its result domains reachable through `timeStep` too; `.mail` (v16.0.0, Code_Aster meshes) added one of each; `.mphbin` (v16.1.0, COMSOL binary meshes) added one of each; `elmer` and `.feb` (v16.2.0, Elmer mesh directories and FEBio input) added one of each, the Elmer writer creating its directory in MEMFS and the reader finding it by sniffing the directory, and `.xplt` (FEBio plot files) one **read-only** key, its states reachable through `timeStep` and its compressed states inflated by Emscripten's zlib port; `.rst`/`.rth` (v16.3.0, Ansys MAPDL results) added one more **read-only** key, its result sets reachable through `timeStep`; `.xda`/`.xdr`, `.fil` and `.rad` (v16.7.0: libMesh, Abaqus results, OpenRadioss) added three **read-only** keys, the `.fil` increments reachable through `timeStep`, and Z88 one of each, found by its fixed file name; `marc` (`.dat` files that open as MSC Marc decks), `.t19` (Marc post files, increments through `timeStep`) and `ansys_rst_cyclic` (by name) (v16.8.0) added three more **read-only** keys; `lsdyna_d3plot` (a family written into MEMFS as `d3plot`, `d3plot01`...: found by its file name, its states through `timeStep`) and `.op2` (Nastran OP2 results, steps through `timeStep`) (v16.9.0) added two more; `.pcd` is also the one format whose LZF stream (`binary_compressed`) is decoded in WASM with no dependency at all. `openfoam` became writable in v9.20.0; `gid` (the GiD postprocess format, on a vendored gidpost) needed no WASM-specific binding at all — it reaches every registry-driven consumer, including this one, purely by being registered in the shared `registry.cpp` writer/extension tables, and `-sUSE_ZLIB=1` supplies the `<zlib.h>` its vendored C sources need at the compiler (not just the linker).

> **Five Python-only formats are absent here.** `pmsh`, `zarr`, `cae` and `usd` (v10.35.0, the physics-ML data path — see [formats](./formats.md)), plus [`neuroglancer`](./formats/neuroglancer.md), are registered in the Python layer only, not in the shared C++ dispatch registry, so this surface does not see them. Two of the five are directory stores and one needs `usd-core`; the first four are export targets for a training pipeline rather than interchange formats, and `neuroglancer` reads and writes but has no C++ implementation.

`abaqus`, `abaqus_fil` (Abaqus results `.fil`, **read-only**; every increment is a step, see [Abaqus `.fil`](./formats/abaqus_fil.md)), `ansys`, `ansys_rst` (Ansys MAPDL results, **read-only**; every result set is a step, see [Ansys results](./formats/ansys_rst.md)), `ansys_rst_cyclic` (the full rotor of a static cyclic `.rst`, **read-only**, by name), `ansysinp` (read/write; `CMBLOCK` components become regions, see [Ansys coded database](./formats/ansysinp.md)), `avsucd`, `cgns`, `code_aster` (Code_Aster `.mail`; `GROUP_MA`/`GROUP_NO` become regions, see [Code_Aster](./formats/code_aster.md)), `dex`, `dolfin`, `elmer` (Elmer mesh directories; written into MEMFS, read back with no format by sniffing the directory — see [Elmer](./formats/elmer.md)), `ensight` (EnSight Gold geometry, `.case`/`.geo`, ASCII + C-binary, and Fortran binary read since v16.17.0), `exodus`, `femap` (Femap neutral `.neu`; every output set is a step, see [Femap](./formats/femap.md)), `febio` (FEBio `.feb` meshes; surfaces become side regions, see [FEBio](./formats/febio.md)), `flac3d` (including `ZGROUP`/`FGROUP` cell groups, which ride on the mesh object as named regions — see [FLAC3D](./formats/flac3d.md#data-mapping)), `flux`, `frd` (CalculiX results, **read-only**; every increment is a step, see [CalculiX results](./formats/frd.md)), `freefem`, `gid` (GiD postprocess; *writing* rides a vendored gidpost and needs zlib, *reading* is meshio++'s own code and needs nothing for the ascii flavour), `gltf` (**write-only**, `.glb`/`.gltf` — the surface as glTF 2.0 with the default options, see [glTF](./formats/gltf.md)), `gmsh`, `h5m`, `hmf`, `ip`, `libmesh` (libMesh `.xda`/`.xdr`, read and, since v16.11.0, written; gzip is read, bzip2 and compressed writing are Python-only; see [libMesh](./formats/libmesh.md)), `lsdyna` (LS-DYNA keyword decks; `*PART`/`*SET_*` become regions, see [LS-DYNA](./formats/lsdyna.md)), `lsdyna_d3plot` (LS-DYNA state databases, **read-only**; the file named `d3plot` and its numbered family, every state a step, see [LS-DYNA d3plot](./formats/lsdyna_d3plot.md)), `marc` (MSC Marc input decks; a `.dat` that opens as a Marc deck reads as one, Tecplot's otherwise; written by name since v16.17.0, `writeMesh(path, mesh, 'marc')`, since a `.dat` alone writes Tecplot — see [Marc](./formats/marc.md)), `marc_t19` (Marc formatted post files `.t19`, **read-only**; every increment is a step), `mdpa` (Kratos; mesh-level blocks only — see [MDPA](./formats/mdpa.md#c-core)), `med`, `medit`, `mfem` (MFEM `.mesh`; a `.mesh` whose first line names an MFEM mesh is read as one, see [MFEM](./formats/mfem.md)), `mff`, `mfm`, `mphbin`, `mphtxt` (COMSOL meshes, binary and text; Selections become regions, see [COMSOL](./formats/mphtxt.md)), `nastran` (any bulk-data deck; HyperMesh components and OptiStruct `SET`s become regions, see [Nastran](./formats/nastran.md)), `nastran_h5` (MSC Nastran HDF5 results, **read-only**; every result domain is a step, see [MSC Nastran HDF5](./formats/nastran_h5.md)), `nastran_op2` (Nastran OP2 results, **read-only**; every subcase, mode or time is a step, see [Nastran OP2](./formats/nastran_op2.md)), `netgen`, `obj`, `off`, `openfoam` (read/write; writing creates a `constant/polyMesh` directory in MEMFS), `patran` (Patran 2 neutral `.pat`/`.out`; components become regions, see [Patran](./formats/patran.md)), `pcd` (PCL point clouds; reads `ascii`/`binary`/`binary_compressed`, writes `ascii`/`binary`), `permas`, `ply`, `pvd` (a time-indexed collection of `.vtu` steps), `pvtp`, `pvtu` (parallel indices over per-part pieces; index and pieces sit side by side in MEMFS), `radioss` (OpenRadioss starter `_0000.rad`, read and, since v16.17.0, written; `#include` files resolved next to the deck in MEMFS, see [Radioss](./formats/radioss.md)), `radioss_anim` (OpenRadioss animation files `<run>A001`…, **read-only**, v16.11.0; see [OpenRadioss animation files](./formats/radioss_anim.md)), `radioss_th` (OpenRadioss time-history files `<run>T01`, **read-only**, v16.12.0, each output a step through `timeStep`; see [OpenRadioss time-history files](./formats/radioss_th.md)), `lsdyna_binout` (LS-DYNA's binout, **read-only**, v16.12.0; see [LS-DYNA binout](./formats/lsdyna_binout.md)), `stl`, `su2`, `svg` (**write-only**, 2D visualization), `tecplot` (`.dat`/`.tec`, and binary `.plt` read-only), `tetgen`, `tikz` (**write-only**, 2D LaTeX visualization), `triangle` (`.poly` by default; see the ambiguous-extensions table for `.node`/`.ele`), `ugrid`, `unv` (`.unv`/`.uff`; groups become regions, results are steps — see [UNV](./formats/unv.md)), `vti` (VTK XML ImageData: a regular lattice, so writing needs one — see [VTI](./formats/vti.md)), `vtk`, `vtkhdf` (HDF5-backed: UnstructuredGrid, PolyData and the two composite types, including polyhedra, partitions and time; `piece`/`timeStep` select what to read), `vtp`, `vtu` (zlib compression works via Emscripten's built-in port), `wkt`, `xdmf` (XML, Binary **and** HDF), `xplt` (FEBio plot files, **read-only**; every state is a step, see [FEBio plot files](./formats/xplt.md)), `xyz` (headerless ASCII point clouds, see [XYZ](./formats/xyz.md)), `z88` (Z88 `z88i1.txt`, found by file name; its `z88o2.txt`/`z88o3.txt` results are read from beside it, see [Z88](./formats/z88.md)). The three field-only formats (`dex`, `ip`, `mff`) read/write geometry-less meshes (field values in `point_data`).
...
- `encoding` — `'ascii'` or `'binary'`; throws for a format with only one variant. `'raw_appended'` (since v16.21.0) writes a `vtu`'s arrays as raw binary in one `<AppendedData>` section and throws for any other format.
