...
Some formats carry metadata a generic `Mesh` cannot represent at all: OpenFOAM patch names/types, MED field units and family groups, MDPA entity names, Ansys/UNV named point and cell sets, Gmsh `$Entities` bounding-entity tags, Exodus info/QA records. `readMeshSelective(path, {info: true})` attaches it as the result's `.info` for the formats that have one; `writeMesh(path, mesh, format, {info})` writes it back (or, with no explicit `options.info`, reuses `mesh.info` when its own `format` field matches the write target — a `{info: true}` read round-trips through a write with no extra plumbing):
...
**As of v8.0.0 the WASM build ships every format the C++ core has**, including the five that need HDF5 or netCDF — there is no longer a WASM-specific format gap. That is 46 readable and 49 writable format keys (`svg`, `tikz` and `gmsh22` are write-only). `gid` gained a reader in v10.19.0 and is now both. `.vts`/`.vtr`/`.vtm` (v11.6.0, tier B4) added three of each, none needing a `js_bindings.cpp` change at all -- see the format list above. `.pvd`/`.pvtu`/`.pvtp` (v14.1.0, roadmap §1.1) added three more of each, again with no `js_bindings.cpp` change: they are registered in `registry.cpp` and reached through the existing read options. `openfoam` became writable in v9.20.0; `gid` (the GiD postprocess format, on a vendored gidpost) needed no WASM-specific binding at all — it reaches every registry-driven consumer, including this one, purely by being registered in the shared `registry.cpp` writer/extension tables, and `-sUSE_ZLIB=1` supplies the `<zlib.h>` its vendored C sources need at the compiler (not just the linker).
...
`abaqus`, `ansys`, `ansysinp` (read/write), `avsucd`, `cgns`, `dex`, `dolfin`, `ensight` (EnSight Gold geometry, `.case`/`.geo`, ASCII + C-binary), `exodus`, `flac3d` (including `ZGROUP`/`FGROUP` cell groups, which ride on the mesh object as named regions — see [FLAC3D](./formats/flac3d.md#data-mapping)), `flux`, `freefem`, `gid` (GiD postprocess; *writing* rides a vendored gidpost and needs zlib, *reading* is meshio++'s own code and needs nothing for the ascii flavour), `gmsh`, `h5m`, `hmf`, `ip`, `mdpa` (Kratos; mesh-level blocks only — see [MDPA](./formats/mdpa.md#c-core)), `med`, `medit`, `mff`, `mfm`, `mphtxt`, `nastran`, `netgen`, `obj`, `off`, `openfoam` (read/write; writing creates a `constant/polyMesh` directory in MEMFS), `permas`, `ply`, `pvd` (a time-indexed collection of `.vtu` steps), `pvtp`, `pvtu` (parallel indices over per-part pieces; index and pieces sit side by side in MEMFS), `stl`, `su2`, `svg` (**write-only**, 2D visualization), `tecplot`, `tetgen`, `tikz` (**write-only**, 2D LaTeX visualization), `triangle` (`.poly` by default; see the ambiguous-extensions table for `.node`/`.ele`), `ugrid`, `unv`, `vti` (VTK XML ImageData: a regular lattice, so writing needs one — see [VTI](./formats/vti.md)), `vtk`, `vtkhdf` (HDF5-backed: UnstructuredGrid, PolyData and the two composite types, including polyhedra, partitions and time; `piece`/`timeStep` select what to read), `vtp`, `vtu` (zlib compression works via Emscripten's built-in port), `wkt`, `xdmf` (XML, Binary **and** HDF). The three field-only formats (`dex`, `ip`, `mff`) read/write geometry-less meshes (field values in `point_data`).
