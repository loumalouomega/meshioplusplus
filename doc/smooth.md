...
- **C API** — `mio_smooth(mesh, method, iterations, lambda, mu, fix_boundary, preserve_features, feature_angle, guard_inversion, &nodes_moved, &max_displacement, &skipped_inversion)` returns a plain `mio_mesh*`; the three counter out-params are individually optional. `method` is passed straight through, so `"odt"` needs no C API surface of its own beyond the string. `frozen` is reached through `mio_smooth_opts`/`mio_smooth_ex` (the `mio_refine_opts`/`_ex` pattern): `opts.frozen`/`opts.num_frozen` name 0-based point ids to pin outright, range-checked (an out-of-range id fails the call by name); `mio_smooth` itself is exactly `mio_smooth_ex` with `frozen` unset. See the [C API reference](/c_api).
- **Fortran** — `mesh%smooth(method, iterations [, lambda, mu, fix_boundary, preserve_features, feature_angle, guard_inversion, nodes_moved, max_displacement, skipped_inversion, frozen])`; `frozen` is a 1-based `integer(int64)` array, shifted to 0-based across the ABI. See the [Fortran reference](/fortran).
- **Julia** — `smooth(mesh; method="taubin", iterations=10, ..., frozen=nothing)`; `frozen` is 1-based. See the [Julia reference](/julia).
- **R** — `mio_smooth(mesh, method="taubin", iterations=10L, ..., frozen=NULL)`; `frozen` is 1-based. See the [R reference](/r).
