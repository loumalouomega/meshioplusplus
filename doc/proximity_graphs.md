...
Both searches are `O(N + E)` in the bucket grid, and `E` is what dominates: a radius graph's edge count grows as the cube of the radius, so the honest unit is edges per second rather than points. Measured on one core over a uniform cloud at unit density with the numpy search (before v16.17.0):
