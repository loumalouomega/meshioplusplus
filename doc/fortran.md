...
Format inference is the shared registry's, the C API's: the extension picks the format, with four exceptions: a `.mesh` file whose first line names an MFEM mesh is read as `mfem` rather than `medit` (v16.5.0), Z88's fixed file names (`z88i1.txt`, `z88o2.txt`, …) are `z88` rather than xyz (v16.7.0), a `.dat` file that opens as an MSC Marc input deck is `marc` rather than `tecplot` (v16.8.0), and a file named `d3plot`, which has no extension, is LS-DYNA's `lsdyna_d3plot` (v16.9.0). Formats added to the registry (Patran, Femap and MFEM in v16.5.0; libMesh, Z88, Abaqus `.fil` and Radioss in v16.7.0; Marc's `marc` and `marc_t19` and the `ansys_rst_cyclic` full rotor in v16.8.0; `lsdyna_d3plot` and `nastran_op2` in v16.9.0) reach this binding with no code change; `mio_format_readable`/`mio_format_writable` report what the linked library holds.
