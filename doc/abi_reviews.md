...
| 4 | v9.9.0 | `formats/cgns.hpp`, `version.hpp` | `version.hpp` is the routine release bump (macro values only, no declaration). `formats/cgns.hpp`'s change is **comment-only**: `write_cgns`/`read_cgns`'s declarations are byte-identical to v9.8.0's, and the v9.9.0 `FlowSolution_t` work (point/cell data) is entirely inside the `.cpp`. Nothing a consumer compiles against changed at all — not a declaration, not a default argument, not a type — so a translation unit built against v9.8.0 headers is identical either way. As in the v9.8.0 row, the observable difference is in the *bytes written to a file*, which is a file-format change rather than an ABI one. |
