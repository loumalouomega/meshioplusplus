...
- **VTU/VTP**: the file is still read and XML-parsed, because pugixml always materializes PCDATA. What is skipped is base64 decoding (parallel and decoded straight into the target array since v16.20.0, and often the larger half), decompression, allocation and byte-swapping. Expect a solid multiple, **not** an asymptotic change. The *appended* data format (`WriteEncoding::RawAppended`, `--appended`; see [VTU](formats/vtu.md)) is read too, in one pass since v16.21.0.
