...
The header bundles [pugixml](https://pugixml.org/) (MIT), so the XML-based formats (VTU, XDMF-XML, DOLFIN) work out of the box.
...
Everything defaults to a zero-dependency build. Define these **before** including the header to change that:
...
Optional-dependency code stays behind its `MESHIOPLUSPLUS_HAS_*` guard, so the default include compiles with no third-party libraries at all — the same format set as the [WebAssembly build](wasm.md).
