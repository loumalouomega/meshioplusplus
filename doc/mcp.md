...
87 tools; the six marked *gated* need a further extra and return a named install error without it. Transforming tools take `input_path`/`output_path` (+ optional `input_format`/`output_format`, otherwise inferred from the extension) and return the written path plus a mesh summary and the operation's report.
...
| `surface_watertight_check` | a surface's boundary, non-manifold, inconsistent-orientation and degenerate counts (the same check `voxelize`/`compute_sdf`/`distance_to_surface`/`sample_distance` skip via `watertight_check="off"` when called as tools) |

### Conversion

| Tool | Notes |
|---|---|
| `convert` | any-to-any format conversion; `points_only`/`arrays`/`time_step` narrow the read and `piece` keeps one partition/block of a partitioned file (VTKHDF, `.pvtu`/`.pvtp`, one part of a `.pvd` step, or one rank of a parallel MFEM mesh) instead of the merged mesh and `ghosts: keep\|drop` keeps or removes the ghost cells (halo) of a `.pvtu`/`.pvtp`/`.pvd` (`stats` takes both too); `mode: ascii\|binary\|raw_appended` (`raw_appended`: a `.vtu`'s arrays as raw binary in one `<AppendedData>` section) and `compression: zlib\|lz4\|zstd\|lzma\|gzip\|lzf\|none` (`lzf` is PCD's `binary_compressed`; `gzip`/`none` also for `vtkhdf`; the VTK XML block codecs also reach `.pvd`, `.pvtu` and `.pvtp`, whose pieces they compress) subsume the CLI's `ascii`/`binary`/`compress`/`decompress` verbs; `grid_functions` (`{name: path}`) reads MFEM `.gf` fields onto an MFEM `.mesh` input, `patran_results` (`{name: path}`) reads Patran 2.5 result files (`.nod`/`.dis` nodal, `.els` element, text or binary) onto a Patran neutral input, and `write_grid_functions` writes an MFEM output's data as `<stem>.<name>.gf` files beside it (a `.mesh` target is then MFEM, not Medit); `elmer_halo` adds ElmerGrid's `-halo` layer to the `partitioning.<N>` an Elmer output writes from `partition:part` |
...
| `sequence` | run a multi-file / transient [sequence](sequences.md) (`input_pattern` **or** `input_paths`, `output_path`; optional `mode`/`times`/`time_from`). A `{step}`/`{index}` token in `output_path` writes one file per step (fan-out); a plain path writes one multi-step file (fan-in, only for `xdmf`, `gid`, `usd`, `vtkhdf`, `pvd` and `femap` — anything else fails by name rather than keeping step 0). Ordering is natural-numeric, so `out_9` precedes `out_10`. A pattern's **directory** component is containment-checked against the sandbox root before it is expanded, and every matched file is re-checked individually |

### Mesh operations

`extract_surface`, `extract_skin`, `reorder` (reports bandwidth before/after), `clean`, `crop` (bbox, half-space, or a `where_array`/`where_compare`/`where_value` `cell_data` predicate), `slice`, `isosurface`, `voxelize` (a regular grid around a mesh, `fill` `all`/`surface`/`interior` narrowing it to the shell or the interior — see [mesh and regular grids](/grids)), `compute_sdf` (a grid over a surface, filled — `structure` `voxel`/`octree`), `distance_to_surface` (the signed distance from a mesh's points or cell centres to a second `surface_path` mesh, attached as `point_data`/`cell_data`; `band` narrows it to a shell and reports `num_banded` — see [signed distance](/sdf)), `sample_distance` (the same signed distance for a bare list of `[x, y, z]` points against a surface, with no output mesh — returns `distances` directly), `transform`, `convert_cells`, `tessellate` (isoparametric subdivision of the five higher-order cell types onto a refinement lattice, with a provenance map `Tessellation.gather`/`.scatter`/`.aggregate` use to move data between the tessellated mesh and its source; reports `num_curved_source_cells`/`num_pass_through_source_cells`/`watertight` — see [curved tessellation](/tessellation)), `subdivide` (polyhedral refinement: one polyhedral child per 3D cell face, connected to a new interior point — no per-type template table, unlike `refine`; see [subdivide](/subdivide)), `agglomerate` (polyhedral coarsening: merge groups of cells into single larger polyhedral cells via greedy seed-and-grow over the shared-face dual; see [agglomerate](/agglomerate)), `refine` (uniform, or a subset via `cells`/`region`/`where` with a conforming `closure`; `record_hierarchy` attaches the persistent `refine:cell_id`/`refine:parent_id` parent/child hierarchy a multigrid caller resolves across the sequence of meshes it keeps — see [refine](/refine#refinecell_id-and-refineparent_id)), `undo_green` (restores a `refine` transitional/green cell to its coarse parent, read verbatim from `coarse_path` — a two-mesh tool like `interpolate`; reports `num_groups_undone`/`num_cells_removed` — see [green-element undo](/undo_green)), `decimate`, `decimate_volume` (the volume-mesh sibling of `decimate`: quadric-error tet-edge collapse; `preserve_boundary` defaults `False`, the opposite of `decimate`'s own default, since boundary vertices participate by real quadric error here — see [volume decimation](/decimate_volume)), `remesh` (replaces a surface's triangulation with a new, well-shaped one at a chosen vertex count by ACVD clustering — the output has NO correspondence to the input, point_data/cell_data/named regions are dropped; see [remesh](/remesh)), `remesh_volume` (the volumetric sibling of `remesh`: retetrahedralizes a volume mesh or closed surface at a chosen resolution by isosurface stuffing over a BCC lattice — same no-correspondence output contract, plus `warp_fraction` trading boundary tet quality for a small, measured chance of non-manifold boundary edges reported as `num_non_manifold_edges`; see [remesh_volume](/remesh_volume)), `optimize_volume` (ODT-remeshes a tetrahedral mesh: raises its worst element quality by relocating vertices AND flipping connectivity (2-3/3-2, predicate-free) — the point set is invariant so point data and Point regions carry, cell data and Cell/Side regions are dropped; tet-only and C++-core-only; see [optimize_volume](/optimize_volume)), `smooth` (`method` `taubin`/`laplacian`/`odt` — `odt`, optimal-Delaunay-triangulation smoothing, is tet-only and C++-core-only with no pure-Python fallback), `merge` (N inputs), `split` (one file per piece, `name_template`), `partition` (one file per part, or one `.pvtu`/`.pvtp` parallel index over every part when `name_template` ends in that extension and has no `{part}`, which keeps `ghost_layers` halos as `vtkGhostType` and returns the index as `index`), `interpolate` (source → target field transfer), `conservative_interpolate` (mass-preserving overlap-measure weighted transfer — unlike `interpolate`, an unset `arrays` covers every source point_data AND cell_data array; see [conservative interpolation](/conservative_interpolate)), `gradient` (the gradient, divergence or curl of a `point_data` field — see [field derivatives](/gradient); reports `num_skipped` and `num_fallback`), `hessian` (the Hessian, second derivative, of a **scalar** `point_data` field — a composition of two `gradient` calls, `gradient`'s companion one order further; see [second derivatives](/hessian); reports `num_skipped` and `num_fallback`), `estimate_error` (the Zienkiewicz-Zhu recovery-based error indicator of a `point_data` field, plus optional `absolute`/`fraction`/`dorfler` marking into `error:marked` for `refine`'s own `where` selector — see [error estimation](/error); reports `global_error`, `num_skipped` and `num_marked`). `export_gltf` (the surface of a mesh as glTF 2.0, `.glb` or `.gltf` plus a `.bin`, for three.js, Blender and dashboards: normals split at creases (`split_angle`), point fields as raw `_NAME` attributes, `color_by` baked into an unlit `COLOR_0`, one node per cell region, Y-up with the axis change, `scale` and recentring on the root node — `convert` writes the same format with the defaults; see [glTF](/formats/gltf)). `curvature` (per-vertex mean and Gaussian curvature by the angle defect and the cotangent Laplace-Beltrami operator, over no input array at all — top-level like `quality`/`stats`, not a `data` verb — `dual_area` `mixed-voronoi` (default, C++-core-only) or `barycentric` (twinnable in numpy); reports `total_angle_defect`, the Gauss-Bonnet invariant (`2*pi*chi`, `4*pi` for a closed genus-0 surface), plus `num_boundary`/`num_isolated`/`num_degenerate` and the input's surface `quality` — see [curvature](/curvature)). `normals` (point and cell normals of a surface, written as `normals`; `split_angle` set to a number of degrees in `[0, 180]` duplicates points at creases so every point carries exactly one normal, appending the copies after the original points while cells keep their numbering — never reorients, so it reports the input's surface `quality` (`inconsistent_pairs` nonzero means some normals average faces that disagree about which side is out) plus `num_added_points`/`num_split_points`/`num_isolated`/`num_undefined`/`num_degenerate` — see [normals](/normals)). `repair` (a surface's orientation, holes and pinched vertices -- the three defects `clean` does not touch: triangles are rewound by the topological half-edge rule (exact across any crease, where a normal-angle test is not), boundary loops of at most `max_hole_edges` edges are fan-filled consistently with the surrounding surface, bowtie vertices are duplicated and closed components oriented outward; reports both the input's and the output's defect counts, so what was fixed and what remains are both visible -- see [surface repair](/repair)). `shrinkwrap` (projects every selected point of the input onto the surface of a second `target_path` mesh -- one projection, no iteration, optionally offset along the hit FEATURE's pseudonormal (the bisector at a crease) rather than the selected triangle's normal; every point of the source moves whatever cells it carries and only the target must be a surface, `weights` names a `point_data` array that selects or blends, and a point beyond `max_distance` is left alone and counted -- see [shrinkwrap](/shrinkwrap)). `sobolev_deform` (smooths a raw per-point displacement field through the mesh's own P1 operators and moves the points by the result -- a screened-Poisson low-pass filter whose cutoff wavelength is `length_scale`, which turns a jagged displacement into one a mesh can follow without tangling; every top-dimensional block must be a linear simplex, nothing is pinned by default (so a constant displacement is preserved exactly) and non-convergence is reported through `converged` with the last iterate returned -- see [Sobolev deformation](/sobolev_deform)). `grid_sample` (a mesh's point data onto a regular lattice, written as a `.vti` grid — the step a convolutional or superresolution model needs before it can see a mesh at all; reports the channel names and the `coverage`, the fraction of grid points inside the mesh, since a grid around a concave domain is mostly fill and a model would learn it), `grid_scatter` (the inverse, by trilinear interpolation — consecutive `name_0`/`name_1`/`name_2` channels rebuild one multi-component array), `grid_resample` (onto a finer or coarser lattice over the same box — the baseline a superresolution model has to beat) and `grid_power_spectrum` (azimuthally averaged; whether a super-resolved field carries the right small-scale content, which a pointwise error cannot see — see [mesh and regular grids](/grids)). `proximity_graph` (a radius or k-nearest-neighbour graph over a mesh's points or cell centroids, written as `line` cells with a `degree` array — the neighbourhoods a particle state needs, since it has positions and no connectivity; `box_size` turns on the periodic minimum-image convention and reports the degree spread and the isolated count — see [proximity graphs](/proximity_graphs)). `subsample` (a mesh reduced to a point cloud of exactly `count` points under a token budget — `method` `farthest` for exact farthest-point sampling, `grid` for the `O(N + count²)` lattice-then-farthest variant a million-point cloud needs, `random` for the baseline; `bounds` restricts the candidates to a box and `record_ids` attaches `budget:original_point_id`; the cells and `cell_data` are dropped, the point data and Point regions kept — see [point-cloud budgets](/point_budgets)). Parameters mirror the Python API / CLI one-to-one; operations that produce reports (`clean`, `decimate`, `decimate_volume`, `remesh`, `remesh_volume`, `optimize_volume`, `smooth`, `gradient`, `hessian`, `estimate_error`, `curvature`, `normals`, `repair`, `shrinkwrap`, `sobolev_deform`, `tessellate`) include them in the response.
