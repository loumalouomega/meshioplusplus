...
| Tool | Notes |
|---|---|
| `convert` | any-to-any format conversion; `points_only`/`arrays`/`time_step` narrow the read and `piece` keeps one partition/block of a partitioned file (VTKHDF, `.pvtu`/`.pvtp`, or one part of a `.pvd` step) instead of the merged mesh and `ghosts: keep\|drop` keeps or removes the ghost cells (halo) of a `.pvtu`/`.pvtp`/`.pvd` (`stats` takes both too); `mode: ascii\|binary` and `compression: zlib\|lz4\|zstd\|lzma\|gzip\|none` (`gzip`/`none` also for `vtkhdf`; the VTK XML block codecs also reach `.pvd`, `.pvtu` and `.pvtp`, whose pieces they compress) subsume the CLI's `ascii`/`binary`/`compress`/`decompress` verbs |
