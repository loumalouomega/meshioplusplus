...
```cmake
find_package(meshioplusplus REQUIRED)  # pin a minimum version if you need one, e.g. `meshioplusplus 16.0`
                                       # (compatibility is SameMajorVersion: a 15.x request rejects a 16.x install)
...
Variants mirror the Conan options / vcpkg features above (`+hdf5`/`+netcdf`/`+zlib`/`+zstd`/`+lz4`/`+bzip2`/`+kahip`/`+cgnslib`/`+adios2`, all named the same way) plus `+fortran`, `+cxx_api` (the [installable C++ API](https://loumalouomega.github.io/meshioplusplus/cpp_api), with `cxx_api_backends=meshio,native,kratos` selecting which mesh-backend libraries to install), `parallel=auto|seq|stl|openmp|tbb|kokkos`, and `mesh_backend=meshio|native|kratos` for the standalone build. Two Conan/vcpkg options are deliberately **not** Spack variants: `with_tecio`, because Spack's [`tecio`](https://github.com/spack/spack-packages/blob/develop/repos/spack_repo/builtin/packages/tecio/package.py) is `manual_download` and the `.szplt` reader needs a Tecplot install, and `with_gidpost`, because gidpost is vendored in-tree and hard-gated on zlib rather than on a Spack package, so it follows `+zlib`. `with_polyscope`, `with_eigen` and `with_json` are likewise not variants: the first two are vendored git submodules the release tarball omits, and the third degrades to a no-op when its vendored copy is absent. See [Installation → Spack](installation.md#spack) for the matching `py-meshioplusplus` Python package.
...
| Lifecycle & I/O | `mio_mesh_create`, `mio_mesh_free`, `mio_read`, `mio_write`, `mio_convert` (format `NULL`/`""` = infer from extension; `.msh` → gmsh, `.inp` → abaqus, `.node`/`.ele` → tetgen — pass `"triangle"` for 2D Triangle pairs; `.mesh` → medit, unless the file's first line names an MFEM mesh, which goes to `mfem`; Z88's fixed file names `z88i1.txt`, `z88structure.txt`, `z88o2.txt`, `z88o3.txt` → `z88` rather than xyz; `.dat` → tecplot, unless the file opens as an MSC Marc input deck, which goes to `marc`; `.plt` → tecplot (binary, read-only); a file named `d3plot` → `lsdyna_d3plot`; a file named like `crashA001` (a stem, `A` and three or more digits, no extension) → `radioss_anim`; a file named `binout` (or `binout0000`…) → `lsdyna_binout`; a file named like `crashT01` (a stem, `T` and two digits, no extension) → `radioss_th` — the seven exceptions to the extension rule, shared by Fortran, Julia, R and WASM. The two content checks are for reading: `mio_write` and `mio_convert`'s output go by the extension alone, so a `.dat` is written as Tecplot and a `.mesh` as Medit whatever the file being replaced holds (v16.17.0); pass `"marc"` to write a Marc deck) |
...
> **Four Python-only formats are absent here.** `pmsh`, `zarr`, `cae` and `usd` (v10.35.0, the physics-ML data path — see [formats](formats.md)) are registered in the Python layer only, not in the shared `registry.cpp` dispatch tables, so this ABI does not see them.
...
- `mio_read_opts.drop_ghosts` — nonzero removes the ghost cells (halo) of a partitioned `.pvtu`, `.pvtp` or `.pvd`: every cell with a `vtkGhostType` bit set, and the points only they used. Zero (the default) keeps them; every other reader ignores it. It took the fifth former `reserved` slot (`reserved[2]` → `reserved[1]`), so `sizeof(mio_read_opts)` (80, now guarded by a `static_assert` in `c_api.cpp`) and every preceding offset are unchanged. The C++ `ReadOptions::mGhosts` behind it is what took the C++ ABI to 15, so a consumer of the installed C++ headers rebuilds; the C API is unaffected. See [`doc/selective_read.md`](selective_read.md#dropping-ghost-cells).
