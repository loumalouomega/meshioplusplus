...
| Lifecycle & I/O | `mio_mesh_create`, `mio_mesh_free`, `mio_read`, `mio_write`, `mio_convert` (format `NULL`/`""` = infer from extension; `.msh` → gmsh, `.inp` → abaqus, `.node`/`.ele` → tetgen — pass `"triangle"` for 2D Triangle pairs; `.mesh` → medit, unless the file's first line names an MFEM mesh, which goes to `mfem` — the one content-aware default, shared by Fortran, Julia, R and WASM) |
