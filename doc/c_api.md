...
| Lifecycle & I/O | `mio_mesh_create`, `mio_mesh_free`, `mio_read`, `mio_write`, `mio_convert` (format `NULL`/`""` = infer from extension; `.msh` → gmsh, `.inp` → abaqus, `.node`/`.ele` → tetgen — pass `"triangle"` for 2D Triangle pairs; `.mesh` → medit, unless the file's first line names an MFEM mesh, which goes to `mfem`; Z88's fixed file names `z88i1.txt`, `z88structure.txt`, `z88o2.txt`, `z88o3.txt` → `z88` rather than xyz; `.dat` → tecplot, unless the file opens as an MSC Marc input deck, which goes to `marc`; `.plt` → tecplot (binary, read-only); a file named `d3plot` → `lsdyna_d3plot` — the four exceptions to the extension rule, shared by Fortran, Julia, R and WASM) |
