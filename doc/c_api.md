...
`mio_write_ex` takes a `mio_write_opts*` whose `encoding` is `MIO_ENCODING_DEFAULT`, `_ASCII`, `_BINARY` or, since v16.20.0, `MIO_ENCODING_RAW_APPENDED` (`vtu` only: raw binary arrays in one `<AppendedData>` section; any other format fails by name).
