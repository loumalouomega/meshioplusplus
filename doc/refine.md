...
- **WebAssembly / JavaScript** — `refine(mesh, levels, recordParentIds, options, returnMaps)` where `options` is `{cells, region, array, compare, value, closure, recordLevels, recordHierarchy}`, returning the bare mesh or `{mesh, pointMap, cellMaps}` when `returnMaps` is set; the `convertSurfaceOps` pipeline op `{op: 'refine', ...}` takes the same fields. The comparison is spelled `compare` there because `op` is the pipeline step's own discriminant. See the [WebAssembly reference](/wasm).
