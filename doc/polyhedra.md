...
Reading polyhedra: **MED** (`POE`), **EnSight** (`nfaced`), **OpenFOAM**, **VTU** (type 42), **VTKHDF** (type 42 plus the four face arrays, since v14.0.0) and **CGNS** (`NGON_n`+`NFACE_n`). CGNS additionally reads ADF containers and the CGNS 3.x section layout on a build with the optional [cgnslib backend](/formats/cgns). **Tecplot** (`FEPOLYHEDRON`, ASCII and `.plt`) since v16.11.0. Reading jagged polygons: **MED** (`POG`/`POG2`), **EnSight** (`nsided`), **VTP**, **OpenFOAM**, **Tecplot** (`FEPOLYGON`, since v16.11.0).

Writing polyhedra: **MED** (`POE`), **EnSight** (`nfaced`) and **VTU** (type 42) since v9.19.0; **VTKHDF** since v14.0.0 (declared as VTKHDF 2.5); **OpenFOAM** since v9.20.0, where they are the native cell shape; **CGNS** (`NGON_n`+`NFACE_n`) since v9.21.0. **Tecplot** ASCII (`FEPOLYHEDRON`) since v16.11.0. Writing jagged polygons: **MED** (`POG`/`POG2`), **EnSight** (`nsided`), **VTP**, **CGNS** (`NGON_n`), **Tecplot** ASCII (`FEPOLYGON`, since v16.11.0).

**VTKHDF's encoding** differs from VTU's face stream: four arrays, `FaceConnectivity`/`FaceOffsets` (the faces, `NumberOfFaces + 1` boundaries per piece) and `PolyhedronToFaces`/`PolyhedronOffsets` (which faces make each cell, with `NumberOfCells + 1` boundaries that cover *every* cell, so a non-polyhedral cell is a zero-length span and a mixed mesh needs no `-1` sentinel). The face table may be shared between neighbouring polyhedra; meshio++ owns faces per cell, so the sharing is expanded on read and faces are not de-duplicated on write. Node counts bucket a polyhedron run into `polyhedron<N>` blocks **per piece**, so a piece stays one contiguous range. See [VTKHDF](/formats/vtkhdf#polyhedra).
