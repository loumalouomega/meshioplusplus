...
**FEBio `.xplt`** joined in v16.2.0: its steps are the states of the plot file, one per saved time. `read_xplt_metadata` reads each state's header (inflating it when the file is compressed), and `time_step` then reads only that state's data. The state's time, index and status are attached as `field_data["meshio:time"]`, `xplt:step` and `xplt:status`, and `convert run.xplt 'out_{step}.vtu'` fans a run out to one `.vtu` per state. The format is read-only, so it is never a fan-in target. See [FEBio plot files](formats/xplt.md).

**Ansys `.rst`/`.rth`** joined in v16.3.0: its steps are the result sets of the file (load steps, substeps or modes), in the order of its data-set index. `read_ansys_rst_metadata` reads the headers and the model and takes every set's time, or frequency in a modal analysis, from the time table, so `convert file.rst 'mode_{step}.vtu'` writes one file per mode. `time_step` then reads only that set's nodal solution. See [`ansys_rst`](./formats/ansys_rst.md).
...
'gid', 'vtkhdf' and 'pvd' can); write one file per step with an Output path containing
'{step}' instead
```
