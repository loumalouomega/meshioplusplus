...
- **Tensor-aware results** — `GiD_Matrix`/`MainMatrix`/`Complex*` have no meshio++ counterpart: the writer splits a k∉{1,2,3} array into k scalars rather than risk a wrong component order (GiD's symmetric-tensor order differs from meshio/VTK's), and the reader returns a k-component array with the declared type dropped rather than reinterpreted. The result is a documented round-trip asymmetry. An opt-in `GidInfo` side channel (the `MedInfo`/`GmshInfo` shape) carrying the declared type and component order is the natural home; note it would be a **Tier A** ABI change. **M**
