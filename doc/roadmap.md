...
- **Hoist the per-element dtype switch in the remaining formats.** Through the private `detail/typed_view.hpp` (`Int64View`, `DoubleView`: an array as `read_int`/`read_double` read it, zero-copy when the dtype matches), as the VTK XML and Exodus loops already do. About 400 per-element `read_double`/`read_int` calls remain in the formats, the most in `gmsh` (35), `cgns` (32), `med` (20), `libmesh`, `febio`, `gid`, `z88` and `pcd`; `write_double`/`write_int` dispatch per element too. **S each**
...
- **Waiting for the next ABI bump.** Every serial phase this section once listed inside an operation is delivered (v16.16.0 to v16.19.0, in the CHANGELOG), and `test_op_goldens.cpp` pins 28 cases to digests taken from the previous implementation. What is left needs an installed layout to change: **M**
