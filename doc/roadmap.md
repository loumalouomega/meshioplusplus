...
Status at time of writing: **v10.28.0** — 43 formats, thirty-four mesh operations + five data operations, six language surfaces (Python / C / Fortran / Julia / R / WASM), two viewers plus a browser dataset manager, a Blender add-on, an MCP server, a settings-driven pipeline engine, a dataset-manifest layer with a PhysicsNeMo adapter, and a versioned ABI (`MESHIOPLUSPLUS_ABI_VERSION` 11).
...
- **Grid/CNN data path (superresolution)** — *mostly shipped*: the transfer in v10.27.0 ([`doc/grids.md`](grids.md)) and the dataset side in v10.28.0 — an optional `Target` block pairing a coarse and a fine series on a manifest entry ([`doc/datasets.md`](datasets.md#paired-cases)), plus `grid_sample_pair`/`iter_grid_samples`/`grid_stats` and `GridSpec.upscale_samples`, the sample-count upscale a convolutional model actually needs. What remains is one thing: an **`srresnet` family in `TrainSpec`**, with the grid construction recorded on the model card alongside the column contract. **S**
