...
Status at time of writing: **v10.27.0** — 43 formats, thirty-four mesh operations + five data operations, six language surfaces (Python / C / Fortran / Julia / R / WASM), two viewers plus a browser dataset manager, a Blender add-on, an MCP server, a settings-driven pipeline engine, a dataset-manifest layer with a PhysicsNeMo adapter, and a versioned ABI (`MESHIOPLUSPLUS_ABI_VERSION` 11).
...
- **Grid/CNN data path (superresolution)** — *partly shipped in v10.27.0*: the transfer itself is in place ([`doc/grids.md`](grids.md)) — `GridSpec`/`GridArray`, `sample_grid`, `scatter_grid`, trilinear `interpolate_grid`/`resample_grid`, `squeeze_grid`/`expand_grid`, and an azimuthally averaged `power_spectrum`, plus four CLI verbs and four MCP tools. What remains is the two ends of it: a **coarse/fine pairing rule** on a dataset entry, so a manifest can describe a superresolution dataset rather than a single-mesh one; and an **`srresnet` family in `TrainSpec`**, with the grid construction recorded on the model card alongside the column contract. **S**
