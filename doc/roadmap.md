...
*Recommended posture:* the text and VTU I/O, then the boundary items as their consumers ask; the operations' layout-bound leftovers with the next ABI bump. Every item is judged by the determinism check (`--hash`, v16.15.0).
...
4. **Performance ([§4](#_4-performance))** — the text and VTU I/O, then the boundaries; the operations' layout-bound leftovers with the next ABI bump.
