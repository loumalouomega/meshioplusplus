...
- **What welding leaves.** Since v16.17.0 `clean` and `merge` share `detail/weld.hpp`: cell keys, a sort of the points into cells and each cell's neighbour cells are parallel, and the keep-first loop walks per-cell representative lists by index (it stays serial by design: chains make it order-dependent); `clean`'s rectangular duplicate-cell pass runs on the sort-based table. `clean`'s ragged and polyhedral duplicate passes still build a `std::string` key per cell, and `merge` still deduplicates cells through a `std::map` keyed by node vectors (`drop_duplicate_cells`, off by default). **S**
- **Distance-kernel grid insertion.** `build_distance_query` inserts every triangle's bounding box into `SpatialGrid`'s hash map serially; the triangle soup, the vertex normals and the edge table are parallel or sort-based since v16.17.0. Moving the grid off its `unordered_map` changes `DistanceQuery`'s installed layout (Tier A). `sample_distance`, `distance_to_surface`, `shrinkwrap` and `remesh_volume` still build two edge tables where one would do. A BVH is deliberately *not* the fix: `detail/surface_distance.hpp` rejects one because its visit order would make the equidistant-triangle tiebreak observable. **S–M**
...
- **Operations with no parallel phase at all**: `remesh` (serial clustering), `remesh_volume` (serial lattice, classification and cut), `split` (a serial union-find), `hessian` (its own cost is `clone_mesh` and per-block deep copies around two parallel `gradient` passes) and `compute_normals` (`normals.cpp`). `agglomerate` computes its face areas once in parallel and keeps no hash map per seed, and `undo_green` groups by sorting, since v16.17.0. `compute_quality` (chunked partial histograms merged in chunk order) and `sobolev_deform` (a gather-form sparse product, one thread per vertex and no scatter) are the in-repo models. **L in total**
  ...
  - **Gate.** `test_op_goldens.cpp` pins `split`, `hessian` and `compute_normals` to their current output; add `remesh` and `remesh_volume` cases before changing them.
...
*Recommended posture:* the text and VTU I/O, then the boundary items as their consumers ask; the remaining operation items (dtype hoists, the operations with no parallel phase, welding's leftovers) alongside. Every item is judged by the determinism check (`--hash`, v16.15.0).
...
4. **Performance ([§4](#_4-performance))** — the text and VTU I/O, then the boundaries; the remaining operation items alongside.
