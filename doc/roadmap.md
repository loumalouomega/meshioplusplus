...
Status at time of writing: **v15.3.0** — 52 core formats plus four Python-only physics-ML ones, thirty-eight mesh operations + five data operations, six language surfaces (Python / C / Fortran / Julia / R / WASM), two viewers plus a browser dataset manager, a Blender add-on, a ParaView plugin, an MCP server, a settings-driven pipeline engine, a dataset-manifest layer with a PhysicsNeMo adapter, and a versioned ABI (`MESHIOPLUSPLUS_ABI_VERSION` 15).
...
Sizes: **S** ≈ days, **M** ≈ two weeks, **L** ≈ a month or more, including tests, docs page and CLI wiring. Sizes assume the shared infrastructure in §1.24 exists; the first format that needs a component pays for it.
...
- **Structure.** One mesh/node per region, named; point clouds export as `POINTS` primitives and edges as `LINES`, which gives the point-cloud files (`.pcd`, `.xyz`) a web output for free.
...
### 1.2 Half-gaps of shipped formats
...
- **CalculiX `.frd`: binary layout and the `.dat` tabular file** — **S–M**. The reader of v15.3.0 handles the ASCII short and long layouts (format flags 0 and 1); flag 2 is refused by name. The binary layout is documented only through `frd2vtu` ([source](https://github.com/wr1/frd2vtu)), and the `.dat` file orders tensor components differently (`sxx syy szz sxy sxz syz`) and holds integration-point values rather than nodal ones. Read either only when a file is in hand. Also unverified: the cgx shell types 7 to 10 (`ccx` never writes them, so they are tested against a hand-written file) and the derived fields against `cgx` itself, which is not in the test environment; displacements were checked against the `.dat` print of `ccx` 2.23.
- **Tensor invariants as a data operation** — **S–M**. `derived=True` on the `.frd` reader is the only route to von Mises and principal values, and it is not carried by the generic `read`, the CLIs, MCP or the flat bindings. `data_calc` has no component indexing and no `pow`, so it cannot express them; a data operation for the invariants of a six- or nine-component array (von Mises, principal values, hydrostatic and deviatoric parts) would serve every format that stores a tensor, and let the `.frd` flag go.

---

### B. FEM interchange, solver inputs and result files — native implementations

Python meshio covers none of §1.3–§1.21 except OptiStruct `.fem` as plain Nastran bulk; these are greenfield. Cross-validation partners are named in each block.

### 1.3 I-DEAS Universal File `.unv` / `.uff` (read / write) — **S–M**
...
### 1.4 MSC Nastran HDF5 results `.h5` (read) — **S–M**
...
- **Out of scope.** OP2/OP4/punch (see §1.20); non-MSC HDF5 dialects until a file is in hand.
...
### 1.5 Code_Aster `.mail` mesh (read / write) — **S**
...
### 1.6 Altair OptiStruct `.fem` (read; Nastran reader extension) — **S**
...
### 1.7 COMSOL `.mphtxt` / `.mphbin` mesh (read / write) — **M**
...
### 1.8 FEBio `.feb` input and `.xplt` results (read / write `.feb`; read `.xplt`) — **M**
...
### 1.9 Elmer mesh directory (read / write) — **S–M**
...
### 1.10 MSC Patran neutral file `.pat` / `.out` (read / write) — **S–M**
...
### 1.11 Femap neutral file `.neu` (read; mesh write) — **M**
...
- **Out of scope.** The binary `.modfem` database (§1.23).
...
### 1.12 MFEM `.mesh` and `.gf` (read / write) — **M**
...
### 1.14 Z88 / Z88Aurora (read / write mesh; read results) — **S**
...
### 1.15 Abaqus `.fil` results file (read) — **M**
...
### 1.16 Radioss / OpenRadioss Starter `_0000.rad` (read) — **S–M**
...
### 1.17 MSC Marc input `.dat` (read) and `.t19` (read, later) — **M**
...
- **`.t19`.** Post codes and record layout are described in Volume C; a native reader is feasible and is the only licence-free route to Marc results. The binary `.t16` stays on the PyPost route (§1.23).
...
### 1.18 ANSYS `.cdb` archive (read / write) — **S–M**; `.rst` / `.rth` results (read) — **L**
...
### 1.19 LS-DYNA `d3plot` family (read) — **L**
...
### 1.20 Nastran OP2 (read) — **M–L**

Fortran-record binary of named data blocks (`GEOM1–4`, `EPT`, `MPT`, `OUGV1`, `OES1X`, `OSTR1`, `OEF1X`, `OQG1`, `OGPFB1`, `ONRGY1`…), MSC and NX/Simcenter dialects, 32/64-bit. Only after §1.4 shows demand beyond HDF5.
...
### 1.21 Tecplot binary `.plt` (read) — **M**
...
- **Out of scope.** `.szplt` — TecIO only (§1.23).
...
### 1.22 DOLFINx ADIOS2 `.bp` (read; optional ADIOS2 build) — **M**
...
### 1.23 Vendor-runtime routes (exporter scripts and optional plugins, no native parser) — **M** in total
...
- **Abaqus `.odb`.** No public specification; readable only through the ODB API on a licensed install, version-locked (`abaqus upgrade -odb`). Ship an `abaqus python` exporter script (instances → regions, steps/frames → sequence, field outputs by position) and document it; an optional C++ plugin against the ODB API comes only if a user asks. Where no licence is available, §1.15 (`.fil`) is the fallback. — [ODB2VTK (reference exporter)](https://github.com/Arris-Composites/ODB2VTK) · [abqpy](https://github.com/haiiliin/abqpy) †
- **MSC Marc `.t16`.** Read through PyPost (`py_post`), shipped with Marc/Mentat. Ship an exporter script; §1.17 covers `.t19` natively.
- **Femap `.modfem`.** COM API on Windows only. The route is "export a neutral file" → §1.11.
- **Tecplot `.szplt`.** Undocumented; TecIO only. Optional link against a user-installed TecIO, or "re-save as `.plt`" → §1.21. — [TecIO](https://tecplot.com/products/tecio-library/) †
- **ANSYS results beyond §1.18.** Document the PyDPF export route.
- **Done when.** Each route has a docs page, a script under `contrib/`, and one manual test recorded with the vendor version used.

---

### 1.24 Shared infrastructure worth building once
...
- **HDF5 schema helper, remaining half: typed compound-table reads** for Nastran `.h5` (§1.4). The rest of it — fixed-length string attributes, integer-array attributes, hyperslab row reads, chunked appendable datasets, creation-ordered groups and soft links — shipped in `detail/hdf5_util.hpp` with VTKHDF.
- **Directory-as-format support** in the path and CLI layer — Elmer, OpenFOAM, ADIOS2 `.bp`.
- **Optional-plugin mechanism and a `contrib/` script convention** — §1.22, §1.23.

### 1.25 Suggested order

1. **Cheap, high reuse:** §1.2 half-gaps (fill between larger items).
2. **First FEM wave (build tokenizer + permutation registry):** §1.3 UNV → §1.5 `.mail` → §1.6 OptiStruct.
3. **HDF5 and web outputs:** §1.4 Nastran `.h5` → §1.1 glTF (gated on `compute_normals`).
4. **Second FEM wave:** §1.7 COMSOL → §1.9 Elmer → §1.8 FEBio → §1.18 ANSYS `.cdb`.
5. **Legacy and research reach:** §1.10 Patran → §1.11 Femap → §1.15 Abaqus `.fil` → §1.12 MFEM → §1.13 libMesh → §1.14 Z88 → §1.16 Radioss → §1.17 Marc.
6. **Heavy binaries, on demand:** §1.19 d3plot (pull forward if crash-dataset users appear) → §1.18 `.rst` → §1.20 OP2 → §1.21 `.plt`.
7. **Routes:** §1.23 scripts as users ask; §1.22 when a FEniCSx user asks.
...
- EnSight Gold symmetric-tensor component order (§1.2).
- UNV quadratic node ordering against Salome, Code_Aster and pyuff samples (§1.3).
- Marc element numbers other than 7, and their node orderings, against Volume B (§1.17).
- Every link marked †.
