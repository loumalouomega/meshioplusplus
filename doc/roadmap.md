...
Status at time of writing: **v16.1.0** — 56 core formats plus four Python-only physics-ML ones, thirty-nine mesh operations + six data operations, six language surfaces (Python / C / Fortran / Julia / R / WASM), two viewers plus a browser dataset manager, a Blender add-on, a ParaView plugin, an MCP server, a settings-driven pipeline engine, a dataset-manifest layer with a PhysicsNeMo adapter, and a versioned ABI (`MESHIOPLUSPLUS_ABI_VERSION` 16).
...
Sizes: **S** ≈ days, **M** ≈ two weeks, **L** ≈ a month or more, including tests, docs page and CLI wiring. Sizes assume the shared infrastructure in §1.18 exists; the first format that needs a component pays for it.
...
Python meshio covers none of §1.1–§1.15; these are greenfield. Cross-validation partners are named in each block.

### 1.1 FEBio `.feb` input and `.xplt` results (read / write `.feb`; read `.xplt`) — **M**
...
### 1.2 Elmer mesh directory (read / write) — **S–M**
...
### 1.3 MSC Patran neutral file `.pat` / `.out` (read / write) — **S–M**
...
### 1.4 Femap neutral file `.neu` (read; mesh write) — **M**
...
- **Out of scope.** The binary `.modfem` database (§1.17).
...
### 1.5 MFEM `.mesh` and `.gf` (read / write) — **M**
...
### 1.7 Z88 / Z88Aurora (read / write mesh; read results) — **S**
...
### 1.8 Abaqus `.fil` results file (read) — **M**
...
### 1.9 Radioss / OpenRadioss Starter `_0000.rad` (read) — **S–M**
...
### 1.10 MSC Marc input `.dat` (read) and `.t19` (read, later) — **M**
...
- **`.t19`.** Post codes and record layout are described in Volume C; a native reader is feasible and is the only licence-free route to Marc results. The binary `.t16` stays on the PyPost route (§1.17).
...
### 1.11 ANSYS `.cdb` archive (read / write) — **S–M**; `.rst` / `.rth` results (read) — **L**
...
### 1.12 LS-DYNA `d3plot` family (read) — **L**
...
### 1.14 Tecplot binary `.plt` (read) — **M**
...
- **Out of scope.** `.szplt` — TecIO only (§1.17).
...
### 1.15 MSC Nastran HDF5, remaining half: coordinate systems and multi-row tables — **S**
...
### 1.16 DOLFINx ADIOS2 `.bp` (read; optional ADIOS2 build) — **M**
...
### 1.17 Vendor-runtime routes (exporter scripts and optional plugins, no native parser) — **M** in total
...
- **Optional-plugin mechanism and a `contrib/` script convention** — §1.16, §1.17.

### 1.19 Suggested order

1. **Next FEM wave:** §1.2 Elmer → §1.1 FEBio → §1.11 ANSYS `.cdb` (which needs the tokenizer's format-line half, §1.18).
2. **Legacy and research reach:** §1.3 Patran → §1.4 Femap → §1.8 Abaqus `.fil` → §1.5 MFEM → §1.6 libMesh → §1.7 Z88 → §1.9 Radioss → §1.10 Marc.
3. **Heavy binaries, on demand:** §1.12 d3plot (pull forward if crash-dataset users appear) → §1.11 `.rst` → §1.13 OP2 → §1.14 `.plt`; §1.15 (Nastran HDF5 coordinate systems and per-ply tables) as soon as a `CP != 0` or composite `.h5` is in hand.
4. **Routes:** §1.17 scripts as users ask; §1.16 when a FEniCSx user asks.
...
- Marc element numbers other than 7, and their node orderings, against Volume B (§1.10).
- Every link marked †.
...
1. **Format reach ([§1](#_1-format-reach))** — the next FEM wave: Elmer, FEBio, then ANSYS `.cdb`.
