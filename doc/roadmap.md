...
Status at time of writing: **v10.3.0** — 42 formats, twenty-five mesh operations + five data operations, six language surfaces (Python / C / Fortran / Julia / R / WASM), two viewers plus a browser dataset manager, an MCP server, a settings-driven pipeline engine, a dataset-manifest layer with a PhysicsNeMo adapter, and a versioned ABI (`MESHIOPLUSPLUS_ABI_VERSION` 7).
...
- **Polyhedral coarsening (agglomeration)** — `decimate` still raises by name on a polyhedron, pointing at `convert_cells(simplexify)`; the fixed-template QEM edge collapse it is built on has no analogue for merging arbitrary polyhedral cells. Closing this means polyhedral agglomeration — a genuinely different algorithm (greedy seed-and-grow over the shared-face dual, `CellMapKind::Global` region carrying), not another template table. The *refinement* half of this gap closed in v10.3.0 (see below) — only coarsening remains. **L**
- **Green-element undo** — the standard rule is to restore a transitional cell to its parent and re-split from scratch before a new refinement pass; this implementation refines the transitional children directly, so repeated selective passes over the same region degrade element quality without bound. The persistent parent/child hierarchy that rule needs shipped in v10.1.0 (`refine(..., record_hierarchy=True)` → `refine:cell_id`/`refine:parent_id`, plus `refine:level`); implementing the undo itself — inverting `refine_templates.cpp`'s subdivision tables against a sibling run to recover the parent's connectivity, since `refine:entity` records *an* entity rather than a *generation* and cannot distinguish it alone — remains open. **M**
...
**Closed in v10.3.0**: the polyhedral-*refinement* half of the bullet above — `subdivide` (`operations/subdivide.hpp`, [`doc/subdivide.md`](doc/subdivide.md)) splits every eligible 3D cell (tabulated or an existing polyhedron block, handled uniformly through `detail::cell_rings`/`orient_rings` — no per-type template table at all) into one polyhedral child per face, connected to a new interior point. Automatically conforming (a shared face is never touched, so there is no closure/hanging-node bookkeeping to do), and shipped across every binding surface. Polyhedral *coarsening* (agglomeration) is unaffected and stays open, above.
