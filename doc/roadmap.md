...
Status at time of writing: **v15.5.0** — 53 core formats plus four Python-only physics-ML ones, thirty-nine mesh operations + six data operations, six language surfaces (Python / C / Fortran / Julia / R / WASM), two viewers plus a browser dataset manager, a Blender add-on, a ParaView plugin, an MCP server, a settings-driven pipeline engine, a dataset-manifest layer with a PhysicsNeMo adapter, and a versioned ABI (`MESHIOPLUSPLUS_ABI_VERSION` 15).
...
Sizes: **S** ≈ days, **M** ≈ two weeks, **L** ≈ a month or more, including tests, docs page and CLI wiring. Sizes assume the shared infrastructure in §1.22 exists; the first format that needs a component pays for it.
...
Python meshio covers none of §1.1–§1.19 except OptiStruct `.fem` as plain Nastran bulk; these are greenfield. Cross-validation partners are named in each block.

### 1.1 I-DEAS Universal File `.unv` / `.uff` (read / write) — **S–M**
...
### 1.2 MSC Nastran HDF5 results `.h5` (read) — **S–M**
...
- **Out of scope.** OP2/OP4/punch (see §1.18); non-MSC HDF5 dialects until a file is in hand.
...
### 1.3 Code_Aster `.mail` mesh (read / write) — **S**
...
### 1.4 Altair OptiStruct `.fem` (read; Nastran reader extension) — **S**
...
### 1.5 COMSOL `.mphtxt` / `.mphbin` mesh (read / write) — **M**
...
### 1.6 FEBio `.feb` input and `.xplt` results (read / write `.feb`; read `.xplt`) — **M**
...
### 1.7 Elmer mesh directory (read / write) — **S–M**
...
### 1.8 MSC Patran neutral file `.pat` / `.out` (read / write) — **S–M**
...
### 1.9 Femap neutral file `.neu` (read; mesh write) — **M**
...
- **Out of scope.** The binary `.modfem` database (§1.21).
...
### 1.10 MFEM `.mesh` and `.gf` (read / write) — **M**
...
### 1.12 Z88 / Z88Aurora (read / write mesh; read results) — **S**
...
### 1.13 Abaqus `.fil` results file (read) — **M**
...
### 1.14 Radioss / OpenRadioss Starter `_0000.rad` (read) — **S–M**
...
### 1.15 MSC Marc input `.dat` (read) and `.t19` (read, later) — **M**
...
- **`.t19`.** Post codes and record layout are described in Volume C; a native reader is feasible and is the only licence-free route to Marc results. The binary `.t16` stays on the PyPost route (§1.21).
...
### 1.16 ANSYS `.cdb` archive (read / write) — **S–M**; `.rst` / `.rth` results (read) — **L**
...
### 1.17 LS-DYNA `d3plot` family (read) — **L**
...
### 1.18 Nastran OP2 (read) — **M–L**

Fortran-record binary of named data blocks (`GEOM1–4`, `EPT`, `MPT`, `OUGV1`, `OES1X`, `OSTR1`, `OEF1X`, `OQG1`, `OGPFB1`, `ONRGY1`…), MSC and NX/Simcenter dialects, 32/64-bit. Only after §1.2 shows demand beyond HDF5.
...
### 1.19 Tecplot binary `.plt` (read) — **M**
...
- **Out of scope.** `.szplt` — TecIO only (§1.21).
...
### 1.20 DOLFINx ADIOS2 `.bp` (read; optional ADIOS2 build) — **M**
...
### 1.21 Vendor-runtime routes (exporter scripts and optional plugins, no native parser) — **M** in total
...
- **Abaqus `.odb`.** No public specification; readable only through the ODB API on a licensed install, version-locked (`abaqus upgrade -odb`). Ship an `abaqus python` exporter script (instances → regions, steps/frames → sequence, field outputs by position) and document it; an optional C++ plugin against the ODB API comes only if a user asks. Where no licence is available, §1.13 (`.fil`) is the fallback. — [ODB2VTK (reference exporter)](https://github.com/Arris-Composites/ODB2VTK) · [abqpy](https://github.com/haiiliin/abqpy) †
- **MSC Marc `.t16`.** Read through PyPost (`py_post`), shipped with Marc/Mentat. Ship an exporter script; §1.15 covers `.t19` natively.
- **Femap `.modfem`.** COM API on Windows only. The route is "export a neutral file" → §1.9.
- **Tecplot `.szplt`.** Undocumented; TecIO only. Optional link against a user-installed TecIO, or "re-save as `.plt`" → §1.19. — [TecIO](https://tecplot.com/products/tecio-library/) †
- **ANSYS results beyond §1.16.** Document the PyDPF export route.
- **Done when.** Each route has a docs page, a script under `contrib/`, and one manual test recorded with the vendor version used.

---

### 1.22 Shared infrastructure worth building once
...
- **HDF5 schema helper, remaining half: typed compound-table reads** for Nastran `.h5` (§1.2). The rest of it — fixed-length string attributes, integer-array attributes, hyperslab row reads, chunked appendable datasets, creation-ordered groups and soft links — shipped in `detail/hdf5_util.hpp` with VTKHDF.
- **Directory-as-format support** in the path and CLI layer — Elmer, OpenFOAM, ADIOS2 `.bp`.
- **Optional-plugin mechanism and a `contrib/` script convention** — §1.20, §1.21.

### 1.23 Suggested order

1. **First FEM wave (build tokenizer + permutation registry):** §1.1 UNV → §1.3 `.mail` → §1.4 OptiStruct.
2. **HDF5 results:** §1.2 Nastran `.h5`.
3. **Second FEM wave:** §1.5 COMSOL → §1.7 Elmer → §1.6 FEBio → §1.16 ANSYS `.cdb`.
4. **Legacy and research reach:** §1.8 Patran → §1.9 Femap → §1.13 Abaqus `.fil` → §1.10 MFEM → §1.11 libMesh → §1.12 Z88 → §1.14 Radioss → §1.15 Marc.
5. **Heavy binaries, on demand:** §1.17 d3plot (pull forward if crash-dataset users appear) → §1.16 `.rst` → §1.18 OP2 → §1.19 `.plt`.
6. **Routes:** §1.21 scripts as users ask; §1.20 when a FEniCSx user asks.
...
- UNV quadratic node ordering against Salome, Code_Aster and pyuff samples (§1.1).
- Marc element numbers other than 7, and their node orderings, against Volume B (§1.15).
- Every link marked †.
...
1. **Format reach ([§1](#_1-format-reach))** — the first FEM wave (`.unv`, `.mail`, OptiStruct).
