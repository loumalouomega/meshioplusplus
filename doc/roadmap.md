...
Status at time of writing: **v10.12.0** — 42 formats, thirty-two mesh operations + five data operations, six language surfaces (Python / C / Fortran / Julia / R / WASM), two viewers plus a browser dataset manager, an MCP server, a settings-driven pipeline engine, a dataset-manifest layer with a PhysicsNeMo adapter, and a versioned ABI (`MESHIOPLUSPLUS_ABI_VERSION` 9).
...
Only the volumetric bullet remains:
...
*Recommended posture: a separate project in its own right, with no shared code to reuse from the surface work above beyond the pattern (clustering + dual construction). No other bullet in this section is open.*
...
1. **Remeshing (§1) — shipped in full except the volumetric counterpart**, removed from this document down to that one bullet: v10.10.0 (`remesh`, a clean-room isotropic clustering engine derived from the MIT `pyvista/pyacvd` plus an original `metric="quadric"` synthesis over meshio++'s own pre-existing Garland-Heckbert quadric machinery, across every binding surface in one release); v10.11.0 (curvature gradation via a new osculating-paraboloid vertex-curvature estimator, plus boundary pinning/dual-edge insertion and separately-reported non-manifold-vertex detection, both extending `remesh` and its full binding surface); v10.12.0 (`metric="anisotropic"`, packing a curvature tensor into the existing quadric accumulator, closing the section's L estimate faster than expected once gradation had already supplied the curvature machinery it needed — `MESHIOPLUSPLUS_ABI_VERSION` 8→9). The volumetric CVD/ODT counterpart (**XL**, a separate project) remains open in §1. See `doc/remesh.md`.
