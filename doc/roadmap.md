...
Status at time of writing: **v10.19.0** — 43 formats, thirty-four mesh operations + five data operations, six language surfaces (Python / C / Fortran / Julia / R / WASM), two viewers plus a browser dataset manager, an MCP server, a settings-driven pipeline engine, a dataset-manifest layer with a PhysicsNeMo adapter, and a versioned ABI (`MESHIOPLUSPLUS_ABI_VERSION` 11).
...
## 1. GiD postprocess format: the remaining refinements

**Shipped.** The GiD postprocess format is now **read and written in all three flavours** — v10.18.0 delivered the write path (vendored gidpost 2.14, ascii/binary/hdf5, extension dispatch, every registry-driven surface), and v10.19.0 the reader. The reader is meshio++'s own code against the on-disk grammar, since gidpost's public API has zero read functions; its dependencies are therefore **per flavour** (ascii needs nothing, binary needs zlib, hdf5 needs HDF5), which makes `gid` readable in strictly more build configurations than it is writable. It handles the two real-world variants meshio++'s own writer cannot produce — a node table repeated in every `MESH` block, and element ids restarting per block — and honours `ReadOptions::mTimeStep` for multi-step results files. The same v10.19.0 release also confirmed `hexahedron20`'s mid-edge node order against a documentary conflict (CIMNE's own GiD 6-era figure disagreed with Kratos's production writer; Kratos's order is the one GiD actually expects), added `hexahedron27`/`wedge15`/`pyramid13` by deriving their orderings directly from Kratos's own geometry classes (self-inverse permutations for the first two, identity for the third — the one type with no Kratos-GiD precedent at all, since Kratos never writes pyramids to GiD), added all nine `GiD_ResultType`s, including `Matrix` and the three `Complex*` types, via a `field_data` declaration, added **arbitrary Gauss points per element** (flat `(ncells, G*k)` storage plus a declaration, with `Given` natural coordinates for the counts GiD cannot place itself; `G == 1` declares nothing, so the default path is byte-identical), **`ResultGroup` reading** (ascii) and the **`ascii_zipped` write mode** — closing what were previously this section's largest open items, one of which this document had listed as a permanent boundary rather than a gap, and a second of which rested on a false premise (see the `Group` bullet). See `doc/formats/gid.md`.

**What remains** is a set of narrow refinements, none blocking anything:

- **`Group` / `OnGroup`** — skipped on read, never written. *This was previously listed as "Regions ↔ GiD mesh groups", which was a category error*: CIMNE's manual describes `Group` as wrapping the `MESH` blocks belonging to **one time step**, so GiD can swap meshes as the step changes — it exists for re-meshing and adaptive analyses, not as a named entity set. The postprocess format has **no node/element set concept at all**; the optional material column, already round-tripping as `gmsh:physical`, is its only grouping mechanism, so there is no region mapping to build. Its real analogue is the transient axis, which makes it a companion to multi-step writing below rather than an item of its own. **S–M**, and only meaningful alongside that.
- **Multi-step writing** — the reader selects a step from a multi-step file, but the writer still emits exactly one step per call. A multi-step writer is a stateful object rather than a `(path, mesh)` function (the `XdmfTimeSeriesWriter` precedent). **M**
- **`gid` in the release CLI binaries** — the statically-linked release CLI and the Windows wheels build `-DMESHIOPLUSPLUS_WITH_ZLIB=OFF`, so they can now *read* the ascii flavour but still cannot write any. Flipping that default needs its own cross-platform static-link validation. **S**

*Recommended posture: nothing here blocks anything else, and the format is fully usable in both directions today — every remaining item is an ordinary follow-up to pick up when someone wants it, none needing an external dependency this project doesn't already have.*
...
3. **GiD postprocess — shipped read/write**: v10.18.0 (vendoring, build integration, all three write flavours, extension dispatch, every registry-driven surface) and v10.19.0 (the hand-rolled reader, all three flavours, plus two upstream gidpost HDF5 bug fixes its own test suite surfaced; the `hexahedron20` node-order conflict against CIMNE's own documentation resolved in Kratos's favour; `hexahedron27`/`wedge15`/`pyramid13` added by deriving their orderings from Kratos's own geometry classes, closing the section's last unverified-ordering item; all nine `GiD_ResultType`s, including `Matrix`/`Complex*`, via a `field_data` declaration; arbitrary Gauss points per element, which this document had previously listed as a permanent data-model boundary; `ResultGroup` reading; and the `ascii_zipped` write mode). §1 is now a short list of narrow refinements rather than a missing capability.
