...
Status at time of writing: **v10.22.0** — 43 formats, thirty-four mesh operations + five data operations, six language surfaces (Python / C / Fortran / Julia / R / WASM), two viewers plus a browser dataset manager, a Blender add-on, an MCP server, a settings-driven pipeline engine, a dataset-manifest layer with a PhysicsNeMo adapter, and a versioned ABI (`MESHIOPLUSPLUS_ABI_VERSION` 11).
...
- **Dataset health summaries, server side** — the browser producer shipped in v10.22.0 (split balance, fields missing across entries, NaN/Inf and inverted/degenerate-cell counts from the WASM `attachQuality`, cached per entry); what remains is a server-side producer of the *same* `ManifestHealth` summary (`compute_quality` + `data_info` over a manifest, one mesh alive) for manifests too large to stage in a browser — the companion process below is its natural home, and the card renderer already takes either producer. **S**
...
*The multi-dataset overview, the per-dataset drill-down, manifest diffing and the browser-side health summaries shipped in v10.22.0 and have been removed from this section: `src/viewer/src/dataset/{overview,health,diff,thumbs,hash}.ts` over the existing `dataset.html`, with no C++/WASM/binding change. See [`doc/dashboard.md`](dashboard.md).*
...
10. **Dataset dashboard (§1)** — the non-training pieces shipped in v10.22.0 (multi-dataset overview, drill-down, browser-side health summaries, diffing); the server-side health producer and training launch/monitoring wait on the companion-process design pass before implementation.
