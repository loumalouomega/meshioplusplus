...
Status at time of writing: **v10.23.0** — 43 formats, thirty-four mesh operations + five data operations, six language surfaces (Python / C / Fortran / Julia / R / WASM), two viewers plus a browser dataset manager, a Blender add-on, an MCP server, a settings-driven pipeline engine, a dataset-manifest layer with a PhysicsNeMo adapter, and a versioned ABI (`MESHIOPLUSPLUS_ABI_VERSION` 11).
...
- **Launch PhysicsNeMo training from the dashboard** — a "train" action against a manifest/split that kicks off training (generalizing the `example/physicsnemo/` recipe). This is the one item here that crosses an architectural line the rest of the viewer never has: everything else runs client-side in WASM with no server, and real GPU training cannot. **The companion process it needs shipped in v10.23.0** — `meshioplusplus-mcp --http`, a token-protected JSON API over the tool registry plus MCP over HTTP, with the page's connect panel and hash-bound cards ([`doc/dashboard.md`](dashboard.md#the-companion-process)); what remains is the in-package trainer (`python -m meshioplusplus.physicsnemo.train --spec`), a job manager exposed as registry tools (`train_start`/`train_status`/`train_stop`/`train_log`/…, so MCP agents get them too) and the launch form. **M**
...
*The multi-dataset overview, the per-dataset drill-down, manifest diffing and the browser-side health summaries shipped in v10.22.0, and the server-side health producer with the companion process in v10.23.0 (`meshioplusplus-mcp --http`, `dataset_find`/`dataset_health`); all have been removed from this section, with no C++/WASM/binding change. See [`doc/dashboard.md`](dashboard.md).*
...
10. **Dataset dashboard (§1)** — the non-training pieces shipped in v10.22.0 (multi-dataset overview, drill-down, browser-side health summaries, diffing) and the companion process with its server-side health producer in v10.23.0; training launch/monitoring (the trainer, the job manager as registry tools, the launch form) is next, on top of that process.
