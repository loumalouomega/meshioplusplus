...
**Shipped.** The GiD postprocess format is now **read and written in all three flavours** — v10.18.0 delivered the write path (vendored gidpost 2.14, ascii/binary/hdf5, extension dispatch, every registry-driven surface), and v10.19.0 the reader. The reader is meshio++'s own code against the on-disk grammar, since gidpost's public API has zero read functions; its dependencies are therefore **per flavour** (ascii needs nothing, binary needs zlib, hdf5 needs HDF5), which makes `gid` readable in strictly more build configurations than it is writable. It handles the two real-world variants meshio++'s own writer cannot produce — a node table repeated in every `MESH` block, and element ids restarting per block — and honours `ReadOptions::mTimeStep` for multi-step results files. The same v10.19.0 release also confirmed `hexahedron20`'s mid-edge node order against a documentary conflict (CIMNE's own GiD 6-era figure disagreed with Kratos's production writer; Kratos's order is the one GiD actually expects) and added all nine `GiD_ResultType`s, including `Matrix` and the three `Complex*` types, via a `field_data` declaration — closing what was previously this section's largest open item. See `doc/formats/gid.md`.

**What remains** is a set of narrow refinements, none blocking anything:

- **Unverified node orderings** — `hexahedron27`, `wedge15` and `pyramid13` are refused **by name in both directions**. Their GiD orderings were never independently verified (the shipped types were cross-checked against Kratos's own production GiD writer), and guessing a permutation in one direction we decline to guess in the other would be worse than refusing. Needs GiD itself, or its postprocess-data-files specification, to settle. **S**
- **Gauss points with more than one point per element** — read as a warning-and-drop, because meshio++'s `cell_data` is `(n,)`/`(n,k)`, never per-node-within-cell. This is the same structural limit MED's ELNO/ELGA documents: **a boundary, not a defect to fix**, unless the data model itself grows a per-node-within-cell concept.
- **Regions ↔ GiD mesh groups** — `Group`/`End Group` is skipped on read and never written. Mapping it to named regions is the natural next step in both directions; it interacts with results in ways not yet verified. **S–M**
...
- **`ResultGroup`** — several results packed into one wide `Values` row. Refused by name on read (unpacking needs the per-member `ResultDescription` dimensions); never written. **S**
- **`GiD_PostAsciiZipped` as a write mode** — read support came free (the reader sniffs gzip magic and inflates, since the flavour is the same ASCII text), but `GidMode` exposes no write counterpart. **S**
...
*Recommended posture: nothing here blocks anything else, and the format is fully usable in both directions today. The node orderings are the only item with an external dependency (access to GiD or its specification); the rest are ordinary follow-ups to pick up when someone wants them.*
...
3. **GiD postprocess — shipped read/write**: v10.18.0 (vendoring, build integration, all three write flavours, extension dispatch, every registry-driven surface) and v10.19.0 (the hand-rolled reader, all three flavours, plus two upstream gidpost HDF5 bug fixes its own test suite surfaced; the `hexahedron20` node-order conflict against CIMNE's own documentation resolved in Kratos's favour; and all nine `GiD_ResultType`s, including `Matrix`/`Complex*`, via a `field_data` declaration). §1 is now a short list of narrow refinements rather than a missing capability.
