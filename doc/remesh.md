...
Both metrics maximize cluster compactness — the acceptance test is always a "badness to minimize" derived from the same `-|sgamma|^2/srho` form — but `metric="quadric"` **adds** a Garland-Heckbert quadric flatness term on top, and changes the representative point:
