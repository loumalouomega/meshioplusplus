...
- **Every mesh file enters PhysicsNeMo through PyVista/VTK today** (the `VTKReader` hard-requires pyvista and reads `.stl`/`.vtp`/`.vtu`/`.vtk` only). That is the slot this adapter fills: meshio++'s 81 readers, regions, and preprocessing operations, ending in the tensors the datapipes expect.
