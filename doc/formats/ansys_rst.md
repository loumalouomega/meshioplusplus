...
| energies (`ENG`), miscellaneous items (`EMS`, `EMN`), gradients and fluxes (`EGR`, `EFX`), nonlinear data (`ENL`), element temperatures (`EPT`), contact data (`ECT`), state variables (`ESV`) | cell data under the record's name, one row per element holding the record as written, NaN-padded to the longest (since v16.12.0) |
...
- Line and point elements (beams, links, masses) carry no element nodal results in meshio++: their records hold section forces and stresses that are not tensors at nodes. Their raw records (below) are read.
- **Raw records** (since v16.12.0). `ENG`, `EMS`, `EMN`, `EGR`, `EFX`, `ENL`, `EPT`, `ECT` and `ESV` are read as MAPDL writes them: one row per element, `(cells, n)` per block with `n` the longest record of any element, NaN beyond an element's own record and where MAPDL wrote its undefined value. Their items depend on the element type (the element's "Output Data" table in the MAPDL element reference; `ENG` is the volume and the strain, plastic, creep, kinetic and thermal energies for most elements). They are not rotated, averaged or expanded, and a negative pointer (an unwritten record of zeros) reads as zeros.

`arrays=[...]` narrows what is read by name (`S`, `S@top`, `EPEL`, `ENF`, `ENG`, `RF` ...), and `points_only` skips all results.
...
A cyclic-symmetry model holds one sector (the base sector, element numbers up to `csEls`) and, before release 18.2, a duplicate sector. `ansys_rst` reads the file as stored, with a warning. `ansys_rst_cyclic` reads a **static** cyclic analysis as the full rotor: the base sector's cells and points are repeated round the cyclic axis once per sector (global Z, or the Z axis of the local coordinate system the model names), and every vector and tensor result is rotated with its sector; `cell_data["ansys:sector"]` numbers the copies and `field_data["ansys:sectors"]` counts them. Coincident nodes on the sector boundaries are not merged (`meshioplusplus clean` merges them). Regions hold their members in every sector.

A **modal** cyclic analysis (since v16.12.0) is expanded mode by mode, as MAPDL's `/CYCEXPAND` and pymapdl-reader's `CyclicResult` do. Each set carries a harmonic index `h` (the nodal diameter; `field_data["ansys:harmonic_index"]`) and holds one half of a travelling mode, `x`. Its other half, `x'`, is the set's **mode pair** (the neighbouring set of the same frequency, which MAPDL writes with the index `-h` since release 18.2; earlier releases repeat `h`, and the second set of the pair is read as `-h`). Without a pair, `x'` is the **duplicate sector**'s solution, its nodes past `csNds` paired with the base sector's in number order; with neither, it is zero (a standing wave). Sector `i` of the full rotor is then `scale * (x cos(h θᵢ) - x' sin(h θᵢ))`, rotated with the sector, with `θᵢ = 2πi/N` and `scale = 1/√N` for `h = 0` or `N/2`, else `1/√(N/2)`. Every nodal, reaction and element nodal array is combined this way; raw element records are copied from the base sector unchanged. Other analysis types (transient, harmonic ...) are refused.
...
- The other element records (`ELF`, `ECD`, `EHC`, `ESF`, `EDI`, `ETB`, `EXY`, `EBA`, `MNL`).
- The full rotor of an analysis other than a static or modal one.
