...
- **`abaqus_fil`** (`.fil`; read, time-capable, also recognised by content): the Abaqus results file, binary in either byte order (new `detail/fortran_records.hpp`) or ASCII.
  ...
  - Nodal records become point data and element records cell data shaped by their output location (integration points `(cells, points, components)`, centroid, element nodes, nodal average as point data); shell section points above 1 get `@sp<k>`; a solid's six-component tensors are reordered to `xx yy zz xy yz zx`.
