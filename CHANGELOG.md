...
- Gauss-point results with more than one point per element are **dropped with a warning** — meshio++'s `cell_data` is `(n,)`/`(n,k)`, never per-node-within-cell, the same structural limit MED's ELNO/ELGA documents. Tensor result types are read as one k-component array with the declared type dropped rather than reinterpreted; note the resulting round-trip asymmetry, since the writer splits `k ∉ {1,2,3}` into k scalars and the reader does not re-join them. `hexahedron27`/`wedge15`/`pyramid13` are refused by name on read as well as write, their orderings still being unverified.
...
- **Recorded an unresolved conflict over `hexahedron20`'s mid-edge node order** (`doc/formats/gid.md`). CIMNE's own GiD 6-era figure numbers them bottom-ring/verticals/top-ring — exactly Kratos's *internal* order, the one Kratos permutes away from — which taken at face value says meshio++'s identity mapping is wrong. meshio++ continues to follow Kratos's production writer, since CIMNE's current grammar dropped those figures and specifies no mid-edge order at all, and since Kratos's permutation is labelled a "workaround", i.e. added against observed behaviour. Settling it needs an external oracle (a `hexahedron20` file written by GiD itself) that was not available. Relatedly, the `GidOrdering` test suite's comment was corrected: it pins that the writer applies *no permutation*, and cannot pin that meshio++'s order is GiD's, since its expected positions come from meshio++'s own edge table.
