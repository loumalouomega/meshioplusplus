...
- **Both engines write identical bytes** (the LZF-compressed files included) and read identically, pinned against each other on every fixture and mode. Python: `test_pcd.py` (127 with the CLI verbs), `test_xyz.py`; C++: `test_point_cloud_formats.cpp` (25, including a hand-encoded LZF stream, independent of this library's own encoder), and `pcd` and `xyz` join the provenance-tag parity table.
...
- **Not verified in this change:** WASM (no Emscripten; `tests/wasm/smoke.mjs` extended but not run), R and Julia (no toolchain), PCL itself reading a file this library wrote (PCL is not installed; the fixtures are PCL-written files read by this library, not the reverse), and the C++ sources under the KRATOS mesh backend (compiled under MESHIO and NATIVE). The C++ tests were linked against the built core objects rather than through a fresh `build/configure.sh --tests` tree.
