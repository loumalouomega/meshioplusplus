...
- **Headline counts corrected everywhere they appear**: the format count (43/46/50 → 84 total, 79 in the native core, 81 readable/72 writable in Python, 76/68 in WASM) and the operation count (34+5 → 39 mesh + 6 data operations), in `README.md`, `doc/index.md`, `doc/architecture.md`, `doc/.vitepress/config.mts`, the WASM package description, and the generated architecture/mesh-backends/data-locations diagrams (`doc/diagrams/figures/`, regenerated). `neuroglancer` is documented as a fifth Python-only format alongside `pmsh`/`zarr`/`cae`/`usd`. The MCP tool count is corrected to 87 (was 84/57).
- **Roadmap status line** updated to the current release, format count and ABI version (18, was 17); §0 ("update the doc") is closed and removed.
- **`find_package` version pins and ABI examples** updated from stale `9`/`10`-series numbers to the current major and release (`README.md`, `doc/cpp_api.md`, `doc/c_api.md`, `doc/abi.md`, the ABI-tiers diagram); the C++ `SOVERSION` claim in `doc/cpp_api.md` corrected to track `MESHIOPLUSPLUS_ABI_VERSION` rather than a flat `0` (only the C library keeps `SOVERSION 0`).
- **Text describing since-parallelised or since-changed behaviour** brought current: `doc/mmap.md`'s `FileSource` coverage list (34 readers, up from 6), `doc/proximity_graphs.md`'s "pure Python" claim, the "serial" cut/gather/dedup claims in `doc/isosurface.md`, `doc/slice.md`, `doc/data_average.md`, `doc/remesh_volume.md`, `doc/extract_surface.md` and the `data_locations` diagram (all parallelised since v16.16.0–v16.19.0), the MCP `sequence` tool's "XDMF only" fan-in claim (now also `gid`/`usd`/`vtkhdf`/`pvd`/`femap`), the `.msh`/`.dat`/`.mesh` content-aware read-order notes in `doc/formats.md` and `doc/formats/{mfem,openfoam}.md`, the VTU base64/appended-data claims in `doc/selective_read.md`, `doc/formats/elmer.md` and `doc/formats/vti.md`, and Femap's "results are not written" line in `doc/formats.md`.
- **Fixed a factual error**: `doc/formats/mdpa.md`'s Kratos type table had `Element3D13N` as `wedge15`; it is `pyramid13` (`Element3D15N`/`Prism3D15` is `wedge15`). Added the missing `Pyramid3D5`/`Pyramid3D13` rows.
- **Documented previously-undocumented surface**: `voxelize`, `distance_to_surface`, `sample_distance` and `surface_watertight_check` MCP tools; the `curvature` and `tessellate` CLI verbs (and which CLI verbs are Python-only, and the native `surface` alias for `extract-surface`); VTU's raw-appended write option and structure (`doc/formats.md`, `doc/formats/vtu.md`, `README.md`, `doc/cli.md`); `resolve_write_format` and `_core.registry_formats()`; Radioss/Marc/EnSight write options; the CSR polyhedron-ingestion overloads and MESHIO's CSR storage (`doc/cpp_backends.md`); the v16.18–v16.21 tokenizer, parallel-writer and appended-VTU performance gains (`doc/benchmarks.md`); seven previously-undocumented public Python names (`extension_to_filetypes`, `has_viewer`, `FeatureMatrix`, `has_zarr`, `PowerSpectrum`, `BistrideHierarchy`, `topological_dimension`); the PVD/PVTP/PVTU/VTM/VTR/VTS format-list entries and the curvature/normals/repair/shrinkwrap/Sobolev-deformation/grid-and-SDF operations in `README.md`.
- **New page**: `doc/notebooks.md`, an index of the 24 Python example notebooks (linked from the sidebar) plus pointers to the C++/Julia/R examples.
- Refreshed `BASELINE_HASHES` in `tests/python/test_io_baseline.py` for the version-string change (verified against the old hashes with the old version substituted back in — no unrelated output change). Regenerated the single header (version macros only).
...
**Closes roadmap §1.1 (shared infrastructure) and §1.2 (reader and writer parity).** The Radioss and Marc decks are written, Femap writes a series, every format read but not written says why, and the shared tokenizer, node-order registry, Fortran record reader and directory sniffing are used by the formats that kept their own. Moving the node orders into the registry found two formats writing quadratic hexahedra in the wrong order, fixed below.

- **Radioss starter-deck writer**, in both engines, byte for byte the same: `write_radioss` / `meshioplusplus.radioss.write` / `write("run_0000.rad", mesh)` give `/BEGIN`, `/PART` and `/SUBSET`, `/NODE`, `/BRICK` (pyramids and wedges as degenerate bricks), `/TETRA4`, `/TETRA10`, `/BRIC20`, `/SHELL`, `/SH3N`, `/TRUSS`, `/GRNOD` and `/GR<family>` groups and `/SURF/SEG` surfaces, reading back to the same mesh, parts and regions. `stubs=True` adds placeholder `/MAT/LAW1` and `/PROP` cards so the OpenRadioss starter accepts the deck on its own. Checked with the OpenRadioss starter (build latest-20260728): of its 76 QA decks with a mesh, 75 are accepted as written (the other has a zero-length `/TRUSS`) and 70 read back identical ([Radioss](doc/formats/radioss.md#writing)).
- **Marc input-deck writer**, in both engines: `write_marc` / `meshioplusplus.marc.write` / `write(path, mesh, file_format="marc")` give an `EXTENDED` deck with `CONNECTIVITY`, `COORDINATES` and `DEFINE NODE`/`ELEMENT SET`s that reads back to the same mesh, element numbers, types and sets; `marc:type` is kept, and defaults cover solids, shells and plane strain. No Marc licence was at hand: the deck follows Volume C and Mentat's decks, and a run of Marc is listed under "Awaiting a licensed run" ([Marc](doc/formats/marc.md#writing)).
- **Femap series**: `write_sequence("run.neu", steps)`, `femap.SeriesWriter` and the native `FemapSeriesWriter` write the mesh once and one output set per step, which read back by `time_step`; the native pipeline and `sequence_to_timeseries` take `.neu` too. Multi-component arrays stay split into component vectors, recorded as the limit ([Femap](doc/formats/femap.md)).
- **EnSight Fortran binary** is read (geometry in both engines, variables in the core) through the shared Fortran record reader, and written with `write_ensight(path, mesh, binary, fortran)` / `ensight.write(..., fortran=True)`; VTK's EnSight reader reads what meshio++ writes ([EnSight](doc/formats/ensight.md)).
- **UNV** cuts records in their Fortran columns with the shared card tokenizer when a value fills its field and touches the next, where whitespace splitting read one number ([UNV](doc/formats/unv.md)).
- **Sequence globs keep Elmer mesh directories**, which have no suffix, in both engines ([sequences](doc/sequences.md)).
- **Read-only formats name their reason**: `conformance_spec.READ_ONLY` holds a reason for each (result files: the solver is the only producer; `vtx` and `szplt`: only ADIOS2 and TecIO read them), a test holds it against both registries and fails a new format that reads without writing and without a reason, and `tools/gen_conformance_table.py` now generates the Read and Write columns of the format table and a "Read-only formats" section of the [conformance page](doc/conformance.md#read-only-formats). `_core.registry_formats()` reports the native registry's lists.
- **Node-order registry**: the gmsh, CGNS, GiD, Kratos (`.mdpa`) and Exodus tables moved into `detail/node_order` and `_node_order.py`, and the self-test that could not fail is replaced by file-order reference elements taken from each format's own documentation ([node ordering](doc/node_ordering.md)).
- **Fixed**: `write("a.dat", mesh)` failed with "Unknown format 'marc'" since v16.8.0, Marc's read-only `.dat` registration standing in front of Tecplot's writer. A write now picks only among the extension's writers, `.dat` stays Tecplot's, and naming a read-only format says it "can be read but not written". Natively, every write resolved its format from an existing file's content: writing over a Marc deck or an MFEM mesh picked `marc` (no writer) or `mfem`. The new `resolve_write_format` goes by the name alone, in `registry_write_ex`, the C API, WASM, both CLIs, pipelines and sequences.
- **Fixed**: **Exodus `hexahedron20` and `hexahedron27` were written and read in VTK's node order**, not Exodus's (SEACAS lists the vertical mid-edges before the top ring, and the Hex27 body centre before the face centres), so other Exodus readers put nodes on the wrong edges; `wedge15` and `pyramid13` are now read and written too. Files from other Exodus writers now read correctly; ones earlier meshio++ versions wrote with these cells read back scrambled.
- **Fixed**: **Kratos `.mdpa` `hexahedron20` and `hexahedron27` used a table that swapped mid-edge slots** 9/11, 13/15 and 17/19 against Kratos's geometry classes, and `wedge15` (`Prism3D15`) had none; the Python reader now also reads `Prism3D15`, `Pyramid3D5` and `Pyramid3D13`, and `Element3D13N` is a `pyramid13`.
- **Fixed**: EnSight Fortran binary files were never recognised (the check looked for `Fortran Binary` at byte 0, where a record marker sits) and failed in the ASCII reader; binary variable files began with a `C Binary` record EnSight does not define, so VTK and ParaView read none of meshio++'s variables. They no longer have it, and files that do still read.
- **Fixed**: a pipeline fan-in to `.gid`, `.pvd` or `.usd` (Python `run_sequence_pipeline`, and the MCP `sequence` tool), or natively to a GiD `.post.msh`, wrote XDMF under that name; each now uses its format's series writer.
- ABI 18 unchanged: seven installed headers gain declarations only ([ABI reviews](doc/abi_reviews.md)).
- Docs: roadmap §1.1 and §1.2 removed, two Non-goals added ("Result-file writers", "Readers for the export-only surfaces"), and the map; the Radioss, Marc, Femap, Z88, EnSight, Exodus, MDPA, UNV and Elmer pages; formats, conformance, node ordering, regions, sequences, and the C, Fortran, Julia, R and WASM pages; notebook `24_format_parity.ipynb`.
...
- **Breaking (C++ API/ABI):** the `MESHIO` backend's `CellBlock` stores polygon and polyhedron blocks as CSR (`mFlat`, `mRowOffsets`, `mFaceOffsets`, as the `NATIVE` backend does) instead of `mPolygonRows`/`mPolyhedronRows`; `detail::SurfaceEdgeMap` is a key-sorted `std::vector` of (edge, record) pairs rather than an `unordered_map`; `detail::DistanceQuery`'s `mEdgeNormal` map is replaced by `mEdgeOfCorner` and `mEdgeNormals`; `detail::FaceLookup` and `detail::FacetIndex` hold sorted tables. Code that reads cells through the uniform API (`CellView::Row`/`Face`) is unaffected. `MESHIOPLUSPLUS_ABI_VERSION` is 18.
- **Parallel primitives** in `parallel.hpp`: `parallel_sort` (a comparator that is a total order gives one sequence on every backend: TBB's and the STL's parallel sorts, a chunk-sort-and-merge for OpenMP and Kokkos), `parallel_reduce` (fixed chunks combined in chunk order, so floating-point results do not depend on the thread count) and `parallel_exclusive_scan` (integers). The six hand-rolled chunked reductions (`compute_stats`, the point bounding box, the data-array statistics, `compute_quality`'s histogram, `diff`, `sobolev_deform`) use `parallel_reduce`.
- **The facet and edge table**: `extract_surface`, `extract_skin`, `smooth`'s boundary pass, `refine`, `convert_cells(elevate)`, `decimate`, the global face list (OpenFOAM, CGNS and Fluent writers, `agglomerate`, `decimate_volume`), `build_surface_edges` and the distance kernel's edge normals group their keys by a counting sort on a node-id bucket and small per-bucket sorts, and number them in first-seen order by flags and an exclusive scan -- the ids and counts the serial hash-map sweeps gave, now in parallel and linear. On the 160k-tet benchmark: `extract_surface`/`extract_skin` 2.3× faster on one thread and about 4× on eight; `refine` and `elevate` about 1.5× on eight; `optimize_volume` 1.5–1.7× on eight (its flip tables use `parallel_sort`). A golden-digest test (`test_facet_tables.cpp`) pins every one of these outputs to the hash-map implementation.
- **CSR ingestion**: every mesh backend takes ragged blocks whole through new `AddPolygonBlock(type, nodes, offsets)` and `AddPolyhedronBlock(type, nodes, face_offsets, cell_offsets)` overloads (`detail/ragged_csr.hpp`), with no allocation per cell or face; the nested-vector overloads remain. The VTK XML polyhedron reconstruction (no per-cell sort into a vector, no `std::map`), the OpenFOAM reader (no deep copy of every face), CGNS NGON/NFACE, MED polygons and polyhedra, the C API and the WASM binding use them, and verbatim ragged copies in six operations and writers go CSR to CSR. The MED and CGNS readers now check their polyhedron offsets and reject a file whose offsets run out of range or decrease, where they used to read past the arrays.
- **Fixed**: a writer with no place for provenance (MED, CGNS, MDPA, libMesh, UNV) left the notes it raised -- "a Side region was dropped", say -- for the next file written through a format writer called directly (`meshioplusplus.vtu.write`, or `write_vtu` in C++), which carried a note that was not about it; and a note containing `--` made a VTK XML file unparseable, since an XML comment may not hold `--`. Such writers now drop their notes on the way out (`detail::ProvenanceSlotlessWrite`, and a Python decorator for the pure-Python twins), and XML comments break every `--` apart.
- Docs: roadmap §4 narrowed and its map, [polyhedra](doc/polyhedra.md) (CSR everywhere), [C API](doc/c_api.md), [ABI](doc/abi.md).

## v16.15.0 (2026-09-25)

**Roadmap §4 (performance), first part:** the determinism check every other §4 item is judged by, the measured I/O regressions, and the small isolated wins. No output changes: every operation's result is byte-identical to v16.14.0's on SEQ, OpenMP and TBB at 1, 4 and 8 threads, which the new check proves.

- **Determinism check**: `meshioplusplus_bench_ops --hash` adds a digest of each row's result (points, connectivity, data, regions, index maps) to its CSV, and `tools/bench_ops.sh` fails when one row's digest differs between backends or thread counts, or, with `BASELINE=earlier.csv`, from an earlier sweep. A new `determinism` CI job runs it on every pull request, and the weekly `benchmark` workflow with `--hash`. Fourteen new rows (`extract_skin`, `smooth_volume`, `elevate`, `clean_weld`, `reorder_hilbert`, `optimize_volume`, `agglomerate`, `split`, `undo_green`, `hessian`, `compute_normals`, `compute_curvature`, `shrinkwrap`, `voxelize`) give the later §4 items their "before"; the benchmark cube's tets are now all positively oriented, so its surface is consistently wound and the `decimate` and `compute_sdf` rows do real work. Probing every row on today's code found no disagreement. The coverage CI leg installs Hypothesis, so `test_properties.py` runs somewhere.
- **HDF5 gzip in parallel** (XDMF, and any writer passing a gzip level): a gzip dataset over about 1 MiB is written in ~1 MiB whole-row chunks compressed in parallel and written in order, where it was one chunk compressed on one thread — which also lifts HDF5's 4 GiB limit on a chunk, so such a dataset could not be written at all. Reading inflates the chunks of any gzip dataset chunked in whole rows in parallel (ours, and typically h5py's), straight into the result. On a 1M-tet cube with the default level 4, an XDMF write went from about 115 ms to 27 ms and a read from 24 ms to 8 ms on 16 threads; SEQ builds are unchanged. Smaller datasets keep their single chunk and their bytes.
- **MED read** no longer zero-fills each dataset before HDF5 overwrites it, nor the transposed copy, and turns families into regions in one pass over the entities instead of one per family: the single-block read is now level with pure-Python meshio (it measured 0.81–0.93×).
- **XDMF mixed topologies**: packing on write and splitting on read copy rows in parallel with one dtype switch per block, into uninitialised buffers.
- **`optimize_volume`** keeps its relocation state across sweeps instead of building a `Mesh` and calling `smooth()` per sweep (the boundary pin mask is computed once — neither flip changes the boundary — and the node adjacency ODT never used is no longer built), and its 2-3 and 3-2 flips find candidates in sorted face and edge tables instead of three hash maps: 1.6× faster on SEQ and 2.1× at 8 threads on the 160k-tet benchmark row, byte-identical. `smooth(method="odt")` also skips that unused adjacency.
- **`reorder`** (Hilbert, Morton) and **`partition`** (space-filling curve) sort their curve keys with a stable LSD radix sort (`detail::sfc_stable_argsort`) instead of an indirect `std::stable_sort`, `reorder` sorts cells with a counting sort and computes its bounding box in parallel: `reorder_hilbert` is about 1.9× faster, with the same order.
- **Base64 decoding** (VTK XML binary) sizes its output once from a count of valid characters and decodes in parallel chunks, reproducing the serial decoder exactly (whitespace, padding and stray characters skipped).
- **MCP read cache**: the server keeps parsed inputs between tool calls, bounded by `MESHIOPLUSPLUS_MCP_CACHE_MB` (default 512, `0` disables), keyed on the file's identity (path, device, inode, size, modification and change times) plus the format and read options, so asking `stats`, `quality`, `data_info`, `regions` and `bandwidth` of one file parses it once, while a changed file is always re-read. Tools get a private copy; multi-file formats are never cached. The tools were described as stateless; they are now file-path based with this one piece of state ([MCP](doc/mcp.md#read-cache)).
- **`import meshioplusplus`** no longer imports `rich` (now loaded only when a message is printed), `xml.sax.saxutils` with `urllib.request`, `http.client` and `ssl` (one attribute-quoting function, now local), or `importlib.metadata` (the version is written into `meshioplusplus/_version.py` at build time) — about 50 ms. `test_import_footprint.py` keeps them out.
- **Benchmarks page**: the Gmsh binary write regression (0.68× in `benchmark/results.csv`) does not reproduce — it measures about 1.9× against meshio 5.3.5 — and the page says so; a "Determinism check" section documents `--hash`.
- **Fixed**: reading an ASCII VTK XML array parsed each integer with `strtoll`, which under ASan's `strict_string_checks` measures the rest of the text on every call, so the `cpp-sanitize` job timed out on a 100k-cell polyhedral file (on master too); integers now go through `std::from_chars`, with `strtoll`'s sign, saturation and wrap-around behaviour.
- **Fixed** (Python engines, which run where the core is built without HDF5, as on Windows): the HMF reader split raw cell data by the length of each block's type name instead of its cell count, so a multi-block mesh with cell data failed to read; the MED writer numbered a cell field's steps per (block, step) entry, so each block opened a timestep of its own. The conformance round trips no longer delete a memory-mapped `.pmsh` scratch file while it is still open, and the GiD declaration is skipped in a build without zlib.
- ABI 17 unchanged: `detail/space_filling.hpp` gains one inline function; the other installed-header changes are comments ([ABI reviews](doc/abi_reviews.md)).
- Docs: roadmap §4 narrowed to what remains, and its map; benchmarks; MCP; architecture.

## v16.14.0 (2026-09-25)

**Closes roadmap §3 (quality of implementation)** but for the OSS-Fuzz submission, fuzzing the library-backed readers and a benchmark trend store, which stay on it. No new format or operation: this release makes the existing ones safer to change, and fixes what that turned up.

- **Sanitizers**: `MESHIOPLUSPLUS_SANITIZE` (`build/configure.sh --sanitize address,undefined`) builds the core, the C API and the tests under ASan and UBSan; a new `cpp-sanitize` CI job runs the whole C++ suite that way with leak checking, and it is clean.
- **Fuzzing**: `MESHIOPLUSPLUS_BUILD_FUZZERS` (`--fuzzers`, Clang) builds a libFuzzer target over every native reader (`tests/fuzz/`), with seed corpora, dictionaries and a campaign runner (`tools/fuzz/`), a weekly `fuzz` workflow, and a replay driver that turns every committed finding under `tests/fuzz/regressions/` into a ctest. See the new `doc/fuzzing.md`.
- **One error for a malformed file**: every native reader entry point (the registry, so the C, Fortran, Julia, R and WASM surfaces and the native CLI, and the Python `_core.*_read` bindings) now rethrows a parser's `std::` exception as `ReadError` naming the format (`detail/read_guard.hpp`), where it used to surface as `ValueError`/`IndexError` from Python and as an unclassified error elsewhere.
- **Operation fallbacks**: the 53 broad `except Exception` handlers around the core in the operation shims now ask `core_op_declined`: a bad argument (`ValueError`, `TypeError`) propagates instead of running the Python twin, a missing core falls back silently, and the new C++ `Unsupported` (Python `NotImplementedError`) and a mesh the numpy conversion cannot carry (`ReadError`/`WriteError`) are recognised declines. `test_no_broad_core_swallow.py` now covers the operations too.
- **Reader hardening, from the fuzz campaign**: header counts are checked against the file before they size an allocation, tokens before they are indexed, and integers stored as reals before they are converted (`detail/parse_guard.hpp`), in the Abaqus (`.inp` and `.fil`), AVS-UCD, CalculiX `.frd`, DEX, DOLFIN, Fluent, Ansys `.rst` and `.cdb`, Femap, FLAC3D, FLUX, FreeFem, IP, Gmsh (2.2 and 4.1), libMesh, LS-DYNA keyword and `d3plot`, Marc, MDPA, Medit, MFEM, MFF, MFM, COMSOL, Nastran OP2, Netgen, OBJ, OFF, PERMAS, PLY, Radioss animation and time-history, SU2, Tecplot, Triangle, UGRID, UNV, VTK and Z88 readers, in the VTK-family cell reconstruction (`detail::check_vtk_cell_arrays`) and in the VTU compressed-block decoder. `NDArray` refuses a shape whose byte size overflows, and `detail::read_int` no longer casts a NaN or out-of-range float. Four fuzz passes over every reader (about 35, 25, 13 and 6 findings) left 85 minimized inputs under `tests/fuzz/regressions/`.
- **Fixed** (found by the new checks): `memset`/`memcpy` on an empty `NDArray`'s null buffer (undefined behaviour); a leak in `mio_sequence_open` when the pattern matched nothing, and in nested provenance scopes; the Python TetGen and Medit readers looping for ever on a file ending early; the Python h5m writer crashing on any cell data; **XDMF mixed topologies holding vertex cells were misread by both readers** (the writers give a Polyvertex its node count, the readers skipped it only for a Polyline), and the Python twin could not read one holding a quadratic cell at all (its node-count table lacked them), which failed the Windows CI legs, whose core has no HDF5; the libMesh `.gz`/`.bz2` fixtures were checked out through CRLF conversion on Windows (marked binary now); **`agglomerate` could build a polyhedron whose volume `compute_stats` refused** (two lobes sharing an edge), so volume was not conserved — the polyhedron kernel now accepts a non-manifold edge whose stored winding is balanced; **`conservative_interpolate` lost mass between aligned meshes** (a source corner on a target face plane was left out of the clip's cap, or clipped as outside while capped as on-plane), by up to 0.7% of the total; `diff`/`meshes_equal` in the core threw on any mesh with field data (it looked the names up as point data), so the numpy twin always answered for those; and the ParaView plugin's reader, which could not read any file (it unpacked a `CellBlock` as a tuple).
- **Fixed** (found by an audit of roadmap §4): **the ASCII readers of Gmsh, legacy VTK, EnSight, the VTK XML formats and every other reader that parses numbers through `detail::parse_double` took quadratic time** — since v12.1.0 it measured the rest of the buffer with `strlen` on every number, so an 11.8 MB ASCII `.msh` took 103 s; it now reads only the number (bit-identical results). **Writing a time series to GiD could crash or write freed memory** when the steps came from a generator keeping no reference of its own, or held non-contiguous arrays: each step's arrays were released before the step was written. An **uncompressed binary VTK XML array of 4 GiB or more was written with a truncated size header**; such files now get `header_type="UInt64"` (smaller ones keep their bytes), and a size that does not fit is refused rather than truncated. **`UInt64` values above 2⁶³ in ASCII VTK XML files** were written as negative numbers and clamped on reading. Base64 decoding of VTK XML files built its lookup table without synchronisation, a data race for C, Fortran, Julia or R callers reading from several threads.
- **Format conformance matrix**: every writable format round-trips one canonical mesh against a declaration of what it keeps (`tests/python/conformance_spec.py`, `test_conformance.py`); the new `doc/conformance.md` and a "Round trip" column in `doc/formats.md` are generated from it (`tools/gen_conformance_table.py`).
- **Property-based tests** (Hypothesis, `tests/python/test_properties.py`): volume and mass conservation, partition of unity, conformity, determinism and map composition, each tied to the page that states it.
- **Benchmarks**: `benchmark/bench.py` times every readable and writable format (`--sizes S,M,L`, legacy meshio now optional), `meshioplusplus_bench_ops` (`MESHIOPLUSPLUS_BUILD_BENCHMARKS`) times nine operations over a size sweep, `tools/bench_ops.sh` sweeps the parallel backends and thread counts, and a weekly `benchmark` workflow records both without gating.
- **ParaView plugin**: installed with the wheel under `share/meshioplusplus/paraview/`, tested under `pvpython` (`test_paraview_plugin.py`, and a `paraview-plugin` CI job with conda-forge ParaView), and its page rewritten.
- **ABI 17** (Tier B): three inline bodies in `ndarray.hpp` changed (above); see `doc/abi.md`.
- Docs: `doc/fuzzing.md`, `doc/conformance.md`, the formats page's fallback tables, the architecture page, installation, the ParaView page, the roadmap and its map, and `AGENTS.md`.

## v16.13.0 (2026-09-25)

**Closes roadmap §1.1 (DOLFINx ADIOS2 `.bp`) and §1.2 (vendor-runtime routes).** Both items are removed; the rest of §1 is renumbered (§1.3–§1.4 become §1.1–§1.2), and the run of each route against its licensed tool, with what to install for it, joins "Awaiting a licensed run". The C++ ABI stays 16: `formats/vtx.hpp` and `formats/szplt.hpp` are new headers (see `doc/abi_reviews.md`).

- **`vtx`** (new, read-only, time-capable): DOLFINx `VTXWriter` output, an ADIOS2 BP4/BP5 `.bp` directory, found by extension or by its files. Every step is a mesh at its `step` time; a `VTXMeshPolicy.reuse` step takes the last mesh written; the MPI ranks' blocks are concatenated as ParaView merges them, or welded onto the serial mesh with `ghosts="drop"` (`ReadOptions::mGhosts`); VTK Lagrange cells of degree 1 and 2 with a matching node order read as the linear and quadratic cells. DOLFINx 0.11's malformed first-step ghost blocks are skipped. Native with the new optional `MESHIOPLUSPLUS_WITH_ADIOS2` (never a default and never in the wheels; Conan `with_adios2`, vcpkg `adios2`, `_core.__has_adios2__`), else through the `adios2` package (`pip install meshioplusplus[adios2]`). Checked on real DOLFINx 0.11 runs (one and two ranks, BP4 and BP5, P1, P2 and DG0) against ParaView 6.1's VTX reader and the raw ADIOS2 arrays.
- **`szplt`** (new, read-only, time-capable): Tecplot SZL files through TecIO, which meshio++ never redistributes: natively with the new optional `MESHIOPLUSPLUS_WITH_TECIO` and `TECIO_ROOT` (Conan `with_tecio`, `_core.__has_tecio__`), else through a shared TecIO named by `MESHIOPLUSPLUS_TECIO_LIBRARY` (ctypes). TecIO fills the zone model the ASCII and `.plt` readers share, so a `.szplt` reads exactly as the same data in `.plt`; checked on TecIO-written files against ASCII twins.
- **Vendor-runtime routes** (`doc/vendor_routes.md`, `contrib/`): scripts that run in the vendor's own runtime and write files meshio++ reads — Abaqus `.odb` (`abaqus python`, ODB API: instances as parts, frames as steps, field outputs by position, sets as masks), Marc `.t16` (PyPost), Ansys results through PyDPF (to VTKHDF), Femap `.modfem` (COM API, to a neutral file) and Tecplot `.szplt` (PyTecplot or a macro, to `.plt`), with the numpy-only `.pvd`/`.vtu` series writer `contrib/common/mio_vtu_series.py`. Tested against stand-ins of the vendor APIs; the runs against the real tools are listed in the roadmap.
- **Sequences**: globs keep `.bp` directories in both engines, and a `.bp` directory is sniffed without its extension.
- **Build**: `build/configure.sh` and `configure.bat` take `--with-adios2`, `--with-tecio <dir>` and `--with-bzip2`; the amalgamated header lists the three macros; a CI job builds and tests the ADIOS2 and TecIO readers and their Python twins.
- Docs: the two format pages, one page per route, the C, Fortran, Julia, R, WASM and CLI format lists, installation, sequences, the roadmap and its map, and notebook `example/python/23_dolfinx_vtx.ipynb`.
- **Fixtures:** DOLFINx 0.11 runs of meshio++'s own scripts (`tools/gen_vtx_fixtures.py`) with ParaView 6.1 references (`tools/gen_vtx_reference.py`), and TecIO-written SZL files (`tools/gen_szplt_fixtures.cpp`, TecIO 2018.3 as vendored by SU2); the programs' licences cover the programs, not the files they write.

## v16.12.0 (2026-09-24)

**Closes roadmap section A (§1.1–§1.5), the FEM interchange formats.** The section is removed: its items shipped, and the checks that still need a vendor tool or a file only it writes are listed under the roadmap's new "Awaiting a licensed run". The rest of §1 is renumbered (§1.6–§1.9 become §1.1–§1.4). The C++ ABI stays 16: every installed-header change is an addition (see `doc/abi_reviews.md`).

- **Tecplot**: binary `.plt` of versions 71, 75 and 100–111 (Tecplot 7's zone records, 1-based connectivity and 2-D text and geometry records; the version gates of the later layouts), unit-suffixed coordinate names (`X(M)`) and comma-separated ASCII values. Both engines read VisIt's 60-file Tecplot suite identically.
- **MFEM**: `MFEM NURBS mesh` files and NURBS grid functions, each knot span evaluated (rational basis, global control points) to a Lagrange cell of the NURBS order, matching PyMFEM to 3e-14 on 19 meshes; Bernstein (`H1Pos`) and serendipity (`H1Ser`) spaces through their own bases (**Breaking:** read at their vertices before); grid functions on non-conforming meshes, in MFEM's space-filling-curve leaf order; parallel non-conforming runs (`ParPrint` rank files with ghosts).
- **Patran**: loads and boundary conditions read and written, packet 06 (`patran:distributed_load`, `_values` field data), 07/08 (`patran:force|displacement:<set>`, with `_frame:<set>`), 10/11 (`patran:temperature:<set>`, `element_temperature:<set>`); Patran 2.5 result files `.nod`/`.dis`/`.els`, text and Fortran binary, through `patran.read(path, results={name: path})` (C++ overload `read_patran(path, std::vector<PatranResultFile>)`, MCP `convert`'s `patran_results`). Checked on WARP3D's result files.
- **Abaqus `.fil`**: eigenvalue modes as steps (records 1980, with participation factors and effective masses), energies (1999, Standard and Explicit layouts), records 301–310 as field data, surface and contact output (1501–1504 and 15xx), element matrices, and rebar values. A real Abaqus 6.14 truss run's `U` and `S11` equal its `.dat` printout, and a real 6.23 buckling run gives its five modes.
- **Radioss**: per-card unit systems (`/UNIT`), boxes in a skew (`/SKEW/FIX`), `/SURF/BOX`/`BOX2`, `/SURF/PLANE` and `/SURF/ELLIPS` (field data), 4.x decks (input versions 41 and 44, titles in the keyword path), and engine decks (`<run>_0001.rad`, `radioss:engine:<keyword>` field data; a starter reads its sibling's). A real 4.1 deck reads as its 2017 translation.
- **`radioss_th`** (new, read-only, time-capable): OpenRadioss / Radioss time-history files (`<run>T01`), found by name or content; each output a step of field data (global, part and subset variables, one array per TH group). Checked on a run of OpenRadioss itself, value for value against its `th_to_csv`, and on the 81 T01 files of OpenRadioss's QA suite.
- **Z88**: the 16-node plate (type 19) is read whole as an order-3 `VTK_LAGRANGE_QUADRILATERAL` and written back (**Breaking:** read as its corner quad before); `z88i5.txt` surface loads read and written (`z88:surface_load`, `z88:surface_load:cells`); Z88Aurora's `z88sets.txt` written from cell and point regions; the 12-node volume shell (type 22) reads with a positive volume (its corners were inverted). New Z88R-solved fixtures for types 19, 21, 22 and 23; Z88R cannot compute type 23's Gauss-point stresses (documented).
- **libMesh**: the refinement tree is kept (`libmesh:tree`, `libmesh:tree:nodes`) and written back level by level with boundary sets lifted to level-0 elements, checked against meshes libMesh 1.8.0 refined itself; the native writer compresses (gzip; bzip2 with the new optional `MESHIOPLUSPLUS_WITH_BZIP2`, Conan `with_bzip2`, vcpkg `bzip2`, `_core.__has_bzip2__`), and the native reader inflates bzip2 too. **Breaking:** `.xdr.gz` and `.xdr.bz2` are written as plain XDR, as libMesh writes them and all it reads under those names (they were compressed).
- **Marc**: `INCLUDE` files, followed recursively; Herrmann, rebar and composite element types from Volume B; edge and face sets from the deck and the post file as `(cell, Marc number)` field data (`marc:edge_set:<name>`, `marc:face_set:<name>`); post-file increments that remesh bring their own mesh (they were refused).
- **Ansys `.rst`**: raw element records (energies, fluxes, gradients, nonlinear, contact and state data) as cell data, and the modal cyclic expansion (`ansys_rst_cyclic` on modal files), checked against pymapdl-reader.
- **LS-DYNA `d3plot`**: SPH particles and airbag particles as vertex cells, rigid roads, rigid-body motion, 20- and 27-node solids, `d3part` files. **`lsdyna_binout`** (new, read-only, time-capable): the LSDA binout, `nodout` outputs as steps and the other databases as field data, checked against lasso-python.
- **Nastran OP2**: complex tables (`_real`/`_imag`), random tables, SORT2 tables, element forces, fluxes and energies, springs and dampers, `PARAM,POST,-2` files, and points from `BGPDT`/`EQEXIN` without GEOM1.
- Docs for every format above, the C, Fortran, Julia, R, WASM and CLI format lists, the roadmap map, and notebook `example/python/22_section_a.ipynb`.
- **Fixtures, with their sources and licences:** VisIt's Tecplot test data (BSD-3); MFEM's meshes, evaluated with PyMFEM (BSD-3); WARP3D's result files (NCSA); cjekel's TenBarArea and Bertoldi's buckling run (MIT); pyNastran's OP2 models (BSD-3); Ansys' example data through lasso-python (MIT); an OpenRadioss run and libMesh-refined meshes of meshio++'s own decks (the programs' licences cover the programs, not the files they write).

## v16.11.0 (2026-09-24)

**Closes most of roadmap §1.1–§1.3, the remaining halves of Tecplot, Elmer, FEBio, Patran, Femap, MFEM, libMesh, Z88, Abaqus `.fil` and Radioss** (narrowed to what is left: Tecplot against `preplot`, Femap's newer element and group records, MFEM NURBS meshes, a binary Abaqus run, old Radioss decks and a few Z88 element types). One new read-only format; ABI 16 held (new functions and overloads only).

- **Tecplot polygonal and polyhedral zones** (`FEPOLYGON`/`FEPOLYHEDRON`, ASCII and `.plt`): read as `polygon` and `polyhedron<N>` cells from their faces (right-hand rule: a face is outward for its left element), zone by zone with cell-centred values; the ASCII writer writes polygon and polyhedron blocks back as such zones. TecIO-written fixtures read identically to their ASCII twins.
- **Elmer halo elements**: `elmer.write(..., halo=True)` (C++ `write_elmer(path, mesh, true)`, MCP `elmer_halo`) adds `ElmerGrid -halo`'s layer, an element copied into every part it touches by a whole side, written `id/owner`; semantically equal to ElmerGrid's on four meshes, and ElmerSolver's parallel run equals the serial one. A halo-only part is never listed first in `.shared`, which made ElmerSolver leave those unknowns at zero.
- **FEBio `.xplt` edges**: edge sections become `line`/`line3` blocks with an `xplt:edge` id and a cell region, and edge variables (FEBio 4.12's `edge contact gap`) cell data on them.
- **MFEM at any order.** **Breaking:** order-3+ `H1` meshes and fields (Gauss–Lobatto, closed-uniform and the legacy `Cubic`) are read as `VTK_LAGRANGE_*` cells instead of their vertices, interpolated from MFEM's degrees of freedom to VTK's equispaced nodes; the writer writes Lagrange cells back as `H1_<d>D_P<p>`. Checked on 14 meshes of orders 3–5 against MFEM's own high-order VTK output (1.1e-13), and MFEM reads the written files (1.4e-13). `MFEM NC mesh` files are read as their leaf elements.
- **MFEM parallel meshes.** **Breaking:** opening one rank file (`<prefix>.000000`) reads every sibling rank as one mesh with shared vertices and nodes merged (through `communication_groups`, or by position for `ParMesh::Save` files) and `partition:part` per cell; `piece=k` (C++ `ReadOptions::mPiece`, MCP `piece`) reads one rank as before. Grid functions are named by a rank file. New overload `read_mfem(path, grid_functions, ReadOptions)`.
- **Femap**: the writer writes output sets (`450`/`451`) from point and cell data, one vector per component; the reader takes Femap 2401's per-value function lists in property records. Checked against Femap 9.1–11.1 and 2401 files (not redistributable).
- **Patran**: 9-node quadrilaterals and 7-node triangles are read and written; real P3/PATRAN 3.0 and PATRAN 2.5 exports from WARP3D and Tahoe (committed under `tests/python/meshes/patran/real/` with their NCSA and BSD-3 licences) and a CUBIT 13.2 export read with their components as regions.
- **libMesh writer** (`.xda`, `.xdr`; libMesh-1.8.0 layout, byte-identical engines): subdomains from `libmesh:subdomain` or cell regions, side sets, node sets, edge and shell-face sets, original node ids and p-levels. gzip-compressed files are read natively and bzip2 through Python; `.gz`/`.bz2` names are written compressed by Python. **Breaking:** edge sets are read as `line`/`line3` cells in `<name>:edge` cell regions and shell-face sets as `<name>:shellface0`/`1` regions (they were skipped). libMesh 1.8.0, built from source, reads every written file with the same sets. C0POLYGON/C0POLYHEDRON stay refused: `XdrIO` stores no per-element node count.
- **Z88, the whole deck**: `z88i2.txt` constraints (`z88:bc:u`, `z88:bc:f`), materials (`z88:material`, `z88:E`, `z88:nu`), element parameters (`z88:elp`) and integration orders (`z88:int`) are read and written back; `z88o3.txt` is read by its column labels for every element family (beams, shafts, trusses, tori, plates, shells as scalar arrays beside the solids' `SIG`) and `z88o4.txt`'s nodal sums as `F`; a kept 3-D element type keeps the file 3-D; Z88Aurora's `z88sets.txt` element and node sets are regions. Z88R solves every written Z88OS example deck to identical displacements and stresses. New Z88R-solved fixtures: a beam and truss frame, a plate, flat shells and a torus.
- **Radioss starter decks.** **Breaking:** `/BEGIN`'s input and work length units are applied (coordinates were read as written), with `field_data["radioss:length_scale"]`. `/BOX` (`RECTA`, `CYLIN`, `SPHER`, `BOX`) and `GENE`/`GEN_INCR` groups are resolved, `BOX` and `BOX2` as the starter decides; `/SURF/PART`, `SUBSET`, `MAT`, `PROP` (`EXT`, `ALL`), `GRBRIC` (`EXT`, `FREE`), `GRSHEL`, `GRSH3N`, `GRTRIA` and `SURF` are side regions. A keyword's id is its first integer field, so a trailing unit-system id (`/BOX/RECTA/3/1`) no longer replaces it.
- **`radioss_anim`** (new, read-only, time-capable): OpenRadioss/Radioss animation files (`<run>A001`…, magic `0x542C`), recognised by name and content in every surface. One state per file with its time in `meshio:time` and `read_metadata`, so a glob over a run is a transient sequence; 1-D, 2-D, 3-D and SPH elements, nodal and element scalars, vectors, tensors, force/moment sets, ids, masses, parts, materials, properties and alive flags. Both engines match OpenRadioss's `anim_to_vtk` on generated files and on three real ones.
- **Abaqus `.fil`**: every real ASCII file of pybaqus's suite reads with both engines; no public binary run with a `.dat` printout exists, so that check stays open.
- Notebook `example/python/21_format_halves.ipynb`.

## v16.10.0 (2026-09-24)

**Closes roadmap §1.1, binary Tecplot `.plt`, §1.2, the rest of MSC Nastran HDF5, and §1.3, the missing halves of Elmer, FEBio and Fluent** (removed; a new §1.1 records what they left out, §1.7's OP2 corner, ply and coordinate-system items are closed with them, and §1 is renumbered). Every change is in both engines and reaches every registry consumer (C, Fortran, Julia, R, WASM, both CLIs, MCP).

- **Tecplot binary `.plt`** (read): the Data Format Guide's `#!TDV112` format, either byte order, registered as `.plt` under the existing `tecplot` key and recognised by its magic whatever the extension. It decodes into the same zone model as ASCII files: FE zones, variable sharing, passive variables, connectivity sharing, strands and solution times as steps, `float`/`double`/`int32`/`int16`/`byte` data. Geometry, text, label, user and auxiliary records are skipped; bit-packed data, polygonal and polyhedral zones and other versions are refused by name.
  - **Ordered zones**, ASCII and binary: an I, IJ or IJK zone becomes a `line`, `quad` or `hexahedron` block (VTK corner order, `i` fastest), cell-centred values included. Before, an ordered ASCII zone crashed the scanner.
  - **Breaking:** an ASCII zone without `DATAPACKING` is `BLOCK`, and one without `ZONETYPE` `ORDERED`, as the Data Format Guide says; `POINT` was assumed before. A `VARSHARELIST` entry without a zone shares from the previous zone.
  - A zone reuses an earlier zone's points only when all its nodal variables are shared from it (a zone sharing `X`/`Y` but owning `U` gave a point-data length mismatch), and a variable nodal in some zones and cell-centred in others is both point and cell data, NaN where it is stored the other way.
  - The Python reference reader now reads every zone, time step and sharing list, value for value like the core, so `time_step` no longer needs the core.
  - Fixtures: TecIO-written `.plt` files beside ASCII twins with the same content (`tools/gen_tecplot_plt_fixtures.py`); each `.plt` reads identically to its `.dat`. No Tecplot licence was available, so no file written by `preplot` or Tecplot 360 itself has been read.
- **Nastran coordinate systems** (`nastran_h5`, `nastran_op2`). **Breaking:** a GRID with `CP != 0` is moved to the basic system, and the nodal vectors and grid point forces of a GRID with `CD != 0` are rotated to basic; they were kept as written with a warning. The systems are the `CORD1R/C/S` and `CORD2R/C/S` tables (HDF5) or GEOM1 records (OP2), resolved in any order through GRIDs and reference systems; cylindrical and spherical ones rotate per point. OP2 `BOUG*` tables are basic already and are not rotated. `nastran:cp`/`nastran:cd` are still recorded; an unresolvable system leaves its GRIDs as written, with a warning. Checked against pyNastran's positions (1e-16) and displacements, with upstream pyNastran's rotation matrices: 1.4.1 composes the cylindrical and spherical ones the wrong way round (`tools/gen_nastran_cord_reference.py`, `static_solid_shell_bar_cord.op2`). The bulk `.bdf` reader stays raw.
- **Nastran values beyond the centre** (`nastran_h5`, `nastran_op2`): `<G>:<M>` keeps the centre value and `(cells, columns)` arrays are added beside it, NaN-padded, with `field_data["nastran:layout:<name>"] = [columns, 1]`: `<name>@corner` (solid and shell corners, at the GRID's position in the cell), `<name>@ply` (HDF5 `_COMP` tables; OP2 composites 95–98, 232, 233), `<name>@station` (beam and bar stations; OP2 `CBEAM` 2 and `CBAR` 100), and the grid point forces as `GRID_FORCE:<M>` per element node plus `GRID_FORCE:<label>:<M>` point data (`TOTALS`, `APP-LOAD`, `F-OF-SPC`...); OP2 reads `OGPFB*`. These tables were skipped with a warning. Checked against pyNastran 1.4.1 on about 24,700 values of the committed fixtures.
- **Fluent `.msh` writer.** **Breaking:** the `ansys` writer emits Fluent's own face-based layout: every face once with its `c0 c1` cells (3-D faces from `detail/face_mesh.hpp`, right-hand normal into `c0`; 2-D edges with `c0` on the left, the conventions of ANSYS Meshing and GAMBIT files), cell zones from `cell_data["ansys:zone"]` (else one per block, mixed zones with a type list, polygons and polyhedra as type 7), an interior zone, a wall zone per matching boundary block or `ansys:zone` and a default wall, zone names from the regions, ASCII or binary (`3010`/`2013` with Fluent's section framing). It wrote meshio's legacy cell-connectivity layout before, which Fluent cannot read; the reader still reads that. A 2-D mesh is written with 2-D points (it must lie in z = 0), and reads back rebuilt from its faces, so a `.msh` written without a format or gmsh tags no longer round-trips its connectivity row for row. `vtkFLUENTReader` reads every output with the original cell counts and volumes, FEconv's real meshes included; C++ and Python write the same bytes.
- **Elmer.** ElmerGrid's binary meshes (`mesh.*.bin`, and single-precision `mesh.nodes.sbin`) are read, serial or partitioned; the byte order is taken from the first record's id. A mesh with `cell_data["partition:part"]` is also written as `partitioning.N/part.k.{header,nodes,elements,boundary,shared}` (it was dropped with a warning): parts keep the serial ids, shared nodes are owned by their lowest part, boundary elements go to their parents' parts. `ElmerSolver_mpi -np 2` on a written partition gives the serial temperatures to 2e-14. Halo elements are not written.
- **FEBio.** The `.feb` writer writes point and cell data as `<MeshData>` (`<NodeData>`/`<ElementData>`, `scalar`, `vec2`, `vec3`, `mat3s`, `mat3` by width) over its own `meshdata:<name>` set, which the reader makes no region of; they were dropped. FEBio 4.12 uses a written array as a material map. The `.xplt` reader reads **surface variables**: each data surface is a facet block after the domains with a `cell` region of its name and `cell_data["xplt:surface"]`, per-facet and per-surface variables are its cell data, per-node ones averaged point data (**Breaking** for code that assumed one block per domain). A **remeshed run** reads each state on its own mesh instead of raising `ReadError`; metadata describes the first. Edge variables are still skipped.
- New fixtures: `tests/python/meshes/tecplot/plt/`, `nastran_h5/cord_reference.npz`, `nastran_op2/static_solid_shell_bar_cord.op2` with `cord_reference.npz`, the extended `nastran_op2/pynastran_reference.npz`, ElmerGrid's binary output of two suite meshes (`elmer/tet10_two_bodies_bin`, `quad8_2d_sbin`), and FEBio 4.12 plot files of a surface-load and a remeshing model (`febio/xplt/xplt_surface`, `xplt_remesh`). Example notebook `20_tecplot_nastran_fluent_elmer_febio.ipynb`. ABI 16 held: the installed-header changes are additions and comments, recorded in the ABI review table.

## v16.9.0 (2026-09-24)

**Closes roadmap §1.1, the LS-DYNA `d3plot` family, and §1.2, Nastran OP2** (both removed, §1.3–§1.12 renumbered, and a new §1.7 records what they left out). Two new read-only, time-capable formats in both engines and every registry consumer (C, Fortran, Julia, R, WASM, both CLIs, MCP).

- **`lsdyna_d3plot`** (read, time-capable): LS-DYNA's binary state database. No extension: the file named `d3plot` (any case) resolves to it in `resolve_format`, the Python dispatch and both sniffers, and a renamed one is recognised by its control block. The family `d3plot`, `d3plot01` ... `d3plot100` is one stream of states, read in numeric order; a numbered member opened on its own is refused with the base file's name, and sequence globs keep the base file only.
  - Single and double precision, either byte order, sniffed from the control block; `d3eigv` files too. `d3part`/interface files, femzip, `NDIM = 2`, CFD/multi-solver data, adaptive runs and 20/27-node and cubic solids are refused by name.
  - Solids, thick shells, beams and shells as cell blocks per family and type; degenerate 8-node solids collapse by the keyword reader's rules (`detail/degenerate_solid.hpp`), `n4 = n3` shells are triangles, ten-node solids `tetra10`, eight-node shells `quad8`. `lsdyna:nid`, `lsdyna:eid`, `lsdyna:part`; one region per part, named by its title.
  - Each state: `displacement` (current minus initial coordinates), velocity, acceleration, temperature and the other nodal variables as point data; stress, plastic strain, history variables and strains per integration point or layer as `(cells, points × components)` with `lsdyna_d3plot:layout:<name>` (the Abaqus `.fil` convention), shell resultants, thickness and energy, beam forces and stresses as cell data; the deletion flags as the int8 mask `lsdyna:alive`, per cell or per point; global and per-part energies as field data. Rigid shells have NaN state data; rigid bodies, rigid roads, SPH and airbag particles are sized and skipped.
  - Checked against lasso-python 2.0.4 on every state of its five real test families and of three shell + solid + beam families written with its own writer (single, double precision, one file per state), whose shells and a solid are deleted state by state. Two deliberate deviations: the ten-node solids' extra nodes are read once, after the 8-node connectivity, as the database manual has them (lasso-python reads and writes a second copy, skipped when present), and states are counted to a file's last non-zero *word*, so big-endian families keep their last state.
- **`nastran_op2`** (`.op2`; read, time-capable, also recognised by content): MSC and Simcenter (NX) Nastran OP2, 32- and 64-bit (NX's interlaced table names included), either byte order.
  - The mesh is built as `nastran_h5` builds it: GEOM1 GRIDs (NX's double-precision GRID records too; CP/CD kept as `nastran:cp`/`nastran:cd` with a warning), GEOM2 element records through the shared card table (a record length that fits more than one layout takes the first whose entries validate; NX 2019's CQUAD4/CTRIA3 keys), `nastran:eid`/`nastran:pid`, one region per property named from the EPT records (NX's PSHELL twins of PCOMPs ignored, PCOMPG over PCOMP). Without GEOM1 GRIDs, the input deck beside the file (`<stem>.bdf`/`.dat`/`.nas`/`.blk`) gives the mesh.
  - One step per (subcase, analysis, mode/time/load step) of the tables read: real SORT1 displacement, eigenvector, velocity, acceleration, SPC and MPC force, applied load and temperature as point data (`nastran_h5`'s names, `_ROT` twins); the centre stress and strain of rods, tubes, shear panels, bars, shells (centroid and corner types) and solids as `STRESS:<M>`/`STRAIN:<M>` cell data, with `nastran_h5`'s member names plus the derived values (`VON_MISES1`, `MAJOR1`, `PRINCIPAL_A`...). Complex, random and SORT2 tables, corner and ply values, other element types and other tables are skipped with one warning naming them.
  - Checked against pyNastran 1.4.1 on every step of eleven MSC and NX files (SOL 101, 103, 105, 159, 64-bit, NX 2412) and 40 further pyNastran models, and on a file stripped of its geometry beside its deck.
- **Shared pieces.** `detail/nastran_model.hpp` (and `nastran/_model.py`): the Nastran card table, cell blocks, EID map and property regions `nastran_h5` built inline, now shared with OP2 (`nastran_h5`'s output unchanged); `detail::nastran_read_deck`, the bulk reader with the GRID and element ids it read. `_fortran_records.py`, the Python twin of `detail/fortran_records.hpp`, now used by the Abaqus `.fil` and OP2 readers.
- New fixtures: lasso-python's d3plot test families and the lasso-written ones with `lasso_reference.npz` (`tools/gen_d3plot_reference.py`); pyNastran OP2 models with `pynastran_reference.npz` (`tools/gen_nastran_op2_reference.py`), all BSD 3-Clause. Example notebook `19_d3plot_op2_results.ipynb`. ABI 16 held: the new installed headers are additions, recorded in the ABI review table.

## v16.8.0 (2026-09-24)

**Closes roadmap §1.1, MSC Marc, and most of §1.2, Ansys `.rst` element results** (§1.1 removed, §1.2 narrowed into a new §1.8 with what Marc left out; §1.1–§1.12 renumbered). Two new read-only formats and a full-rotor reading of cyclic `.rst` files, in both engines and every registry consumer (C, Fortran, Julia, R, WASM, both CLIs, MCP).

- **`marc`** (`.dat`; read): MSC Marc input decks. `.dat` stays Tecplot's unless the file opens as a Marc deck (a Marc parameter with no `=` first, then `END`, `CONNECTIVITY` or `COORDINATES`): a content check in the C++ `resolve_format` (as for MFEM's `.mesh`) and in the Python reader, which refuses other files so that Tecplot's is tried next; both sniffers recognise decks by content.
  - `COORDINATES` and `CONNECTIVITY` in fixed (5/10 columns, doubled by `EXTENDED`), extended and free (comma) format, with Mentat's reals without an exponent letter and fields that touch; a node defined twice keeps its last definition. Marc element types 2–201 that meshio++ maps (see the page) are all in meshio++'s node order, so no permutation applies; an 8-node brick with repeated nodes collapses to a wedge, pyramid or tetrahedron; other types are skipped with a warning. `marc:element` and `marc:type` cell data.
  - `DEFINE ELEMENT SET` / `NODE SET` (and `ELSQ`/`NDSQ`) as `cell` and `point` regions: numbers, `TO`/`THROUGH` ranges with `BY`, earlier sets' names, `AND`/`EXCEPT`/`INTERSECT`, `C` continuation.
  - Checked on the twelve Marc Mentat 2020 decks of DAMASK's element-library tests (AGPL, not committed): every cell positively oriented, every mid-edge node at its edge's midpoint.
- **`marc_t19`** (`.t19`; read, time-capable): Marc formatted post files (PLDUMP2000 layout, revision 9 on). The same mesh and sets, and one step per increment (`meshio:time`, a frequency or buckling factor in a modal, harmonic or buckling analysis; `marc:increment`, `marc:subincrement`): nodal vectors as point data under the file's names, element post codes per integration point as cell data (`(cells, points × components)` flattened point-major with `marc:layout:<name>`, the Abaqus `.fil` convention, so every writer holds them), a tensor's six codes (301, 311, 401, 411 ...) combined as `xx yy zz xy yz zx`, whose order a real Marc post file confirms (its von Mises stress matches code 17). Remeshing increments are refused. `marc_t19` joins the sequence engine.
- **Ansys `.rst` element results** (`ansys_rst`): each result set also gives
  - reaction forces: `RF` and `RMOM`, rotated to the global axes like `U`, and `RF_<DOF>` (`RF_TEMP` in a thermal `.rth`);
  - element nodal stresses and elastic, plastic, creep and thermal strains (`S`, `EPEL`, `EPPL`, `EPCR`, `EPTH`), rotated from the element system by its Euler angles: per element node as cell data `(cells, nodes × 6)`, flattened point-major with the widest block's node count and `ansys:layout:<name>` (the Abaqus `.fil` convention, so VTU and XDMF round-trip them), NaN at midside nodes, and averaged at the corner nodes as point data, matching pymapdl-reader's `nodal_stress` and `nodal_elastic_strain`; a layered SHELL181/281's top surface as `<name>@top`; the 11-item records of files before release 14.5;
  - element nodal forces `ENF` as cell data `(cells, nodes × DOFs)`, which sum at every node to minus the reaction or the applied load.
  - A negative entry `-n` in an element's result table is read as `n` zeros (MAPDL writes no all-zero record); pymapdl-reader does the same.
- **Distributed solves:** reading the main file (`file0.rst`) reads `file1.rst` ... beside it, merged by node number with reactions summed at shared nodes; it matches the combined file MAPDL wrote. Another partial file is refused, naming the main one.
- **`ansys_rst_cyclic`** (by name only; read, time-capable): a static cyclic-symmetry model's full rotor, the base sector repeated round its cyclic axis (a local coordinate system's Z where the model names one) with every vector and tensor rotated; `ansys:sector` cell data and `ansys:sectors` field data. Matches pymapdl-reader's full rotor to 1e-15 on two MAPDL files. Modal cyclic expansion is refused. `ansys_rst.read_cyclic` in Python.
- `detail::ansys_build_mesh` gains an overload that also returns each element's cell and node slots (`AnsysCellLocation`). New MIT fixtures from pymapdl-reader: a static SOLID186 beam, a release 13 file, SOLID186 with SHELL281, a cyclic rotor about a local axis and a distributed solve with its combined file; `pymapdl_reference.npz` also freezes pymapdl-reader's averaged stresses and strains, reactions and full rotors.
- Example notebook `18_marc_ansys_results.ipynb`. ABI 16 held: the new installed headers and declarations are additions, recorded in the ABI review table.

## v16.7.0 (2026-09-24)

**Closes roadmap §1.1, libMesh, §1.2, Z88, §1.3, Abaqus `.fil`, and §1.4, OpenRadioss** (all four removed; §1.5–§1.16 renumbered, and a new §1.9 records what they left out). Four new formats in both engines and every registry consumer (C, Fortran, Julia, R, WASM, both CLIs, MCP).

- **`libmesh`** (`.xda`, `.xdr`; read, also recognised by content): libMesh's own mesh file, every `XdrIO` version from `libMesh-0.7.0+` to `libMesh-1.8.0`, as ASCII and as big-endian XDR.
  - Only the active elements of the refinement tree become cells (`libmesh:level`, and an inline p-level as `libmesh:p_level`); subdomains are `libmesh:subdomain` and named `cell` regions; side sets are `side` regions, carried from a refined element to its active children; node sets are `point` regions. Unused node ids (NaN in the file) are dropped, the kept ids stored as `libmesh:id`.
  - New `"libmesh"` node-order tables for `hexahedron20`, `hexahedron27`, `wedge15` and `wedge18`, from libMesh's own VTK connectivity. TET14, PRISM20/21 and PYRAMID18 keep the nodes meshio++ has a type for, with a warning. Legacy `DEAL`/`LIBM` files are refused, as libMesh refuses them.
- **`z88`** (`z88i1.txt`, `z88structure.txt`; read/write): Z88's structure file, **dispatched by its fixed file name** ahead of `.txt`'s xyz reader in `resolve_format`, the Python dispatch and both sniffers.
  - Z88OS v15 and Z88 <= V13 / Aurora V1 headers, cylindrical input (`KFLAG = 1`), every element type 1–25 (the cubic and layered ones keep their corners); the type is kept as `z88:type`. New `"z88"` node-order tables: hexahedra list their top face first, tet10 its last mid-edges 2-4, 3-4, 1-4.
  - `z88o2.txt` beside it is attached as `point_data["U"]`, and the solid and plane-stress blocks of `z88o3.txt` as the element mean stress `SIG` and `SIGV`; reading `z88o2.txt` reads the structure file next to it (`z88.read(..., results=False)` skips them).
  - The writer emits the Z88OS v15 layout (`z88.write(..., stubs=True)` also writes empty `z88i2.txt`/`z88i5.txt`). Z88R V15 solved the fixture deck, whose results are committed, and the deck meshio++ writes back to the same displacements.
- **`abaqus_fil`** (`.fil`; read, time-capable, also recognised by content): the Abaqus results file, binary in either byte order (new `detail/fortran_records.hpp`) or ASCII.
  - The model (nodes, elements through the `.inp` type table and element families, `abaqus:id` labels), node and element sets as regions with long names resolved through the 1940 labels, and one step per increment (`time_step`, `abaqus_fil.time_values`, `meshio:time`, `abaqus:step`/`increment`/`step_time`/`procedure`); `abaqus_fil` joins the sequence engine.
  - Nodal records become point data and element records cell data by output location: integration points and element nodes flattened point-major to `(cells, points × components)` with the same width in every block (layout in `field_data["abaqus:layout:<name>"]`), centroid values `(cells, components)`, nodal averages as point data; components keep the file's order; shell section points above 1 get `@sp<k>`.
  - Checked against pybaqus on real Abaqus 2023 output, four files of which are committed (MIT).
- **`radioss`** (`.rad`; read, also recognised by content): the OpenRadioss / Radioss starter deck.
  - 10/20-column fields (8/16 before input version 51, commas accepted), `#include` and `#enddata`, `/END`; `/NODE`, `/BRICK` (degenerate bricks collapsed), `/PENTA6`, `/TETRA4`, `/TETRA10`, `/BRIC20` (new `"radioss"` node-order table), `/SHELL`, `/SH3N`, `/QUAD`, `/TRIA`, `/BEAM`, `/TRUSS`, `/SPRING`; inverted tetrahedra reoriented; `radioss:part`/`property`/`material` cell data.
  - `/PART` and `/SUBSET` as `cell` regions, `/GRNOD` and the element groups (by ids, parts, subsets, other groups, surfaces) as `point`/`cell` regions, `/SURF/SEG` as `side` regions. Read on OpenRadioss's 81 QA decks.
- **Shared helpers:** `detail/binary_stream.hpp` (a bounds-checked byte cursor in either byte order), `detail/fortran_records.hpp`, `detail/abaqus_types.hpp` (the Abaqus `.inp` element table, moved out of `abaqus.cpp`) and `detail/degenerate_solid.hpp` (LS-DYNA's brick collapse, now shared).
- **Fixes:** the Abaqus `.inp` reader mapped `C3D4H` to the invalid type `"tetra4"`; it is now `tetra` (both engines). The degenerate-brick collapse (LS-DYNA and Radioss) also recognises a wedge whose side edge is collapsed in both faces (`n1 n2 n3 n1 n5 n6 n7 n5`), which was left a hexahedron.
- Example notebook `17_libmesh_z88_fil_radioss.ipynb`. ABI 16 held: the new installed headers are additions, recorded in the ABI review table.

## v16.6.0 (2026-09-23)

**Reader fixes from FEconv's example meshes.** A review pointed at [FEconv](https://github.com/victorsndvg/FEconv)'s `examples/`, small meshes in many formats, several of them the same mesh written by different tools. Reading all of them exposed bugs in six readers and one writer, fixed here in both engines. FEconv is GPL, so its files are not committed: `tools/gen_feconv_quirk_fixtures.py` generates small fixtures that reproduce each quirk, and `tests/python/test_feconv_examples.py` runs over a local checkout when `MESHIOPLUSPLUS_FECONV_DIR` is set (140 checks: engine parity, orientation, mid-edge placement and cross-format twins). The COMSOL files the review named (`2objectcubes.mphtxt`, `surfacesphere.mphtxt`) already read correctly since the v16.1.0 rewrite. `MESHIOPLUSPLUS_ABI_VERSION` stays 16: no installed header changes (ABI review row for v16.6.0).

- **Fluent `.msh` (`ansys`): cells rebuilt from their faces.** A Fluent mesh defines its cells only through face rows `n0 .. nk c0 c1`; the reader returned the faces and never a volume cell, and the C++ reader handed every file with face sections to Python. Both engines now give each cell its outward faces (the right-hand normal points into `c0`) and rebuild positively oriented tetrahedra, pyramids, wedges and hexahedra, or polyhedra, through the face-to-cell kernel the OpenFOAM reader already used (now shared: `formats/face_cells_common.hpp`, `_face_cells.py`); 2-D cells are chained from their edges. Boundary faces are kept, wound outward; `cell_data["ansys:zone"]` and one cell region per zone, named from `(39 ...)`/`(45 ...)`. TGrid's out-of-order node zones, GAMBIT's `(13(` and `c0 = 0` rows, bodyless binary declarations and binary mixed face zones are accepted. All 29 Fluent meshes in FEconv read identically in both engines; one gives the same 224 tetrahedra as its UNV twin. Files in meshio's own layout (cell sections with connectivity, what the writer emits) read as before.
  - **Breaking:** interior faces are no longer returned as `triangle`/`quad` blocks, and face-based files now carry volume cells, zone data and regions.
- **FLUX `.pf3` (`flux`): node order.** FLUX lists every solid in VTK's order mirrored, and `tetra10` its mid-edges in its own order, so every solid read inverted. New `flux` tables in the node-ordering registry (`tetra`, `tetra10`, `pyramid`, `wedge`, `wedge15`, `hexahedron`, `hexahedron20`; `wedge15` has no sample) are applied on read and write. Truncated files keep what they hold with a warning, sparse node ids are renumbered, and the file is read as Latin-1.
- **VTU: appended data, byte order and multiple pieces.** The C++ reader refused any `<AppendedData>` and any second `<Piece>`, and the Python one failed on offsets padded with blanks, on base64 appended data and on BigEndian raw files, and kept only the last piece's cells. Both engines now read raw and base64 appended data (header and body encoded together or apart), honour `byte_order="BigEndian"` inline and appended, and merge pieces, including polyhedra; data an array some piece lacks is dropped with a warning. `read_metadata` follows the same rules.
- **VTU writer: `faceoffsets` after an ordinary block.** The Python writer (which writes every polyhedral file) put a polyhedron's offset one short when an ordinary cell preceded it, and VTK 9.4+ / ParaView 6 refused the file; OpenFOAM-derived meshes have that layout.
- **Gmsh:** section keywords may be indented. **Medit:** a file without `Dimension` takes it from the first vertex row.
- **UNV:** Code_Aster's `%` comments after integer records, and a 164 units record whose description touches its code (`5mm`), are read.
- Roadmap: a Fluent writer Fluent reads (§1.14), appended data and pieces in the other VTK XML readers (§5).
## v16.5.0 (2026-09-23)
...
- **Breaking:** `CellType` gains `Triangle7`, appended to the X-macro that precedes `CellType::Custom`, so `Custom` moves from 76 to 77 and the inline cell-type tables grow (the C API's `MIO_CELL_Custom` moves with it). An appended enumerator was documented as additive; with `Custom` after it, it is Tier A, and `doc/abi.md` now says so. Code compiled against v15 headers must be rebuilt. `test_abi_layout.cpp` pins both values.
- **Code_Aster `.mail`.** `COOR_1D/2D/3D`, the twenty element keywords from `POI1` to `HEXA27`, `GROUP_MA` → cell regions and `GROUP_NO` → point regions (no tag; one name can be both kinds). Tokens are separated by blanks or commas and `%` starts a comment. **Only the first 80 columns of a line are read**, as Code_Aster reads them. Records are token streams that may wrap, and block-header options may span lines. Undefined or duplicated names are a `ReadError`, as they are fatal in Code_Aster; unsupported keywords are skipped with a warning. The writer names nodes and elements `N…`/`M…`, keeps every line within 80 columns, writes coordinates as `%.16E`, sanitises group names to 24 letters, digits and `_`, and drops side regions and data arrays with a warning and a provenance note. Both engines read the same meshes and write the same bytes. Sniffed from a leading `TITRE`/`COOR_nD`, registered for every registry consumer (C, Fortran, Julia, R, WASM, both CLIs, MCP).
- **Code_Aster's node order is not MED's**, contrary to what the roadmap assumed: Code_Aster's own MED reader (`lrmtyp.F90`) permutes every 3-D element. The `.mail` tables come from its gmsh reader (`inigms.F90`) composed with meshio++'s gmsh tables, and agree with its MED reader up to a symmetry of the reference cell; `HEXA20/27` and `PENTA15/18` put the vertical mid-edges before the top ring. All 565 `.mail` meshes of Code_Aster's test suite (read locally, not redistributed) read identically in both engines, with the quadratic mid-nodes on their edges. The done-when holds: a `HEXA20` `.mail` through MED and back keeps its connectivity and groups line for line.
- **Node-ordering registry** (`detail/node_order.hpp`, `_node_order.py`, [node ordering](doc/node_ordering.md)). Every *(format, cell type)* permutation in both directions as gather tables. The MED, `.frd` and UNV tables move there from their readers, unchanged (every existing test passes and every writer's output is byte-identical). A self-test maps a reference element through every table, checking midpoints, centroids and orientation, and compares the C++ and Python tables (`_core.node_order_table`). The gmsh, CGNS, GiD, Exodus, Kratos and COMSOL tables stay in their readers for now (roadmap §1.20).
- **MED `H27`/`P18`.** `hexahedron27` and `wedge18` read and write as MED `HEXA27`/`PENTA18` in both engines. Their tables are derived through Code_Aster's MED reader, and a file written by MED-fichier itself (`tests/python/meshes/med/hexa27_penta18.med`, `tools/gen_med_quadratic_fixture.py`) reads back as the reference elements. `hexahedron27`'s table is not its own inverse, so writing uses the registry's other direction. **Breaking (bug fix):** the Python engine named the `wedge18` group `PE18`; MED's name is `P18`.
- **`triangle7`.** A `triangle6` plus a centre node (Code_Aster `TRIA7`, MED `TR7`, VTK 34), which the VTK and MED readers already produced by name. It now has a node count, a dimension, `triangle6`'s edges, and a linear base for `convert_cells(linearize)`, `extract_surface` and projection.
- **Fixtures and tests.** `tools/gen_code_aster_fixtures.py` writes three `.mail` files in Code_Aster's own numbering: every keyword, a `HEXA20` block laid out as Code_Aster's `PRE_IDEAS` writes one, and a `COOR_2D` plate. `test_code_aster.py` covers both engines, byte-identical writes, mid-edge and orientation checks, the MED round trip, name sanitising and the error paths. Also `test_node_order.py`, `test_triangle7.py`, `tests/cpp/test_code_aster.cpp` (12), `test_node_order.cpp`, `test_triangle7.cpp`, region round-trip, provenance, sniff, registry, MCP and WASM smoke entries.
- **Example.** `example/python/12_code_aster.ipynb`, executed with outputs: groups, every keyword rendered, the node-order tables with a mid-edge check, `.mail` → `.med` → `.mail`, and what the writer emits.
- **Tests and hashes.** The four `BASELINE_HASHES` were refreshed for the version only: the old hashes were verified against the new core with the version substituted first.
- **Not verified in this change:** a real `LIRE_MAILLAGE` run on a meshio++-written file (no Code_Aster installation here; the writer follows Code_Aster's reader and its own writer's layout); `COOR_1D` files (none in Code_Aster's test suite); the R and Emscripten builds (not installed here; the WASM smoke step was syntax-checked only).

## v15.7.0 (2026-09-23)

**Closes roadmap §1.1, MSC Nastran HDF5 results** (removed; §1.2–§1.18 renumbered to §1.1–§1.17, and a new, narrower §1.18 records what the reader leaves). The result database MSC Nastran writes with `MDLPRM,HDF5` is a new **read-only** format, `nastran_h5` (`.h5`): the model and every result domain of a run, readable with no Nastran installation and checked bit for bit against pyNastran on real MSC Nastran 2020 output. `MESHIOPLUSPLUS_ABI_VERSION` stays 15: `formats/nastran_h5.hpp` is a new header and `detail/hdf5_util.hpp` only gains declarations ([ABI review](doc/abi_reviews.md)).

- **The model.** GRID points as written, one cell block per element card and order (`CTRIA3/6`, `CQUAD4/8`, `CQUAD`, `CTETRA`, `CPYRAM`, `CPENTA`, `CHEXA` with their quadratic variants, the line elements, `CONM2` as a vertex), `cell_data["nastran:eid"]`/`["nastran:pid"]`, and one cell region per property id named after its property card (`PSHELL_4`). Quadratic nodes are permuted to meshio++'s order (the bulk reader's hexahedron20/wedge15 tables) and checked geometrically; a card with only some mid-side nodes reads as linear with a warning; scalar-point cards are skipped with a warning; an element on an undefined GRID is a `ReadError`.
- **Results are steps.** Every result domain (a `/NASTRAN/RESULT/DOMAINS` row: a subcase and its mode, time or frequency) an INDEX table references is a step: `time_step`, `read_metadata(...)["time_values"]` and `read_sequence` work as for `.frd`, through `read_nastran_h5(path, ReadOptions)`, `read_nastran_h5_metadata`, the registry (so C, Fortran, Julia, R, WASM and the native CLI) and MCP. Only the chosen domain's rows are read, through hyperslabs. A step carries `meshio:time` (the domain's `TIME_FREQ_EIGR`, as written) and `nastran:domain`/`subcase`/`step`/`analysis`/`mode`/`eigi`.
- **Nodal and element results.** Nodal tables become point data (`DISPLACEMENT`, `DISPLACEMENT_ROT`, `EIGENVECTOR`, the `_CPLX` tables as `<name>_real`/`<name>_imag`, `TEMPERATURE`, `<table>:<member>` for the rest), NaN where a table has no row; the SPOINT rows MSC writes into the same tables are dropped. Element tables become cell data named `<group>:<member>` (`STRESS:X`, `STRAIN:TXY1`), the first entry of an array member (the centre of a solid), NaN for cards without the member. Tables with several rows per entity (per-ply `_COMP`, `GRID_FORCE`) are skipped with a warning.
- **Refused, not misread.** A `.h5` that is not an MSC Nastran file, or whose `/NASTRAN` `VERSION` names another vendor, is a `ReadError`; so is a result table naming a domain `DOMAINS` does not define. `.post.h5` stays GiD.
- **HDF5 helpers.** `h5::compound_members()` and `h5::read_compound_member()` (one member of a compound table over a row range, with an optional dtype conversion) are the first compound-table reads in `detail/hdf5_util.hpp` — the half of the roadmap's "HDF5 schema helper" that VTKHDF did not need.
- **Fixtures and tests.** Six MSC Nastran 2020 files (SOL 101, 103, 105, 107, 108, 159) from the pyNastran repository (BSD 3-Clause, notice in `tests/python/meshes/nastran_h5/README.md`, credited in `CITATION.cff`), through Git LFS, and `pynastran_reference.npz`, pyNastran 1.4.1's own reading of their 38 nodal vector tables (`tools/gen_nastran_h5_reference.py`). `test_nastran_h5.py` (79 tests: both engines, bit-for-bit against pyNastran, engines identical on every step of every file, positive volumes, quadratic mid-nodes on their edges, and h5py-built files for the CP, vendor, SPOINT, INDEX and DOMAINS paths), `tests/cpp/test_nastran_h5.cpp` (7 native tests, building their files with the HDF5 C API), registry lines, an MCP and a WASM smoke step.
- **Example.** `example/python/11_nastran_h5.ipynb`, executed with outputs: the model and its regions, static displacement and shell stress, the steps of six solution sequences, four buckling modes warped, a frequency response's real and imaginary parts, a thermal transient, the pyNastran check and one `.vtu` per mode.
- **Done-when check.** The SOL 103 file converts to a `.vtu` per mode (`meshioplusplus convert job.h5 'mode_{step}.vtu'`) whose eigenvectors, read back by ParaView 6.1.1's own VTU reader, equal pyNastran's bit for bit.
- **Tests and hashes.** The four `BASELINE_HASHES` were refreshed for the version only: the old hashes were verified against the new core with the version substituted first.
- **Not verified in this change:** a file with `CP`/`CD` other than 0 (all six fixtures use the basic system; such coordinates are read as written, with a warning — roadmap §1.18); MSC releases other than 2020 (the schema grows each release; the reader reads members by name); a file from another vendor (refused by the `VERSION` check, not seen); the `ANALYSIS` codes, which are undocumented publicly, so `meshio:time` is never converted to a frequency; the `.op2` files next to the fixtures upstream, which come from an older run of a different model and are no reference; the R and Emscripten builds (not installed here — the WASM smoke step was syntax-checked only).

## v15.6.0 (2026-09-22)

**Closes roadmap §1.1, the I-DEAS Universal File** (removed; §1.2–§1.23 renumbered to §1.1–§1.22). The roadmap called it greenfield; `unv` had in fact read and written meshes for a long time, but only ever against its own writer, and checking it against three independent implementations turned up five places where it disagreed with the SDRL records. This release pins it against gmsh, Salome's reader and pyuff, turns its groups into regions and its results into the steps of a sequence, and adds the test-lab side of the format: the frequency-response functions of dataset 58/58b and the `.uff` extension. `MESHIOPLUSPLUS_ABI_VERSION` stays 15: `formats/unv.hpp` only gains declarations ([ABI review](doc/abi_reviews.md)).

- **Results are steps.** Every 2414, 55 and 56 block is keyed by its analysis type (static, normal mode, complex eigenvalue, transient, frequency response, buckling, non-linear static), its step/mode/frequency number and its time/frequency/eigenvalue; blocks with the same key merge into one step. `time_step`, `read_metadata(...)["time_values"]` and `read_sequence` work as for `.frd`, through `read_unv(path, ReadOptions)`, `read_unv_metadata`, the registry (so C, Fortran, Julia, R, WASM and the native CLI's `--time-step`) and MCP. A step carries `meshio:time`, `unv:analysis` and `unv:step`, which the writer writes back into records 9–13. Complex data reads as `<name>_real`/`<name>_imag`, entities without a value as NaN, and a blank or `NONE` dataset name falls back to the result type (`displacement`, `stress`, …).
- **Dataset 58 and 58b.** A test-lab file's functions at nodal DOF (FRFs, coherences, spectra…) of one kind and one frequency grid are one group; each abscissa value is a step, with the responses at every node as `point_data` (`frf_real`/`frf_imag`, three components for X/Y/Z, `_rot` for rotations, signs from the response and reference directions). The binary 58b's block size is taken from record 7: pyuff writes half the real size for complex data. The values match pyuff's own reader on every step.
- **Groups are regions.** Permanent groups in both layouts (2467/2477/2452/2435 quadruples, 2417/2429/2430/2432 pairs — the latter four were read with the wrong stride or not at all) become a `point` region of their nodes and a `cell` region of their elements, tagged with the group number; the writer emits every point and cell region as a 2467 group. The generic registry paths therefore carry groups for the first time (C API, CLI, WASM without `info`), and UNV leaves `test_region_roundtrip.py`'s Phase-2 list for the round-trip matrix. `UnvInfo` still works and is filled too.
- **More of the format.** Legacy nodes 15 and elements 780; units 164 (`field_data`, not applied); Cartesian 2420 coordinate systems applied to the nodes (`x = M·x_local + o`, Salome's convention; cylindrical/spherical warned about); `quad9`, `pyramid` and `pyramid13`; the plane-stress/strain, plate, membrane and axisymmetric descriptors; material property ids as `cell_data["unv:mid"]`; the `.uff` extension and content sniffing in both engines. An element whose node count does not match its descriptor (gmsh writes its `quad9`/`hexahedron27` under 94/115) is skipped with a warning instead of misread.
- **Breaking:** group entity types were swapped (the SDRL records, gmsh and Salome use 7 for nodes and 8 for elements). Files written by meshio++ ≤ 15.5 now read their node groups as element groups and the other way round.
- **Breaking:** `hexahedron20` and `wedge15` put the four (three) vertical mid-edge nodes after the top ring; UNV lists them between the rings. Pinned against gmsh's `.unv`/`.msh` twins and Salome's reader; quadratic hexahedra and wedges written by older meshio++ read with those nodes misplaced.
- **Breaking:** symmetric tensors are now reordered between the file's `Sxx Sxy Syy Sxz Syz Szz` and meshio++'s `xx yy zz xy yz zx` in both directions; they were copied unpermuted.
- **Breaking:** `code_aster=True` writes element data as dataset **56** (data at elements), not 57 (data at nodes on elements, Code_Aster's ELNO), with records 7–8 sized by their own counts; the reader parses 55/56 that way, so pyuff and Code_Aster files read correctly. The older padded layout is recognised and still read, with a warning.
- **Breaking:** a multi-step file reads its first step by default instead of every dataset as `name`, `name_2`, `name_3`…; and a value missing for an entity is NaN, not 0.
- **Also fixed:** 2414 values are written in double precision (`E20.12`, one line per entity, the layout pyuff's element reader needs) instead of 6 significant digits; element records carry their NDVAL; the property id is no longer written into the material field; a reference to an undefined node is a `ReadError`, not `std::out_of_range`/`KeyError`; the Python reader no longer crashes on a binary dataset header; `test_provenance`'s UNV parity check, which silently never ran the C++ writer, now does.
- **Fixtures and tests.** `tools/gen_unv_fixtures.py` writes `tests/python/meshes/unv/` from gmsh 4.15.2 (quadratic tetrahedra, wedges and hexahedra with their `.msh` twins), pyuff 2.5.6 (MIT; units, 55 mode shapes, 58 and 58b FRFs) and hand-written records (2414 modes and transients, 2420, 15/780, the older layout). `test_unv.py` (46 tests: node-for-node against the `.msh` twins, mid-edge positions, both engines on every step of every fixture, byte-identical writers, pyuff in both directions, strict core, sniffing), `tests/cpp/test_unv.cpp` (11 native tests), a region-matrix row, an MCP and a WASM smoke step.
- **Example.** `example/python/10_unv_universal_files.ipynb`, executed with outputs: gmsh meshes and their groups, three mode shapes read as steps and warped, a measured FRF set plotted over its frequency sequence, and a write-back of groups and a result step.
- **Tests and hashes.** The four `BASELINE_HASHES` were refreshed for the version only: the old hashes were verified against the new core with the version substituted first.
- **Not verified in this change:** a file exported by Salome itself (none installed; its reader's source and a Salome-generated FreeCAD test file stood in, the latter read locally but not committed because of its licence); the orientation convention of cylindrical and spherical 2420 systems (warned about, not applied); the R and Emscripten builds (not installed here — the WASM smoke step's input was checked through the native reader instead).

## v15.5.0 (2026-09-22)

**Closes roadmap §1.1, "Half-gaps of shipped formats"** (the section and the now-empty "A. Shipped formats: the remaining halves" header are removed, §1.2–§1.24 renumbered to §1.1–§1.23). Six holes in already-shipped formats, one branch, one release: a new data operation, and gaps closed in `.frd`, SU2, Tecplot, EnSight and OpenFOAM. `MESHIOPLUSPLUS_ABI_VERSION` stays 15: every native change is a pure addition ([ABI review](doc/abi_reviews.md)).

- **`tensor_invariants`**, the sixth data operation: von Mises, principal values (ascending), hydrostatic and deviatoric parts of a six- (`xx yy zz xy yz zx`) or nine-component (row-major 3×3) tensor array, at point or cell location. Reaches every surface a data operation does — both CLIs, the pipeline (`"TensorInvariants"`), MCP, C, Fortran, Julia, R and WASM. `.frd`'s `derived=True` (von Mises/principal only, since v15.3.0) is reimplemented on top of it rather than its own duplicated cyclic-Jacobi solver, which moved into a shared `detail/sym3_eigen.hpp`/`.cpp`; its output and CLI/MCP surface are unchanged. See [Tensor invariants](doc/tensor_invariants.md).
- **CalculiX `.frd` binary layout and the `.dat` tabular file.** The binary layout (format flag 2, and flag 3 for node coordinates) was undocumented anywhere public; reverse-engineered from real `ccx` 2.23 output (`*NODE OUTPUT`/`*ELEMENT OUTPUT` decks) rather than assumed, with new `*_bin.frd`/`*_bin.inp` fixtures generated the same way as the existing ASCII ones. Both engines now read it, at either result-value width. `meshioplusplus.frd.read_dat(path)` (Python-only — `.dat` has no geometry, so it produces tables, not a `Mesh`) reads `*NODE PRINT`/`*EL PRINT` tables, keeping the file's own tensor order (`sxx syy szz sxy sxz syz`, different from `.frd`'s `xx yy zz xy yz zx`) rather than remapping it. The cgx shell types 7–10 and a `cgx`-side derived-fields check remain unverifiable (no `cgx` in this environment) and stay a documented `doc/formats/frd.md` limitation, not a roadmap item. See [CalculiX `.frd`](doc/formats/frd.md).
- **SU2 multizone (`NZONE=`/`IZONE=`).** Previously silently corrupted (a second zone's `NPOIN` overwrote the first zone's points); now read and written correctly, zones concatenated unwelded with `cell_data["su2:zone"]` naming each cell's zone. A named (non-numeric) `MARKER_TAG` now survives as a Cell [region](doc/regions.md) (`"<name>"`, or `"zone_<i>/<name>"` for a multizone file) instead of being silently dropped by the C++ reader — a genuine bug fix beyond the roadmap item itself, since the Python reader already kept it. See [SU2](doc/formats/su2.md).
- **Tecplot non-transient multiple zones.** Several static `ZONE`s (no `SOLUTIONTIME`) used to read as only the first, the rest silently discarded; they now concatenate into one step, one cell block and one named region per zone — the same shape a transient file's multi-part steps already used. `VARSHARELIST`, `PASSIVEVARLIST` and `CONNECTIVITYSHAREZONE` are understood on read (resolved recursively, with the shared-point case not duplicating points); the header tokenizer now treats quoted/parenthesized values as atomic, fixing a zone literally titled `"VARLOCATION"`. Both writers emit one zone per cell block instead of requiring (C++) or degrading to (Python) a single type, sharing coordinates/point data through `VARSHARELIST` and marking a cell-data array `PASSIVEVARLIST` on the zones it doesn't cover. **Breaking:** a non-transient multi-zone file that used to silently read as one zone now reads as all of them; see [Tecplot](doc/formats/tecplot.md#multiple-zones). VTKHDF's private composite-carving logic moved into a shared `detail/mesh_carve.hpp`, ready for EnSight's own future multi-part writer.
- **EnSight Gold variable write, and tensor read.** `tensor symm`/`tensor asym` `VARIABLE` entries were parsed into nothing; they now read as point/cell data. EnSight's own `tensor symm` file order (`11 22 33 12 13 23`) differs from meshio++'s convention in its last two components — confirmed empirically against ParaView's `vtkEnSightGoldReader`, not assumed. `write_ensight` now writes a `VARIABLE` section (scalar/vector/tensor symm/tensor asym point and cell data, a single field-data scalar as `constant per case`), single-part and non-transient only — multi-part (via the same `detail/mesh_carve.hpp` SU2/VTKHDF/Tecplot's write side draws on) and transient write remain documented remainders. See [EnSight Gold](doc/formats/ensight.md#variable-write).
- **OpenFOAM binary write.** Binary read already existed; `write_openfoam` gains an `OpenFoamWriteOptions` overload (`binary`, `label_bits`/`scalar_bits`, default 32/64) reachable from Python and C++, and the ASCII/binary switch itself reaches the registry, both CLIs and WASM through the existing `WriteOptions`/`mio_write_ex` machinery. `faces` are written the same length-prefixed-per-face shape the (already-shipped) binary reader expects — not `CompactListList`, confirmed by reading the existing, tested reader rather than assumed. Fixes a related reader gap found along the way: `cellZones`/`faceZones`/`pointZones` had no binary path at all; validated against [foamlib](https://github.com/gerlero/foamlib), an independent OpenFOAM parser, for `points`/`owner` at the default width (no local `checkMesh` to validate the rest against; `OpenFoamWrite.CheckMeshAcceptsOurOutput` stays env-gated). Also fixes `convert`'s `--ascii`/`--binary` verbs raising `TypeError` on `.foam` output. See [OpenFOAM](doc/formats/openfoam.md#binary-write).
- **Tests and hashes.** The four `BASELINE_HASHES` were refreshed for the version only: the old hashes were verified against the new core with the version substituted first.
- **Not verified in this change:** the cgx shell types 7–10 and derived-field values against `cgx` itself (no local install); a real `checkMesh` run over the new binary OpenFOAM output (no local install; `foamlib` and bit-exact read-back cover it instead); the R and Emscripten builds beyond compilation (R and the emsdk are not installed here for a full rebuild-and-test pass in this change; see the per-format doc pages for what was and wasn't exercised).

## v15.4.0 (2026-09-22)

**glTF 2.0 output (`.glb`, `.gltf`) and the `compute_normals` operation it needs. Closes roadmap §1.1 and the §6 `compute_normals` item** (both are removed and §1.2–§1.25 renumbered to §1.1–§1.24). A mesh can now be written straight to the web: three.js, Blender, model viewers and dashboards load the file with no meshio++ in the loop, and a `.pcd` or `.xyz` point cloud gets a web output for free.

- **`compute_normals`** attaches `normals` as point data `(n, 3)` and, on request, cell data (the unit vector area of each cell, Newell's normal for a polygon). With a `split_angle` it duplicates points where the surface creases by more than that angle so that every point carries one normal: the corners around a vertex are grouped into smooth fans by a union-find over triangle corners, joined along proper manifold edges within the angle, and the copies are appended after the original points in the layout `repair` uses for its bowtie copies (cells keep their numbering, `point_data` is gathered by row, a copy joins its source's point regions, `normals:parent_point` is opt-in). Boundary, non-manifold and inconsistently wound edges always cut; the triangles of one polygon never separate. It never reorients — the report carries the input's edge defects — and refuses a volume by name pointing at `extract_surface`. The name `normals` is the one `.pcd` and `.xyz` already use, so a write to either emits the columns. See [Normals](https://loumalouomega.github.io/meshioplusplus/normals.html).
- **One kernel, two consumers.** `detail/surface_normals` holds the corner angle, face normals, weighted vertex-normal accumulation and the corner grouping; the signed-distance pseudonormal table is now built on it, with bit-identical results (the SDF, curvature and surface-distance suites are unchanged). The numpy twin `_normals.py` is byte-identical to the core for both weights and for no split and `0`, `30` and `180` degree splits.
- **Every surface for `compute_normals`.** Python, C (`mio_normals_opts`/`mio_normals_report`, layouts pinned), Fortran (`m%normals`), Julia, R, WASM (`computeNormals`), both CLIs (`normals`, byte-identical files), the settings pipeline (`"Normals"`) and MCP (`normals`).
- **`gltf`** is a write-only format. It exports the surface of the mesh: the skin of volume blocks (a 2-D cell that coincides with a skin facet wins, so a boundary patch keeps its own cells), 2-D cells fanned like `convert_cells(simplexify)`, `line` cells as `LINES`, and `vertex` cells or a cell-less mesh as `POINTS`; one named node per cell region (a cell in several regions goes to the smallest, leftovers to `unassigned`). `.glb` writes the binary container and `.gltf` JSON with a `.bin` beside it, chosen by the suffix or `container=`.
- **Per-vertex normals, fields and colour.** Points are duplicated at creases with the kernel above (`split_angle`, `normal_weight`). Every one-to-four component `point_data` array is exported raw as an underscore-prefixed `float32` attribute (`temperature` → `_TEMPERATURE`, original name in the accessor); `color_by` additionally bakes a `point_data` or `cell_data` array through `viridis`, `coolwarm` or `turbo` into `COLOR_0` and marks the material `KHR_materials_unlit`. `COLOR_0` is linear, so the colormap's sRGB bytes go through a 256-entry inverse-transfer table of `float32` bit patterns (`tools/gen_srgb_linear.py`), not a `pow`, which keeps the two engines' bytes independent of libm.
- **Conventions.** `float32`, right-handed, Y-up. The source's up axis (`up_axis`, `auto` picks Y for a flat mesh and Z otherwise), the unit scale to metres and the bounding-box centre that `recenter` subtracts all live on the root node's rotation, scale and translation, never in the coordinates: the transform is a permutation with signs, so the source positions come back exactly, and a model with coordinates in the millions keeps `float32` precision.
- **Byte-identical engines.** The C++ writer and the Python reference (`gltf/_gltf.py`) write identical `.glb`, `.gltf` and `.bin` bytes across cubes, volumes, a sphere, an open cylinder, regions, ragged polygons, lines and point clouds under six option sets, with a hand-written JSON writer (fixed key order, `%.17g`).
- **Validated.** The output passes the Khronos glTF-Validator (the `gltf-validator` npm package) with zero errors, warnings and infos on every fixture in both containers, and loads in trimesh with the right Y-up bounds. A new `gltf-validator` CI job runs it with `MESHIOPLUSPLUS_REQUIRE_GLTF_VALIDATOR=1`, so a missing validator fails instead of skipping.
- **Every surface for `gltf`.** Registry (`.glb`, `.gltf`, writer only) so C, Fortran, Julia, R and WASM write the defaults through `mio_write`/`writeMesh`; `convert` in both CLIs accepts `--color-by` and friends for glTF plus `--split-angle` and `--up-axis`; MCP `export_gltf`. The options themselves stay off the flat ABI, recorded in the roadmap.
- **Tests.** `test_gltf.py` decodes the containers with `struct` and `json` (header, chunk alignment and padding, accessor bounds, `min`/`max`, winding against the stored normals, the root transform, regions, fields, colour, errors, CLI), pins the two engines against each other, and runs the validator and trimesh; `test_compute_normals.py` and `test_compute_normals.cpp` cover the operation; gtests for the writer, the registry rows and the layout pins; C API, Fortran, Julia, R and WASM checks for the new entry points.
- **Example.** `example/python/08_gltf_export.ipynb`, executed with outputs: smooth versus split normals, a coloured `.glb` export and its round trip through an independent loader.
- **Tests and hashes.** The four `BASELINE_HASHES` were refreshed for the version only: the old hashes were verified against the new core with the version substituted first.
- **Not verified in this change:** loading in three.js and in Blender, with the orientation and colours checked by eye (neither is available in the environment); the R and Emscripten builds (R and the emsdk are not installed here — the R and WASM code is compiled by reading and syntax-checked only).

## v15.3.0 (2026-09-21)

**CalculiX result files: `.frd`, read by both engines. Closes roadmap §1.1** (the item is removed and §1.2–§1.26 renumbered to §1.1–§1.25). The open-source chain mesh → `.inp` → `ccx` → `.frd` → `.vtu`/VTKHDF now runs without a proprietary tool. The format is **read-only**. `MESHIOPLUSPLUS_ABI_VERSION` stays 15: `formats/frd.hpp` is a new header ([ABI review](doc/abi_reviews.md)).

- **`frd`** reads the ASCII `.frd` of `ccx` in both the long (`I10` ids, what `ccx` writes) and the short (`I5` ids) layout; the binary layout (flag 2) is refused by name. Records are sliced by column, because `E12.5` values have no separator (`7-1.18144E-06` is a node id and a value). All twelve cgx element types map to `hexahedron`, `wedge`, `tetra`, `hexahedron20`, `wedge15`, `tetra10`, `triangle`, `triangle6`, `quad`, `quad8`, `line` and `line3`; the element group and material are the integer cell data `frd:group` and `frd:material`.
- **The mesh is what `ccx` wrote, not the `.inp` mesh.** `ccx` expands shells and beams into solids (an `S8` returns as a `hexahedron20`, a `B31` as a `hexahedron`), and never writes the cgx shell types 7 to 10. Documented and demonstrated rather than hidden.
- **Node orders, confirmed against `ccx` 2.23.** The he20 and pe15 mid-edge groups and the be3 mid node sit elsewhere than in Abaqus order and are permuted on read: `hexahedron20` `0..11, 16..19, 12..15`, `wedge15` `0..8, 12, 13, 14, 9, 10, 11`, `line3` `0, 2, 1`. The tests check it geometrically (every mid-node lies on the middle of its edge) on the `ccx` output of each single-element deck. A quadratic **beam** is the exception: `ccx` expands it into a `hexahedron20` whose "mid-edge" nodes are the far end of the beam, which is its own convention.
- **Increments are steps.** Every `100C` increment is a step of the sequence engine (`time_step`, negative counts from the end, out of range is an error; `read_metadata(...).time_values`; `read_sequence`; a `{step}` output fans out, a single file for a multi-step input is refused by name). The step's value, step counter and analysis type are `field_data["meshio:time"]`, `frd:step` and `frd:analysis` (0 static, 1 time, 2 frequency, 3 buckling, 4 user). For a frequency step the value is the **frequency** in cycles per time unit (as `ccx` prints it) and modes that share a frequency stay distinct steps, because increments are keyed by their frame id as well as their value.
- **Results.** Each `-4` block becomes a point data array under the file's own name (`DISP`, `STRESS`, `TOSTRAIN`, `NDTEMP`, `FLUX`, `ERROR`, ...): one component `(n,)`, three `(n, 3)`, six `(n, 6)`, more `(n, c)`; nodes a block omits are NaN; components marked as calculated (`ALL` of `DISP`) are dropped; results continue over `-2` lines. Symmetric tensors keep the file's order `xx yy zz xy yz zx` (also VTK's), now written down as the convention for a six-component symmetric tensor in [the data model](doc/mesh_data_model.md#symmetric-tensors). `points_only` and `arrays` parse only the blocks asked for.
- **Opt-in derived fields.** `meshioplusplus.frd.read(..., derived=True)` (`_core.frd_read`, C++ `read_frd(path, options, FrdReadOptions{})`) adds `<NAME>_mises` and `<NAME>_principal` (ascending min, mid, max) beside each `STRESS`, `TOSTRAIN`, `MESTRAIN` and `ZZSTR`, with the formula of ccx2paraview. It is deliberately an argument of the format's own reader and **not** a member of `ReadOptions` (whose layout would have been a Tier A break), so the generic `read`, both CLIs, MCP and the flat bindings do not carry it; roadmap §1.2 records the follow-up (a tensor-invariants data operation).
- **Every surface.** Registry (reader only, `.frd`, metadata reader, sequence predicate), `sniff` (a lone `    1C` record followed by the `1U` header or the `2C` node block, in both sniffers), the Python and native CLIs (`convert`, `info`, `--time-step`, `{step}` fan-out) and the C, Fortran, Julia, R and WASM bindings pick it up from the registry with no per-binding code. No MCP signature or report field moved; `convert`'s `time_step` reaches it.
- **Tests.** Python `test_frd.py` (182 cases plus 18 skips, each run through both engines and the C++ reader called directly so its own errors are not hidden by the fallback): every element type and permutation, glued negatives, `-2` continuation, the short layout, steps and NaN fill, frequency modes with equal values, selective reads, derived fields against NumPy, the engines against each other on every fixture and step, sniffing, read-only registration and the error paths; C++ `test_frd.cpp` (14, building the files from fixed-column records) plus rows in `test_sniff.cpp`, `test_registry.cpp` and `test_abi_layout.cpp` (`FrdReadOptions`); one MCP test. Fixtures in `tests/python/meshes/frd/` are `ccx` 2.23 output for the decks next to them (`tools/gen_frd_fixtures.py` regenerates them; committed as plain text, like the LS-DYNA decks), plus one hand-written short-format file for the cgx types `ccx` never writes. Nothing of the (GPL) CalculiX test suite is used.
- **Verification against the solver.** Displacements match the `*NODE PRINT` of `ccx` to its print precision on every unexpanded element type; the 12 × 2 × 2 cantilever's tip deflection is 0.9975 of Euler-Bernoulli beam theory in both load steps; its first bending frequency is 23158 Hz against 23283 Hz from beam theory; every single-element fixture converts with the native CLI to a `.vtu` that ParaView loads with the expected VTK cell types and arrays (its cell-size filter is not defined for a quadratic wedge and reports 1/12 of the volume of a directly written unit `hexahedron20`, so it says nothing about node order), and the Python and native CLIs fan a multi-step file out to identical files.
- **Example.** `example/python/07_calculix_frd.ipynb`, executed with outputs: the deformed cantilever coloured by von Mises with a beam-theory check, its first four modes, `read_sequence` → `{step}` files, the shell/beam expansion, the node permutations checked geometrically, the short layout and derived fields against NumPy. The C++ notebooks are not mirrored (no xeus-cpp kernel here).
- **Tests and hashes.** The four `BASELINE_HASHES` were refreshed for the version only: the old hashes were verified against the new core with the version substituted first.
- **Not verified in this change:** derived fields and displacements against **`cgx`** itself (it is not installed and cannot be built without root, so the roadmap's "done when" names it and it stays open; the von Mises formula is ccx2paraview's and is tested against NumPy); the cgx shell types 7 to 10 and the short layout against a real cgx file (a hand-written file only); the C++ tests under the NATIVE and KRATOS backends (`frd.cpp` compiles cleanly under both with `-fsyntax-only`, the tests were run under MESHIO only); WASM (no Emscripten: `tests/wasm/smoke.mjs` is extended and syntax-checked, and the file it builds was read by the C++ reader through the Python binding); R, Julia and the Fortran module; binary `.frd` and the `.dat` file, which are not read.

## v15.2.0 (2026-09-21)

**LS-DYNA keyword input decks: `.k`, `.key` and `.dyn`, read and written by both engines. Closes roadmap §1.1** (the item is removed and §1.2–§1.27 renumbered to §1.1–§1.26). The mesh of a crash model — `*NODE`, the `*ELEMENT_*` cards, `*PART` and `*SET_*`, across `*INCLUDE`d files — converts to VTU/VTKHDF with its parts intact. `MESHIOPLUSPLUS_ABI_VERSION` stays 15: `formats/lsdyna.hpp` and `detail/keyword_card.hpp` are new headers ([ABI review](doc/abi_reviews.md)).

- **`lsdyna`** reads `*NODE`, `*ELEMENT_SOLID` (the one-card layout and the two-line layout with up to ten nodes), `_SHELL` (with the `_THICKNESS`, `_BETA`, `_MCID` and `_OFFSET` variants), `_TSHELL`, `_BEAM`, `_DISCRETE` and `_MASS`. Everything is collected across all files before ids are resolved, so a set may name elements defined in a later `*INCLUDE`. Includes are looked up next to the including file, then in each `*INCLUDE_PATH`, then in the working directory; a missing one is a warning (decks routinely include material files that are not shipped with the mesh), nesting stops at eight levels, and `*INCLUDE_TRANSFORM` offsets are not applied.
- **Parts and sets are regions.** A `*PART` is a cell region (title as name, `pid` as tag, highest element dimension as `dim`). `*SET_NODE` is a point region; `*SET_SOLID`, `_SHELL`, `_TSHELL`, `_BEAM`, `_DISCRETE` and `*SET_PART` are cell regions; `*SET_SEGMENT` is a side region whose segments are matched to a (cell, facet) by their corner nodes. `_LIST`, `_GENERATE`, `_LIST_GENERATE` and `_TITLE` are all read. Element ids are unique per family, as in LS-DYNA.
- **Four card formats, mixed freely and detected per card.** Standard fixed fields; **long** (`*KEYWORD LONG=Y`, or a `+` suffix) where every field is 20 columns; **i10** (`*KEYWORD I10=Y`, or `%`) where only the 8-column integers widen to 10, a different mechanism; and comma-separated free format. Fortran real spellings (`1.5D+01`, `2.5-1`) are accepted. The tokenizer is the new `detail/keyword_card.hpp` — the first piece of the shared card infrastructure in the roadmap — with a Python twin in `lsdyna/_cards.py`.
- **Degenerate elements.** Tetra, pyramid and wedge have no card in LS-DYNA: they are hexahedra with repeated nodes, collapsed on read and expanded on write. Both wedge spellings (`n1 n2 n3 n3 n5 n6 n7 n7`, which most preprocessors write, and `n1 n2 n3 n4 n5 n5 n6 n6`) and both tetra spellings are read, with the node order chosen so the cell has a positive volume in meshio++'s convention (asserted numerically); a triangular shell is a quad with `n4 == n3` or a blank `n4`. The writer emits the spellings the roadmap names.
- **Blocks follow the deck.** One block per (element keyword, cell type), extending the previous block when its type matches, so a deck's sections survive a round trip.
- **The writer** emits `*NODE`, one `*ELEMENT_*` keyword per block, `*PART`, `*SET_*_LIST_TITLE` and `*END`, with placeholder section and material ids (`1`, `1`). A cell region with a dimension becomes a `*PART` (its tag the `pid`, renumbered when repeated); every other region becomes a set, so no region is lost; cells no part claims form one part per block. Coordinates use the shortest 16-column scientific spelling that round-trips, else 11 significant digits (10 for a negative number). A mesh whose ids overflow an 8-column field is a `WriteError` rather than a silent switch of format. **Both engines write byte-identical files** and read identically, pinned against each other on every fixture and mode.
- **Not parsed, by design.** Materials, sections, contacts, loads, `*DEFINE_*`, `*PARAMETER` substitution (a mesh card using `&name` is skipped with one warning), the `*ELEMENT_SOLID_TET4TOTET10` / `_H8TOH20` directives (warn), PGP-encrypted blocks (skipped with a warning) and 20-node hexahedra. Shell nodes past `n4`, beam orientation nodes and element masses are dropped. See [the format page](doc/formats/lsdyna.md).
- **Every surface.** Registry (reader, writer, `.k`/`.key`/`.dyn`), `sniff` (an opening `*KEYWORD`, after any `$` comments, checked before the Abaqus rule in both sniffers), the Python and native CLIs, and the C, Fortran, Julia, R and WASM bindings all pick the format up from the registry with no per-binding code. No MCP change: the tools are registry-driven and no signature or report field moved.
- **Tests.** Python `test_lsdyna.py` (149 cases, each parametrised over both engines where it reads or writes): the card formats, the collapse rules and wedge orientation, includes and their failure modes, every set flavour, the warnings and errors, writer part assignment, and the multi-file fixture in `tests/python/meshes/lsdyna/` (hand-written, committed as plain text rather than through LFS); C++ `test_lsdyna.cpp` (18: 14 `Lsdyna`, 4 `KeywordCard`), and `lsdyna` joins the region round-trip matrix, the provenance-tag parity test, both sniff tests and `test_registry.cpp`. `test_abi_layout.cpp` pins `CardField` and `CardMode`.
- **Cross-checked against an independent reader.** Six public decks (`bird.k`, `birdball.k`, `bracket.k`, `wheel.k`, `ex_13_thick_shell_elform_2.k` and the mixed hex/shell `EXP_SC_JOINT_SCREW.key`, from the MIT-licensed `lsdyna-mesh-reader` package) read with the same node count, the same element count per type and the same population per part id as that reader, in both engines, and a `*SET_NODE_LIST_GENERATE` over one of them kept exactly the 313 of its 376 generated ids that the deck defines. The decks are not committed.
- **Tests and hashes.** The four `BASELINE_HASHES` were refreshed for the version only: the old hashes were verified against the new core with the version substituted first.
- **Example.** `example/python/06_lsdyna_decks.ipynb`, executed with outputs: the multi-file deck rendered one colour per part, the collapse with a positive-volume check, the four card formats reading to one mesh, sets, and a write → read round trip. The C++ notebooks are not mirrored (no xeus-cpp kernel here).
- **Verified beyond Python.** The C API was compiled and driven from a C program over the fixture deck (`mio_read` inferring the format from `.k`, `mio_regions_count`, `mio_write`, a re-read, and a `.vtu` write), and the native CLI was built and its `info` and `convert` (`.k` → `.vtu` → `.k`) run over it. The two new C++ sources also compile cleanly under the NATIVE and KRATOS backends (`-fsyntax-only`).
- **Not verified in this change:** a written deck loading in LS-PrePost or running in LS-DYNA (neither is installed — the roadmap's "done when" names this, so treat it as open), the C++ tests under the NATIVE and KRATOS backends (compiled, not run), WASM (no Emscripten; `tests/wasm/smoke.mjs` extended and syntax-checked but not run), R and Julia (no toolchain), and the Fortran module.

## v15.1.0 (2026-09-21)

**Point-cloud files: `.pcd` (PCL v0.7) and headerless `.xyz` in both engines. Closes roadmap §1.1** (the item is removed and §1.2–§1.28 renumbered to §1.1–§1.27). `.pcd` → `subsample` → `proximity-graph` → `.vtu`/`.pcd` now runs from the CLI with no intermediate format. `MESHIOPLUSPLUS_ABI_VERSION` stays 15: `formats/pcd.hpp` and `formats/xyz.hpp` are new headers ([ABI review](doc/abi_reviews.md)).

- **`pcd`** reads and writes `DATA ascii`, `binary` and `binary_compressed` (two `uint32` sizes, then an LZF stream over the struct-of-arrays layout). The LZF codec is an independent implementation of the liblzf stream format in C++ and Python, not a copy of liblzf. Point data is keyed by field name: `normal_x/y/z` → `"normals"`; PCL's `rgb`/`rgba`, a `uint32` held in a `float32` slot, are unpacked by **bit-cast, never by value** → `"rgb"`/`"rgba"` `uint8`; `COUNT > 1` gives `(n, count)`; `_` padding is skipped. Organised clouds (`HEIGHT > 1`) are kept whole with `pcd:width`/`pcd:height` in `field_data` (`drop_invalid=True` drops the NaN returns and the organisation); a non-identity `VIEWPOINT` goes to `pcd:viewpoint` and is never applied. The writer defaults to float32 coordinates, the only precision PCL's typed loaders accept, and records a lossy narrowing in the provenance; `point_dtype="float64"` or `"keep"` override it.
- **`xyz`** (`.xyz`, `.xyzn`, `.xyzrgb`, `.asc`, `.pts`, `.txt`) resolves its columns from an explicit `columns=` list, a header comment naming them (which the writer emits, so files round-trip), the column count and extension, or the value ranges (six columns: unit vectors are normals even when they are all integral, byte-valued integers or unit floats are colours). Anything still ambiguous is an error that asks for `columns=`, never a guess. Whitespace, comma and semicolon delimiters, `#` and `//` comments, and the `.pts` leading count (validated) are accepted. **Chemistry XYZ** (atom count, comment, `element x y z`) shares the extension and is refused by name. **This claims `.txt` and `.asc`**, which no format read before.
- **The mesh is the points plus one `vertex` block**, exactly what `subsample_points` emits. `subsample_points` no longer reports a `vertex` block as "dropped cells": a vertex-only mesh is the point-cloud form, so reading a `.pcd` and subsampling it warns about nothing.
- **Both engines write identical bytes** (the LZF-compressed files included) and read identically, pinned against each other on every fixture and mode. Python: `test_pcd.py` (129, including the CLI verbs), `test_xyz.py` (50); C++: `test_point_cloud_formats.cpp` (25, including a hand-encoded LZF stream, independent of this library's own encoder), and `pcd` and `xyz` join the provenance-tag parity table.
- **Real PCL files.** `tests/python/meshes/pcd/` carries four files from PCL's own test corpus (BSD-3, `LICENSE.PCL` beside them, listed in `CITATION.cff`, `*.pcd` added to the LFS rules), one per `DATA` mode: an ASCII cloud with normals, a binary organised cloud with a `uint32` `rgb`, and two `binary_compressed` clouds (a float-slot `rgb` with a non-identity viewpoint, and an `rgba`). The `binary_compressed` files decode to exactly the declared size and the decoded colours are checked against a hand-written decoder.
- **Every surface.** Registry (readers, writers, extensions), `sniff` (`# .PCD` or a leading `VERSION`/`FIELDS`; headerless XYZ is not sniffable and is not claimed), `WriteOptions` (`pcd` ascii/binary; `xyz` float format), the Python and native CLIs, and the C, Fortran, Julia, R and WASM bindings all pick the formats up from the registry with no per-binding code. The `ascii`, `binary`, `compress` and `decompress` verbs handle `pcd` in both CLIs (rewriting in place at the mesh's own precision; the two CLIs write byte-identical files), and the MCP `convert` tool takes `mode` for it and `compression: lzf` for `binary_compressed`. `binary_compressed` cannot be *written* from the flat bindings yet; the roadmap records the appended-enumerator fix.
- **Tests and hashes.** The four `BASELINE_HASHES` were refreshed for the version only: the old hashes were verified against the v15.0.0 core first, then recomputed with the version substituted.
- **Verified beyond Python.** The C API was compiled and driven from a C program over all four PCL files (sniff, `mio_read`, `mio_write_ex` ascii and binary, the `xyz` write and its `float_format`, and the refusals of a float format or a VTK block codec for `pcd`), and the native CLI was built and its `ascii`, `binary`, `compress` and `decompress` verbs run over the same files and a float64 cloud: they write files byte-identical to the Python CLI's and keep the coordinate precision. All new C++ was syntax-checked under the MESHIO, NATIVE and KRATOS mesh backends and with clang at `-Wall -Wextra -Wshadow -Wconversion`.
- **Not verified in this change:** WASM (no Emscripten; `tests/wasm/smoke.mjs` extended but not run), R and Julia (no toolchain), Fortran (the flat path is the C API driven above, not the Fortran module), PCL itself reading a file this library wrote (PCL is not installed; the fixtures are PCL-written files read by this library, not the reverse), and the C++ tests under the NATIVE and KRATOS backends (compiled there, run under MESHIO only). The C++ tests were linked against the built core objects rather than through a fresh `build/configure.sh --tests` tree, so CI's NATIVE and KRATOS legs are their first real run.

## v15.0.0 (2026-09-20)

**The ParaView index formats — `.pvtu` / `.pvtp` (a partitioned dataset) and `.pvd` (a time-indexed collection) — in both engines, with `vtkGhostType` halos, a `ghosts` switch on every read surface, and `<FieldData>` on `.vtu` / `.vtp`. Closes roadmap §1.1** (the item is removed and §1.2–§1.29 renumbered to §1.1–§1.28). `MESHIOPLUSPLUS_ABI_VERSION` goes 14 → 15. There was no v14.1.0: the first cut of this work was folded into this release before it shipped.

- **Breaking (ABI):** `ReadOptions` gained `mGhosts` (`GhostPolicy`, `Keep` by default), the keep-or-drop switch for the ghost cells of a partitioned file. **`sizeof(ReadOptions)` did not move**: the member sits in the 7 bytes of tail padding after `mPieceSet` (offset 65, pinned in `test_abi_layout.cpp`), so `PipelineInput`, `Pipeline`, `SequenceInput` and `SequencePipeline` are unchanged too — the roadmap's "+16 each" prediction was wrong. It is a Tier A change all the same, because a consumer compiled against v14 headers leaves that byte indeterminate and a v15 library reads it as the policy, so the number is bumped and `doc/abi.md` / `abi_version.hpp` say the size did not move. Rebuild anything compiled against the installed C++ headers. The C API is unaffected: `mio_read_opts.drop_ghosts` took one of the two remaining `reserved` slots (`reserved[1]` left), the struct is still 80 bytes, and `c_api.cpp` gained the `static_assert(sizeof(mio_read_opts) == 80)` it lacked.
- **New formats `pvtu`, `pvtp` and `pvd`**, each a Python reference plus a C++ core the shim prefers and falls back from through `core_declined`, registered in every registry table and sniffed. All write a *sibling directory* named after the index's stem holding zero-padded pieces (`case/case_0000.vtu`), with relative, XML-escaped `Source=`/`file=` paths always read against the index's own directory.
- **`.pvtu` / `.pvtp`** declare `PPoints`, `PPointData` and `PCellData` and name one piece per *part* (`.vtm` carves by cell *block*). `write` carves by the integer `cell_data` array `part_key` (default `partition:part`), else writes one piece; `write_pieces` takes already-carved pieces, so `partition`'s output goes straight in, halo layers included. **Every piece must declare identical arrays, checked before anything is created**, so a refusal leaves no directory and no index; the error names the array, the piece and both declarations. A part that owns no cells is still written. `read` merges without welding, one `piece_<i>` cell region per piece, or `piece=k` keeps one through the `ReadOptions::mPiece` VTKHDF introduced. `partition` → `.pvtu` → read-merge → `clean(weld=True)` returns the original mesh up to point ordering, in tests for both engines.
- **Ghosts.** A `partition:ghost` array becomes `vtkGhostType` on cells (`DUPLICATECELL` for any layer) and on points (`DUPLICATEPOINT` when no owned cell of the piece uses the point) and sets the index's `GhostLevel`; `partition:ghost` is kept too, since `vtkGhostType` collapses layer 2 onto layer 1. A `vtkGhostType` the caller supplied is passed through (`REFINEDCELL`/`HIDDENCELL` survive), and the caller's meshes are never mutated. It is always written as **`UInt8`** whatever dtype the mesh holds it in — the NATIVE and KRATOS mesh backends widen every integer array to `Int64` on the way in, and ParaView ignores a `vtkGhostType` that is not `UInt8`, so the ghost cells would have silently stopped being ghosts (`.vtu`, `.vtp` and the `.pvtu` declarations alike, both engines). Reading **keeps** ghost cells by default; **`ghosts="drop"` now reaches every surface** — `meshioplusplus.read(..., ghosts=)`, `convert --drop-ghosts` on both CLIs, the MCP `stats`/`convert` tools (`ghosts`), `read_sequence`, `ReadOptions::mGhosts` (C++), `mio_read_opts.drop_ghosts` (C), `drop_ghosts=` (Fortran, Julia, R) and `dropGhosts` (WASM `readMeshSelective`). It is honoured by `pvtu`/`pvtp`/`pvd` and ignored by every other reader, like `lenient` and unlike `time_step`/`piece`: a file with no halo is already the answer it asks for. It is not an `Input.Options` key of a pipeline document, any more than `piece` is.
- **`.pvd`** has two axes: `timestep=` selects the step (`time_step=`) and `part=` the piece within it (`piece=`). Steps are the distinct `timestep` values ascending; parts of a step merge with one region per entry (`name=`, else `group/part_<p>`, else `part_<p>`); a step that is a single entry reads as that file does, so `.pvd` → `.pvtu` → `.vtu` nests with no special case. The chosen step's time is attached as `field_data["meshio:time"]`, and `read_pvd_metadata` reports every step's time from the index alone. `.pvd` joins both sequence predicates, so `read_sequence`, `TimeSeries` and `write_sequence(path.pvd, steps)` work, through `PvdSeriesWriter` (C++) and `pvd.SeriesWriter` (Python), which stream one mesh at a time and **rewrite the index after every step** so a killed run still opens. **An entry with no `timestep=` takes its time from the file it names** — VTK's "time in field data" `TimeValue`, else `meshio:time`, else 0 — resolved for every entry in both the read and the metadata so they cannot group steps differently.
- **`<FieldData>` on `.vtu` and `.vtp`, in both engines.** Mesh-level `field_data` is written on the grid before the `<Piece>` with the `NumberOfTuples` VTK requires and read from the grid or a piece (the piece overriding); a non-numeric array (`type="String"`) is skipped with a warning instead of failing a read that used to succeed by ignoring the whole section. **This changes behaviour**: the C++ readers used to drop `<FieldData>` silently (so even the Python `vtu.read` lost it, its fast path succeeding), and both writers used to drop `field_data`; now it round-trips, `read_metadata` names it, and `write_sequence("out_{step}.vtu", ...)` records each step's time in every file. Guarded on `NumFieldData() != 0`, so a mesh without field data writes the bytes it always did: the four pinned `BASELINE_HASHES` reproduced exactly with only the version substituted before being refreshed. The Python VTU writer no longer mutates the caller's `field_data` while byte-swapping it. **Field data is dataset-global, so the index readers take the union across pieces** (first piece wins a name) instead of `merge()`'s `0:name`, `1:name` renaming — also for `.vtm`.
- **The partition command writes an index.** `partition IN OUT.pvtu -n 4 --ghost-layers 1` (both CLIs; also `-o pvtu`) with no `{part}` token writes one index over every part, the only route that keeps a halo. The MCP `partition` tool does the same for a `name_template` ending in `.pvtu`/`.pvtp`, and now **refuses any other template without `{part}`**, which used to overwrite every part with the last. `doc/cli.md` no longer says `--ghost-layers` is "reserved, only 0".
- **Sniffing fixed.** The `vtu`/`vtp` checks were bare substrings and `PUnstructuredGrid` contains `UnstructuredGrid`; the indexes are now matched on the quoted `type=` value first, and a parallel image/structured/rectilinear index is refused by name instead of being mistaken for its serial twin (C++ and Python twins agree).
- **Verified against real ParaView.** `tests/python/test_pvd_paraview.py` drives **ParaView 6.1.1** through `pvpython` (skipped where absent) over the files we write: a `.pvd` of `.vtu` reports the written time steps; a `.pvd` of ghosted `.pvtu` reports them too, with the halo cells (70 of 142 in the fixture) recognised as ParaView's ghost cells flag for flag, and the dataset's field data (`TimeValue`) per step; a `.vtu`/`.vtp` carrying `TimeValue` is a timed dataset in ParaView (`TimestepValues == [0.25]`), which is VTK's own convention. PyVista's independent pure-Python `PVDReader` is checked alongside where installed. `test_pvtu_vtk.py` (VTK 9.7) reads every file either engine writes through `vtkXMLPUnstructuredGridReader`/`vtkXMLPPolyDataReader` and both engines read what VTK's parallel writers write. One deliberate divergence: ParaView treats a `.pvd` entry with no `timestep=` as time 0 and does not consult the file, so `write_sequence` always writes `timestep=`.
- **Verified beyond Python.** The C API was compiled and driven from a C program (`mio_read_ex` with `drop_ghosts = 1`: 142 → 72 cells, `sizeof(mio_read_opts) == 80`), the Fortran module was compiled and the **full Fortran API suite passes** with a new `drop_ghosts` block, and the Julia `_CReadOpts` mirror was checked to be 80 bytes and the files to parse. All new C++ was syntax-checked under the NATIVE and KRATOS mesh backends.
- **Tests.** Python: `test_pvtu.py` (116, both engines, including the generic-read, CLI and MCP ghost switch and the field-data union), `test_pvd.py` (62), `test_vtk_field_data.py` (27), `test_pvtu_vtk.py` (16, needs `vtk`), `test_pvd_paraview.py` (7, needs `pvpython`/PyVista); the core-engine cases run under strict-core, and sabotaging the core was confirmed to fail them. C++: `test_pvtu.cpp` (27), `test_pvd.cpp` (21), `test_vtk_field_data.cpp` (8), plus the ABI layout offset pins for `ReadOptions::mPieceSet`/`mGhosts`; `test_sequence.cpp`'s tripwire that asserted VTU carries no `meshio:time` now asserts the opposite. `PvdSeriesWriter` is pinned in the layout snapshot.
- **Not verified in this change:** the **R** binding (no R toolchain: `mesh.c`, `init.c` arity 9 → 10 and the roxygen block were edited by reading; the committed `man/mio_read.Rd` was already behind its roxygen source and is not regenerated), the **WASM** binding (no Emscripten; `js_bindings.cpp`, `index.mjs` and `index.d.ts` edited and `node --check` passes, `tests/wasm/smoke.mjs` was not extended), the **Julia** runtime tests, and the C++ tests under the NATIVE and KRATOS backends (compiled there, run under MESHIO only). The C++ tests were linked against the already-built core objects rather than through a fresh `build/configure.sh --tests` tree.
- **The Python `.vtu` reader reads a cell-free piece.** A partition with an empty part writes a piece with no cells, and the Python reference reader crashed on it (`need at least one array to concatenate` for a compressed empty array, `IndexError` for empty connectivity). Only a build whose core lacks zlib reaches it, since the core reads such pieces itself — which is how the Windows wheels found it. The core-engine cases of the new Python tests skip on a build without zlib, as they write the default zlib codec under strict-core.
- **Known limits.** `field_data` values that are not numeric arrays are not written (Python warns); a rank above 2 is flattened to `(tuples, components)` and a rank-0 scalar reads back as a length-1 array. The Python reference carves, derives ghost arrays and drops ghosts for rectangular blocks only and raises `NotImplementedError` naming the core for polygon/polyhedron blocks, which the C++ core handles. `seq_time_from_mesh` does not learn `TimeValue`, so a plain `out_*.vtu` glob still takes the filename's number for a VTK-written series.

## v14.0.0 (2026-09-20)

**VTKHDF (`.vtkhdf`, `.hdf`): Kitware's HDF5 VTK format, with partitions, polyhedra, composites and transient `Steps` in one file.** Closes roadmap §1.1 and the HDF5 schema-helper bullet of §1.29. `MESHIOPLUSPLUS_ABI_VERSION` goes 13 → 14.

- **New format `vtkhdf`** in both engines (h5py Python reference and the C++ core, which the shim prefers and falls back from through `core_declined`). `Type` values `UnstructuredGrid`, `PolyData`, `PartitionedDataSetCollection` and `MultiBlockDataSet`; polyhedra through the `FaceConnectivity`/`PolyhedronToFaces` arrays (mixed polyhedron + ordinary-cell meshes included); multi-partition files; transient files through `Steps`. `ImageData`, `OverlappingAMR`, `HyperTreeGrid`, `Table`, `RectilinearGrid` and `StructuredGrid` are out of scope and refused by name.
- **The layout was measured against VTK 9.7's `vtkHDFWriter`/`vtkHDFReader`, not taken from the prose spec**, and `tests/python/test_vtkhdf_vtk.py` (skipped without `vtk`) feeds every file either engine writes back through `vtkHDFReader`. It found several details the prose does not say: `Offsets` carries `NumberOfCells + 1` boundaries with piece-local connectivity; PolyData topology is category-major but its `CellData` is piece-major; a polyhedron's `Connectivity` row is its sorted unique node set; composite blocks are flat groups under `VTKHDF` with soft-linked `Assembly` entries and mandatory creation-order tracking; and a composite with a root `FieldData` makes the reader fail, so field data is replicated onto every block.
- **`Type` is a fixed-length ASCII attribute** (the new `h5::write_attr_string_fixed`), never h5py's variable-length default, which some readers reject. `Version` is the oldest that covers the content (2.0 plain, 2.1 composite, 2.5 polyhedra), and `version=` can pin it lower or refuses with the feature that does not fit. The reader accepts major 1 and 2, dispatches on the arrays present rather than on the minor, and warns above the newest minor it knows (8).
- **Partitions merge into one mesh with one `Cell` region per piece** (`piece_0`, `piece_1`, …), or one piece is selected with the new `piece=` switch on every reader surface (Python, C, Fortran, Julia, R, WASM, both CLIs as `--piece`, MCP). Negative values count from the end; out of range names the piece count. Polyhedra are bucketed per piece so `piece=k` always equals region `k` of the merged read.
- **Transient**: `read(..., time_step=k)`, `read_metadata()["time_values"]`, `read_sequence`, and fan-in through `write_sequence` to a single `.vtkhdf` (so C, Fortran, Julia, R, WASM and the native CLI write transient files with no per-binding code). `VtkhdfTimeSeriesWriter` (C++) and `TimeSeriesWriter`/`TimeSeriesReader` (Python) stream steps into chunked, appendable datasets: nothing is buffered, so a killed run leaves a file ParaView opens covering every finished step. The array-name set is fixed at the first step. A composite cannot be transient (`vtkHDFWriter` itself segfaults on that combination), and reading one is refused.
- **Breaking (ABI):** `ReadOptions` gained `mPiece`/`mPieceSet` (56 → 72 bytes), an ABI Tier A change, hence 13 → 14. Rebuild anything compiled against the installed C++ headers. The C API is unaffected: `mio_read_opts` took two of its `reserved` slots, so its size and offsets are unchanged.
- **Fixed: `.vtu` polyhedral runs that mix node counts read back with misaligned `cell_data`** (C++) or crashed (`ValueError`, Python). A run is bucketed into one block per node count, but `cell_data` was sliced contiguously per bucket (C++) or in ascending size order (Python). Both now gather each bucket's cells from their file rows. Regression tests in `test_vtu.cpp` and `test_vtu.py`.
- **`hdf5_util` grows the schema helpers** the format needed (fixed-string and integer-array attributes, hyperslab reads, chunked appendable datasets, creation-order groups, soft links), all additive.
- **Not verified in this change:** the R and WASM bindings and the Fortran/Julia runtimes were edited and their surface checked by reading, but not built or run (no R, Emscripten, Julia or Fortran toolchain was exercised). The C API and both C++ and Python paths are built and tested.
- **Fixtures**: none committed. `test_vtkhdf_vtk.py` generates its files with VTK's BSD-3 writer at run time, so no GPL bytes enter this MIT repository.

## v13.0.0 (2026-09-19)

**Closes roadmap section 1 "Correctness debts" in full, and reorders what is left.** The last two open items land: C++ streams are pinned to the classic locale, and the per-format Python shims stop swallowing why the fast path declined. The section is removed and **Format reach moves to first place**, so the roadmap is now §1 Format reach, §2 Quality, §3 Performance, §4 Core parity, §5 Operations, §6 Ecosystem reach, §7 Long run; every cross-reference and the roadmap map diagram follow, and three references that were already stale are corrected (a "polyhedron rescan" that shipped in v12.1.0, a "MED and gmsh ordering pair" sequencing note for the same release, and three `#_4-performance` links). Major bump because this closes a top-level roadmap section, as v12.0.0 did, and because of the **Breaking:** fallback change below. `MESHIOPLUSPLUS_ABI_VERSION` stays 13 (see `doc/abi_reviews.md`); `detail/classic_stream.hpp` is a brand-new header carrying only `inline` functions.

- **Breaking:** the per-format Python shims no longer fall back on *any* exception. All 91 handlers (44 `read`, 47 `write`, in 47 format packages) now ask the new `meshioplusplus._fallback.core_declined`: a `ReadError`/`WriteError` falls back quietly (logged at `DEBUG`), a `TypeError`, `MemoryError` or `RecursionError` out of the compiled core now **propagates**, and any other exception (a `ValueError` from a `std::stoll`, a `RuntimeError`, an `AttributeError`) still falls back, but is logged at `WARNING` naming the format, the path and the cause. The visible consequence is a stale editable build: a shim calling the core with an argument the compiled extension predates used to run silently on the slower Python twin, and now raises. The existing re-raise conditions (a non-default `time_step` on Gmsh, XDMF, MED, EnSight, Tecplot and CGNS; an OpenFOAM `region` or multi-region case) are unchanged.
- **`MESHIOPLUSPLUS_STRICT_CORE=1`** (also `on`, `true`, `yes`) turns every fallback into a re-raise, so a test or a CI leg can prove the compiled core really handled a file. Run over the suite (3,882 tests) it fails 111, and every one traces to a decline the roadmap already knows about or to one it now records: Gmsh format 4.0 files (34), lzma-compressed VTU/VTP (29), ragged or polyhedron VTU writes (9), legacy VTK structured grids and `SCALARS` sections (9), MED fields with units or timestep metadata (8), Gmsh `$Periodic` (4), Netgen `identifications` (3), a Nastran variant (2), PLY extra face properties (1), the environment (a build without the JSON parser, and the stdlib base64 handling on Python 3.14 in the pure-Python VTU reader), and **one real core bug**: the C++ Tecplot reader throws `std::stoull` on `tests/python/meshes/tecplot/quad_zone_space.tec`, a file its Python twin reads. That last one is now a `WARNING` instead of invisible; the fix itself is a follow-up.
- **C++ streams are pinned to the classic locale.** `fast_number.hpp` (v12.1.0) covered `strtod` and `snprintf`, which `setlocale` drives; C++ streams are driven by `std::locale::global`, which it declined to cover. Every `std::ifstream`, `ofstream`, `istringstream`, `ostringstream` and `stringstream` under `src/cpp/` (164 sites in 50 files) is now built through the new `detail/classic_stream.hpp` (`make_classic_*`). Under a comma-decimal or digit-grouping global locale, `iss >> x` misread `1.5`, and, worse, `os << n` wrote an *integer* with the locale's grouping (`1234567` as `1.234.567`) in every ASCII writer, corrupting the file rather than misreading it. Output is byte-identical in the default configuration (the four `test_io_baseline` hashes were unchanged across the migration, before the version bump), and `imbue_classic` skips the `imbue` when the stream is already classic, so per-line streams in the readers pay one comparison. `detail/format_compat.hpp` is deliberately excepted: it is an installed function template, so imbuing there is an ABI break, and it only renders log lines and exception text.
- **Guards.** `test_no_locale_sensitive_number_io.py` now also fails a bare stream construction; `test_no_broad_core_swallow.py` fails a broad `except` around a `_core.` call that does not ask `core_declined`. `tests/cpp/test_classic_stream.cpp` proves the stream fix with a synthesized comma-and-grouping `numpunct`, so it runs on a bare CI image with no `de_DE` locale installed, and asserts the hostile locale really corrupts a plain stream so it cannot pass vacuously; its end-to-end cases fail without the migration.
- **Contributor rules** (`AGENTS.md`): floats through `fast_number.hpp`, streams through `classic_stream.hpp`, shims through `core_declined`.
- **Recorded as follow-ups, not done here** (roadmap §2): the 44 broad handlers in 36 package-root *operation* files, which silently substitute a different algorithm and need a per-operation ruling; and a `ReadError` subclass or per-entry-point wrapper in `_core.cpp` so the "recognised decline" contract is total. The roadmap's §4 also gains the Gmsh 4.0 gap this run surfaced.
...
- **`.vts` (StructuredGrid)** — the explicit-points sibling of `.vti`: the same `nx*ny*nz` hexahedron topology, but with a `<Points>` array instead of `Origin`/`Spacing`, so **reading never requires a lattice** (a genuinely curved structured mesh reads correctly) while **writing still does** (`detail::lattice_from_mesh`, as `.vti`'s writer does), since there is no way to recover which extent a mesh's points were meant to tile without one.
- **`.vtr` (RectilinearGrid)** — the genuinely more general sibling: three independent, only *monotonic* per-axis coordinate arrays (a tensor product) instead of explicit points. **Reading is fully general**, with no uniformity check at all — a graded grid, finer near a wall, reads correctly. **Writing still requires a *uniform* dense lattice** today: recovering three arbitrary per-axis arrays from an unstructured point set needs a detector `lattice_from_mesh` does not implement, so a genuinely graded mesh raises `WriteError` — a documented follow-up, not a silent gap.
- **`.vtm` (MultiBlock)** — an index file plus one `.vtu` piece per cell block, with **no lattice restriction at all**: any mesh with one or more cell blocks round-trips. Writing prunes each piece to only the points its own block references (via `clean(remove_orphans=true)`), so a point shared by two blocks is duplicated across their pieces — each piece is a standalone, independently readable `.vtu` by design. Reading combines the pieces with `merge()` (no welding: pieces are pre-separated by construction, not coincident-point fragments) and attaches one `RegionKind::Cell` region per piece, named from the index's `name=` attribute — correct even when same-typed pieces from different blocks consolidate into one output cell block, since it comes from `merge()`'s own per-input cell index map.
- All three are covered by a pure-Python fallback in addition to the C++ core (`meshioplusplus.{vts,vtr,vtm}._{vts,vtr,vtm}`), unlike several of Tier B2/B3's C++-only pieces.
...
- **`cellZones`/`faceZones`/`pointZones` round-trip as named `Region`s** (`mesh.regions`, kind `Cell`/`Side`/`Point`) instead of being silently deleted as stale companion files on write. A `faceZones` entry's cell half is exact; its facet half is not numerically stable across a round trip (the reader is free to relabel local nodes when reconstructing a named cell type from face topology alone) — the geometric face itself is unaffected, and `flipMap` is never read or written.
- **Multi-region case selection** — `OpenFoamInfo::mRegion` selects one region of a case with no single `constant/polyMesh` but `constant/<region>/polyMesh` per region; reading such a case with no region set now names the regions found under `constant/` instead of failing to locate a bare `constant/polyMesh`. Python: `meshioplusplus.openfoam.read(path, region=...)`.
- **Decomposed (`processor*/`) cases are reconstructed into one mesh on read**, mirroring `reconstructParMesh`: each processor's own `point`/`cell`/`faceProcAddressing` maps its local ids back onto the original global ones; a face claimed by two processors is the internal face decomposition split at their shared boundary, claimed by one it is interior to that processor or a real exterior boundary face (a `processor*` inter-rank patch is dropped). Read-side only; a decomposed *write* is a documented follow-up.
- **Time-directory fields** — `<case>/<time>/<field>` dictionaries round-trip as `point_data`/`cell_data`, selected by `time_step`/`arrays` exactly like every other transient format. `volScalarField`/`volVectorField`/`volSymmTensorField`/`volTensorField` become cell data, `pointScalarField`/`pointVectorField` become point data; `uniform`/`nonuniform`, ASCII and binary are all supported. A cell field's boundary-face blocks read `NaN` (`boundaryField` per-patch attachment is a documented follow-up). `read_metadata(...)["time_values"]` is a real, cheap native path (a directory listing); the rest of that summary still needs a full read, like Exodus/EnSight. **Breaking:** a plain `read()` with no `time_step`/`arrays` now attaches time-zero's fields by default when the case has any, where the format previously never carried `point_data`/`cell_data` at all — the same convention Tier B1's EnSight change established.
- All four are **C++ core only**: the pure-Python fallback reader gains none of this, matching the existing precedent that this format's more advanced pieces (the writer, entirely) ship compiled-only.

## v11.3.0 (2026-09-17)

**WASM parity, Tier B1: in-file timelines.** Closes the second tier of roadmap section 1 "WASM parity" — five formats that carried more than one time step in a single file but only ever reported (or read) the first: MED, CGNS, Tecplot, Gmsh and EnSight. `MESHIOPLUSPLUS_ABI_VERSION` stays 12 (see `doc/abi_reviews.md`); every header change is a new, additive declaration.

- **MED** — `read_med_metadata` (new, native): a cheap scan of `ENS_MAA`/`MAI` attributes and dataset extents, plus every `CHA/<field>/<step>`'s `PDT`, filling `MeshMetadata::mTimeValues` with the sorted, deduplicated union of every field's own step times — never a full read. `ReadOptions::mTimeStep` already selected a step on read; the metadata gap this closed is what let the sequence layer treat a multi-step `.med` file as an ordered dataset.
- **CGNS** — `read_cgns`/`read_cgns_mll` now honour `ReadOptions::mTimeStep`: with `BaseIterativeData_t`/`ZoneIterativeData_t` present, the resolved step's `FlowSolutionPointers` names the *one* `FlowSolution_t` read, instead of every `FlowSolution_t` in the zone; without iterative data, an array name written by more than one `FlowSolution_t` now warns naming both rather than silently letting the later one win. `read_cgns_metadata`/`read_cgns_mll_metadata` (new, native) report `Zone_t`'s own dimension triple and `BaseIterativeData_t/TimeValues` with no coordinate or connectivity decode. Implemented independently for the hand-rolled HDF5 reader and the optional cgnslib backend.
- **Tecplot** — every `ZONE` header is now scanned (not just the first), including `SOLUTIONTIME`/`STRANDID`; `mTimeStep` resolves against the timeline (zones sharing the first zone's `STRANDID`, or every zone with a `SOLUTIONTIME` when none has one, sorted by `SOLUTIONTIME`) and reads only that one zone's data. Several *non*-transient zones (no `SOLUTIONTIME` at all) now `log::warn` naming the count instead of silently discarding every zone after the first with no diagnostic. `read_tecplot_metadata` (new, native) scans every zone header's own token budget without decoding it.
- **Gmsh** — `read_gmsh_metadata` (4.1) now reports the sorted, deduplicated union of every `$NodeData`/`$ElementData` section's own real-tag time value (previously always empty). A non-default `mTimeStep` triggers a cheap header-only pre-scan and keeps only the sections matching the resolved time, across every field name — replacing the "first section per name wins" rule the default step (0) still reproduces unchanged, with no pre-scan cost.
- **EnSight** — `.case` files were geometry-only; `TIME`/`VARIABLE` sections are now read. A `scalar per node:`/`vector per node:`/`scalar per element:`/`vector per element:` entry becomes `point_data`/`cell_data`, read from the file its (`mTimeStep`-templated) filename names — EnSight's own convention is one file per step, unlike the other four formats' one file with several sections. **Breaking:** a plain `read(path)` call on a `.case` file whose `VARIABLE` section names files that exist now returns those arrays; previously they were silently ignored. `read_ensight_metadata` (new) reports `TIME`'s values (mesh shape still needs a full read, the same shape Exodus's own metadata override has). Variable *writing* stays out of scope (a roadmap remainder).
- Every format's Python shim (`meshioplusplus.<fmt>.read`) gained a `time_step` keyword forwarding to the same `ReadOptions::mTimeStep`, and `_TIME_CAPABLE_READERS`/`seq_format_may_have_steps` list all five.
...
- **One family table** (`_FAMILIES` in `physicsnemo/_train.py`): which `Model` keys a family reads, which block (`Graph`/`Grid`/`Operator`), which frameworks it imports, whether it accepts `Augmentation`, whether it allows/requires a squeeze. `spec_from_dict`, `spec_to_dict` (per-family emitters), the trainer's `run()`, `_load_checkpoint`, every `predict*`, the shared `require_frameworks` gate, the MCP `train_start` kwargs picker and the dashboard's `families.ts` (pinned equal by name and order) all read it; an unknown family is refused naming the five, never routed to another family's code.
- **`fno`** (`Grid` block): `LatentChannels`/`NumFnoLayers`/`NumFnoModes`/`SpectralPadding`/`PaddingType`/`ActivationFn`/`DecoderLayers`/`DecoderLayerSize`/`DecoderActivationFn`/`CoordFeatures`. Resolution-preserving — no `ScalingFactor`; `grid_kwargs()` pairs the grid with itself. `Grid.Squeeze` is optional and its presence *is* the FNO's `dimension`.
- **`afno`** (`Grid` block): `PatchSize`/`EmbedDim`/`Depth`/`MlpRatio`/`DropRate`/`NumBlocks`/`SparsityThreshold`/`HardThresholdingFraction`. 2-D only, so `Grid.Squeeze` is **required** by name; the sample shape (`Grid.Resolution [n, ...]` gives `n + 1` points) must divide `PatchSize` and `EmbedDim` must divide by `NumBlocks`, both checked **before torch is imported**; the card's recorded `inp_shape` is checked against the mesh at inference before the forward.
- **`deeponet`** (new `Operator` block: `Parameters`, `Trunk` `points|budget`, `TrunkCount`, `TrunkMethod`, `TrunkSeed`, `Float32`): the experimental `physicsnemo.experimental.models.xdeeponet.DeepONet` over an MLP branch on each entry's `Metadata` parameters and an MLP trunk on the mesh points. Takes no `Fields`. `DecoderType` accepts only `mlp` (the installed model refuses `conv` for an MLP branch; `temporal_projection` needs an `output_window`), refused by name. **Fixed geometry, checked and named**: a differing point count or parameter shape across entries is refused at index-build time naming both entries. New pure API `operator_sample`/`OperatorSample`/`iter_operator_samples`/`operator_stats`/`has_deeponet` (`OPERATOR_SAMPLE_VERSION` 1), `make_operator_dataset` in `_torch.py`; `predict_mesh`/`predict_file` (and the MCP `predict_file` tool) gain `parameters=`; a budget's write-back fills NaN, never 0, off the trunk.
- **The 2-D grid contract**: `grid_layout_after_squeeze(axis)` in `_grid_transfer` names the layout of the axes that remain; the grid card records `layout`/`spatial_ndim`/`squeeze`/`squeeze_index`/`x_shape`/`expand_axis`/`expand_size` (afno: `inp_shape`/`patch_size`); `_grid_norm_tensors` shapes the normalizers from `spatial_ndim`; a 2-D prediction is expanded back over the thin axis before it is scattered onto the mesh; `spectrum_rel_l2` is `None` under a squeeze (a 2-D spectrum is a follow-up). **`GRID_SAMPLE_VERSION` 1 → 2**: the grid-sample schema gained `x_shape`, `y_shape` and `expand_size`, so a stored v1 schema now compares unequal — the drift guard working, as with `GRAPH_SAMPLE_VERSION`.
- **Surfaces**: `train_start` gains the three families' kwargs (`latent_channels`, `num_fno_layers`, `num_fno_modes`, `spectral_padding`, `squeeze`, `squeeze_index`, `patch_size`, `embed_dim`, `depth`, `num_blocks`, `parameters`, `trunk`, `trunk_count`, `trunk_method`, `trunk_seed`, `branch_layers`, `branch_layer_size`, `trunk_layers`, `trunk_layer_size`, `width`) and `train_defaults` reports `deeponet` among the frameworks; `train_list` lifts the new hyperparameters by name; the dashboard's model selector offers all five (DeepONet greyed out until the companion reports it), with per-family option panels driven by the new pure `families.ts`, and run comparison lines up `model_name`. The MCP tool count is unchanged at 84.
- **Three GPU-executed worked examples** in `example/physicsnemo/`: `fno_darcy.py`, `afno_advection.py`, `deeponet_beam.py` (shared `_common.py`), each scored against an honest baseline, with `scores/*.json` and renders committed. The FNO beats the effective-medium Darcy solve about 7x in RMSE and the DeepONet beats a per-node linear least-squares fit about 17x; **the AFNO beats persistence about 8x but does not beat first-order upwind with the true velocity** (RMSE 1.85e-2 against 1.11e-2, still improving at its best epoch 98 of 100), reported as run rather than tuned.
- **Three pre-existing bugs fixed** by the family-table refactor: `Grid.Float32: false` on an srresnet was accepted, emitted and silently ignored (read from the `Graph` block); `run_training` demanded `torch_geometric` unconditionally before loading the spec, so a grid run through the public API was refused over a dependency it never touches; and `tool_train_start(model_name="gpt")` built a meshgraphnet spec and failed inside the parser instead of naming the known families. Also fixed: `make_grid_dataset`'s schema was cached on the *class*, so the validation dataset inherited the training one's.

## v10.39.0 (2026-09-09)

**Curved tessellation and a provenance map.** `convert_cells(mode="simplexify")` already splits a curved cell conformingly, but it keeps only the corner nodes — a higher-order cell's own curvature is dropped entirely — and records no provenance from a synthetic simplex back to the cell it came from. `tessellate` closes roadmap section 1's last non-model-family bullet: isoparametric subdivision of the five higher-order cell types onto a refinement lattice, interpolated on the way in, with a full point/cell provenance map so a prediction made on a tetrahedron is written onto the hexahedron it was carved out of. Pure Python over existing tables — no C++, ABI, WASM or binding change. See [`doc/tessellation.md`](doc/tessellation.md).

- **New `tessellate(mesh, levels=2, curved=True, fields=True, record_stencil=False)` → `Tessellation`.** `CURVED_TYPES = ("triangle6", "quad8", "quad9", "tetra10", "hexahedron27")` — `hexahedron20` is deliberately excluded, since its faces are `quad8` (no face-centre node) while `hexahedron27`'s are `quad9`, and curving both would let two neighbouring cells' watertightness keys silently merge two different face-interior positions. Every other cell type passes through unchanged, so `tessellate` is an exact identity on an all-linear mesh.
- **Every parametric node position is derived, never hardcoded** — from `_convert_cells._ELEVATE` (which edge is mid-node `k`) and `_refine_templates.QUAD_FACES` (which face is centre-node `k`), plus a hand-derived reference element per base shape chosen so the isoparametric barycentric solve is an exact identity matrix. No table here is transcribed from Kratos or any other external source, after an earlier finding that Kratos's own `hexahedron27` node ordering does not match meshio++'s convention.
- **Watertightness is a point-identity classification, not a coordinate tolerance.** A lattice point's kind falls out of counting its own nonzero isoparametric corner weights, and its dedup key is built entirely from corner *global point ids* — an edge point keys on the sorted endpoint pair plus a quantized fraction, a face point on a canonical traversal of the face's corner-id cycle (start at the smallest id, walk toward the smaller neighbour), so two cells describing the same physical entity compute the identical key regardless of local numbering. Every key is an all-integer tuple, which is what lets final point ordering be a plain `sorted()` with no custom key function.
- **`Tessellation.gather`/`.scatter`/`.aggregate`/`.measures`/`.from_mesh`** move data between the tessellated mesh and its source in either direction; `.from_mesh` reconstructs a full `Tessellation` (via `record_stencil=True`'s persisted stencil/weights arrays) from a written-and-reread file, with the same stale-key guard `refine:entity` already established.
- **PhysicsNeMo integration**: `Graph.Tessellate` in `TrainSpec` (scoped to `Kind="node"` graphs with `Regions=False`); `_read_sample` applies it for one sample's lifetime; `predict_mesh`/`predict_file`/`_attach` take a `tess=` parameter that scatters/aggregates a prediction back onto the original mesh, with `tess=None` pinned byte-identical to every pre-existing call.
- Full binding surface: a `tessellate` CLI verb, an MCP tool (registry 83 → 84), and `tessellate`/`Tessellation` exported from the top-level `__all__`.
- No ABI change — pure Python, no core touched at all.

## v10.38.0 (2026-09-09)

**Mesh repair, shrinkwrap and Sobolev deformation.** meshio++ could weld and prune a surface but not rewind it, close it or unpinch it; could measure the distance to a surface but not fit a template onto one; and could smooth a mesh's *shape* but not filter a *displacement field* a caller already had. These three operations close roadmap section 1's mesh-operations bullet in full — the last of the operations NVIDIA PhysicsNeMo's `physicsnemo.mesh` exposes and meshio++ did not (curvature shipped in v10.37.0). Adapted from the published algorithms only; no upstream code is read or vendored. See [`doc/repair.md`](doc/repair.md), [`doc/shrinkwrap.md`](doc/shrinkwrap.md) and [`doc/sobolev_deform.md`](doc/sobolev_deform.md).

- **New `repair(mesh, fix_orientation=, orient_outward=, fill_holes=, split_non_manifold=, max_hole_edges=, weld_tolerance=, record_provenance=)`.** Weld (opt-in, through `clean`) → triangulate → split bowties → orient → fill holes → orient outward. Reports the input's **and** the output's four defect counts, so what was fixed and what remains are both visible. Output is all-triangle at the surface with blocks 1:1 with the input, lower-dimensional blocks carried verbatim, and fill triangles in one trailing `triangle` block added only when there is one.
- **Orientation uses the topological half-edge rule, not a normal test** — two triangles sharing an edge agree iff they traverse it in opposite directions. That is exact; upstream's `dot(n_child, n_parent) < 0` is a local-flatness approximation that rewinds a correctly wound strip folded past 90 degrees. A 150-degree crease pins the difference. Fewest flips wins within a component, so a mostly-correct mesh is barely touched, and an unorientable component is counted rather than refused.
- **A hole's fan is wound to agree with the surrounding surface**, against the one triangle on each boundary edge — upstream winds it from the loop's own traversal, which can leave the fill inconsistent with what it closes. Non-manifold *edges* are counted, never split; nested cavities are not detected.
- **New `shrinkwrap(mesh, target, offset=, max_distance=, weights=, target_region=, normal_weight=)`.** One projection onto the target's surface, no iteration and no self-intersection guard, as upstream. Every point of the source moves whatever cells it carries — a volume mesh's interior points too; only the *target* must be a surface. The nearest point comes from the same search `sample_distance` uses, so the two cannot disagree.
- **The offset goes along the hit FEATURE's pseudonormal, not the selected triangle's normal.** At a crease the offset surface's normal is the bisector, so a face normal lands the point off that surface and makes the answer depend on which of two equidistant faces won the tie-break. A 90-degree "book" target pins it. `normal_weight="area"` is the mode the numpy twin reproduces bit for bit under a nonzero offset; `"angle"` (the default) goes through `acos` and is excluded by name there.
- **New `sobolev_deform(mesh, array, length_scale, fixed_points=, fix_boundary=, record_filtered=)`.** Solves `(M + l^2 K) u = M d` over a **new P1 assembly** (the simplex edge Gram matrix, so one formula serves a polyline in 2-D, a triangle surface in 3-D and a tetrahedral volume) and a **new matrix-free Jacobi-preconditioned conjugate-gradient solver** — the first linear solver in this repo. Top-dimensional blocks must be linear simplices; nothing is pinned by default, so a constant displacement is preserved exactly in zero iterations.
- **Three of upstream's choices are matched deliberately** — the Gram-matrix assembly, the uniform mean lumped mass, and one global solve with one global stopping test — which is what makes a gated parity test against `physicsnemo.mesh.Mesh.sobolev_deform` a valid oracle. That test runs here on a triangle surface and a tetrahedral volume, with and without pinned points. Non-convergence is warned about and the last iterate returned, where upstream raises.
- `repair` and `sobolev_deform` have **no numpy fallback** (the outward-orientation sign and the CG stopping test are both branches a second implementation could land on the other side of), and raise `NotImplementedError` by name without the compiled core — the `subdivide`/`agglomerate` precedent. `shrinkwrap` has a near-complete bit-exact twin, with the one documented exclusion above.
- Full binding surface in one release: `repair`, `shrinkwrap` and `sobolev-deform` verbs in both CLIs, `Repair` and `SobolevDeform` settings-pipeline steps (`Shrinkwrap` is excluded like `Interpolate`, needing a second mesh), the C API (`mio_repair`/`mio_shrinkwrap`/`mio_sobolev_deform` with opts/report structs and `_opts_init`), Fortran, Julia, R, WASM, and three MCP tools taking the registry to 83.
- No ABI change (`MESHIOPLUSPLUS_ABI_VERSION` stays 12): three wholly new headers, plus one additive struct and function in `detail/surface_distance.hpp` (`SurfaceProjection`/`query_surface_projections`, sharing the nearest-triangle search and the feature-normal lookup with `query_distances`). `RepairOptions`, `ShrinkwrapOptions` and `SobolevOptions` are pinned in `tests/cpp/test_abi_layout.cpp` from this release.
- Also fixes `conanfile.py`, whose `version` had silently stayed at 10.36.0 through the v10.37.0 release.

## v10.37.0 (2026-09-08)

**Per-vertex curvature.** meshio++ had two-thirds of physics-ML's mesh operations (gradient, Hessian, error estimation, signed distance) but not the one PhysicsNeMo's own `gaussian_curvature_vertices`/`mean_curvature_vertices` compute: per-vertex mean and Gaussian curvature. `compute_curvature` closes that gap with a second, independent estimator alongside `remesh`'s own internal curvature-gradation fit — the standard discrete estimators (angle defect for `K`, cotangent Laplace-Beltrami for `H`), over the caller's own mesh, with a real testable invariant. Narrows roadmap section 1's mesh-operations bullet to shrinkwrap and Sobolev deformation. See [`doc/curvature.md`](doc/curvature.md).

- **New `compute_curvature(mesh, mean=, gaussian=, dual_area=, include_boundary=, record_area=, record_principal=, region=)`.** Writes `curvature:mean`/`curvature:gaussian` as `point_data`, optionally `curvature:area` and `curvature:principal` (the `(n, 2)` pair `k1 >= k2`). Two dual areas: `"mixed-voronoi"` (default, converges markedly better on an irregular tessellation) and `"barycentric"` (cruder but branch-free).
- **The oracle is Gauss-Bonnet.** For a closed surface the sum of every vertex's angle defect is `2*pi*chi` — `4*pi` for anything sphere-like — whatever the tessellation and whichever dual area, reported as `total_angle_defect` so the invariant is checkable from every binding, not only a gtest.
- **`H` is orientation-dependent, `K` is not.** A mesh whose facets disagree about which side is out yields sign-flipped patches with no error raised, so the result always carries the input's surface quality (the same four counts `sample_distance` reports); check `quality["inconsistent_pairs"]` before trusting a sign. This operation never silently repairs its input.
- **Deliberately not `remesh`'s own curvature estimator**, which fits an osculating paraboloid for its gradation/anisotropic-metric machinery, returns only a magnitude, and runs on remesh's own subdivided working copy. That estimator is left exactly as it is.
- **A partial numpy fallback.** `dual_area="barycentric"` has a real, bit-exact numpy twin (every expression is `+ - * / sqrt` and one `atan2`, pinned against the compiled core with exact array equality). `dual_area="mixed-voronoi"` (the default) has none and raises `NotImplementedError` by name when the compiled core is unavailable: its obtuse/non-obtuse test is a discrete branch on a sign, the same class of refusal `smooth`'s inversion guard and `sample_distance`'s `sign="winding-number"` already make.
- Full binding surface in one release: a top-level `curvature` CLI verb in both CLIs (it takes no input data array, unlike `data gradient`/`data hessian`/`data estimate-error`), a `Curvature` settings-pipeline step, the C API (`mio_compute_curvature`/`mio_curvature_opts`/`mio_curvature_report`, an options-struct entry point from the start since there is no flat form to keep back-compatible), Fortran, Julia, R (with its `NAMESPACE` export entry, the documented past miss for this shape of addition) and WASM, plus an MCP tool taking the registry to 80.
- No ABI change (`MESHIOPLUSPLUS_ABI_VERSION` stays 12) — a wholly new header, Tier C additive; `CurvatureOptions` is pinned in `tests/cpp/test_abi_layout.cpp` from this release.

## v10.36.0 (2026-09-08)

**FLAC3D cell groups: fixed, and finally in the C++ core.** [Issue #76](https://github.com/loumalouomega/meshioplusplus/issues/76) reported that both bundled `.f3grid` reference meshes come back with mangled `ZGROUP`/`FGROUP` membership — three of the five groups empty, the other two shifted — where meshio++ 7.14.0 read them correctly. The reader, inherited verbatim from meshio, put **global** block-major cell indices into `cell_sets`' per-block slots; that was harmless while `cell_sets` was plain storage, and wrong from v8.1.0, when it became a write-through view over `mesh.regions` and `blocks_to_global` started adding the block base itself. See [`doc/formats/flac3d.md`](doc/formats/flac3d.md).

- **Group membership is correct again.** The reader now rebases each group to per-block local indices, so `mesh.regions` carries the exact global membership the file describes and `mesh.cell_sets` the matching per-block view. The bundled reference files' five groups are pinned by value — `face:bottom` cells 0-17, `Brick1` 18-44, `Pyramid2` 45-71, `Wedge3` 72-98, `Tetrahedron4` 99-125 — on each engine separately. The old assertion checked the number of *blocks*, which is fixed for any region, so it passed throughout.
- **Breaking: `mesh.cell_sets[name][block]` now holds block-LOCAL indices for this format**, the convention every other format and the whole region layer already use. Code that read those values as global cell indices — which is what made them look right for a single-block group before — should read `mesh.regions` instead, or add `meshioplusplus._regions.block_bases`.
- **Breaking: group names no longer carry the ASCII slot's quotes.** The ASCII reader kept them while the binary one did not, so the same mesh in the two encodings produced different regions (`face:bottom:"Default"` vs `face:bottom:Default`). Both now yield `face:bottom:Default`.
- **`ZGROUP`/`FGROUP` are read and written by the C++ core**, in both ASCII and binary, as named `Cell` regions — so groups are reachable from WASM, the C API, Fortran, Julia, R and the native CLI, which could not see them at all before (the reader threw on sight of a group and the writer emitted hardcoded empty sections). The two engines' output is now byte-identical in both encodings.
- **The writer no longer fails on a mesh with both zones and faces**, which previously raised a `TypeError` outright — so the reference meshes could not be written back at all. Zone and face cell ids are also written as the **separate 1-based namespaces** a real FLAC3D file uses, instead of sharing one counter, which is what let face groups survive a round trip.
- **A group name is now a fixed point.** The writer decomposes a `<zone|face>:<name>:<slot>` region name back into its parts instead of hardcoding `SLOT 1`/`"Default"`, so a file read from disk writes back unchanged rather than gaining a prefix per pass. A name in any other shape (`solid`) is written into the file's vocabulary and reads back as `zone:solid:Default`; FLAC3D is recorded in `tests/python/test_region_roundtrip.py`'s new `NAMESPACED_REGIONS` bucket for that reason.
- Minor: the C++ ASCII writer pads its gridpoint ids to match the Python reference writer, which is what makes the two byte-identical; the padding is whitespace in a whitespace-tokenized field and changes no meaning.

## v10.35.0 (2026-09-08)

**Formats a physics-ML pipeline expects, plus the scalar-array fix they surfaced.** meshio++ could describe a training set but not write one in any shape NVIDIA PhysicsNeMo's own loaders read: `MeshDataset` globs for `**/*.pmsh` or sniffs a Zarr store, the DoMINO and Transolver datapipes want one `.npz` per case in a fixed key layout, and there was no USD export for a twin at all. This release adds all four as ordinary meshio++ formats, so `read`/`write`/`convert`/`read_sequence` reach them with no new API. Python-only throughout — no C++, ABI, WASM or binding change. Closes roadmap section 1's formats bullet, which is removed. See [`doc/formats/pmsh.md`](doc/formats/pmsh.md), [`doc/formats/zarr.md`](doc/formats/zarr.md), [`doc/formats/cae.md`](doc/formats/cae.md) and [`doc/formats/usd.md`](doc/formats/usd.md).

- **New `pmsh` format** — PhysicsNeMo's memory-mapped mesh directory, **hand-rolled in pure numpy rather than delegated to `Mesh.save`**, and that is the point: a solver box with no torch, no CUDA and no NVIDIA stack can write the training set the framework reads. Parity is pinned in both directions by tests that gate on an installed physicsnemo. Arrays come back memory-mapped copy-on-write, so they are writeable and paged in only as touched. Two facts neither guessable nor optional: **a zero-element array has no blob** (tensordict does not persist an empty tensor, so a point cloud's `cells.memmap` is absent while its shape is still declared), and **a `.pmsh` directory containing `zarr.json` is a Zarr store** — the extension names the role, the contents name the codec, exactly as `MeshReader._load_sample` does.
- **New `zarr` format** — the same mesh in PhysicsNeMo's `io_zarr` layout: a v3 group with the type attribute, `points`/`cells` arrays, the three data groups, upstream's own 200k-row chunking and zstd level 3, and consolidated metadata. Writing needs zarr 3.x and says so by name, since zarr-python 2.x cannot produce a v3 store at all. **Not `write_dataset(format="zarr")`**, which writes a training *table*; such a store is refused here by name rather than half-read, and vice versa.
- **New `cae` format** — the per-case `.npz` sample layout, transcribed from the Kratos `PhysicsNeMoApplication`'s own exporter, so a meshio++ case and a Kratos case are interchangeable in one dataset directory. A volume mesh's nodes and nodal fields become the volume half while its skin — extracted and triangulated here — becomes the surface half, with the volume's `cell_data` gathered onto each triangle through its parent cell. Three rules belong to the reader on the other side and each fails silently rather than loudly when broken: every array must be at least 1-D, the two volume keys must share their row count, and **no other key may contain the substring `volume`** (that reader takes its volume length from the first such key in file order, over every key in the file) — so meshio++'s own sidecars are named `node_field_*` and a global parameter containing `volume` is refused.
- **New `usd` format** — time-sampled OpenUSD stages, read and written, behind a new `[usd]` extra (`usd-core`). **n-gons are kept**, unlike every other bridge here: USD stores a face-vertex count per face, so a quad-dominant hex skin arrives as quads. `point_data` becomes `vertex` primvars and `cell_data` `uniform` ones. It joins the sequence engine's fan-in writers: one stage, one sample per step, and topology re-authored **only when it changes**, so a remeshed series stays valid while a fixed-topology one stays compact.
- **`data export-cae` CLI verb and an `export_cae` MCP tool** (the registry goes 78 → 79) writing a whole run at once from a glob, a path list or a multi-step file, one mesh alive at a time.
- **The sequence engine accepts directory stores.** A glob plan entry is now a file *or* a directory whose suffix is `.pmsh`/`.zarr`, so `write_sequence("out_{step}.pmsh", …)` then `read_sequence("out_*.pmsh")` works; deliberately narrower than "any directory". The MCP path sandbox likewise admits an existing directory as an input, and its file-download endpoint keeps rejecting one.
- **Provenance in three of the four.** Zarr keeps the block in a root attribute (plain JSON, so it is recovered without importing zarr), USD in the root layer's `documentation`, and `cae` in a NUL-padded bytes array written as the archive's first member — which the ordinary head scanner then finds with no `.npz`-specific code. `pmsh` has no free-text slot and honestly reports none rather than adding a sidecar file upstream's own loader never wrote.
- The `nvidia-physicsnemo` pin widens to `>=2.1,<2.3`, which is what the parity tests are verified against.
- **Fixed: a 0-d (scalar) `field_data` value was destroyed by every operation that clones a mesh** — `convert_cells`, `clean`, `transform`, `refine`, `smooth`, `crop`, `split`, `merge`, `interpolate`, `attach_quality` and every `data_*` op returned uninitialized memory instead of the value. `NDArray::Size()` reported 0 for an empty shape, so `Nbytes()` was 0, `MakeOwned` copied nothing and left a null pointer, and pybind11 — given a null — had numpy allocate a fresh uninitialized scalar rather than adopt the capsule. `Size()` now counts what the buffer holds. The two formats added above are what made this a live bug rather than a curiosity: `pmsh` and `zarr` both round-trip a genuine 0-d `global_data` scalar, so meshio++ produced the input meshio++ then corrupted — while the numpy fallbacks were correct throughout, meaning the two engines disagreed on the same input.
- **The fix is buffer-aware, deliberately.** An empty shape means both "a scalar" and "a default-constructed absent array", and nothing else separates them. `PropertyValue::IsText` and `KratosMesh`'s `ConcatKind` sentinel both read `Size() == 0` as "absent", so a naive "always return the empty product" fix would have silently broken MDPA's lossless text fallback. `Size()` therefore returns the empty product only when a buffer exists — which generalizes rather than special-cases, since a `{0,3}` array also allocates nothing and counts nothing.
- Three consequences worth knowing, all verified rather than argued: a legitimate 0-d `gid:result_type:*`/`gid:gauss_points:*` declaration is now accepted instead of raising "is empty"; a 0-d `exodus:time` now round-trips as a real time value instead of writing `0.0`, and the C++ MDPA writer emits a 0-d `ModelPartData` scalar instead of warning "has 0 values" and dropping it (reachable from the flat bindings; `meshioplusplus.write` routes any `field_data` to the pure-Python MDPA writer, which is unaffected); and a 0-d point/cell array now fails VTI's row-count check by name rather than being silently accepted.
- **Breaking: `MESHIOPLUSPLUS_ABI_VERSION` 11 → 12.** Editing the body of an inline function in an installed header is Tier B by `doc/abi.md` — both the library and a consumer emit a vague-linkage copy and the linker keeps one arbitrarily — so the version had to move even though `sizeof(NDArray)` is unchanged at 72 and the layout snapshot could not see it. The C++ libraries' `SOVERSION` moves with it, so a C++ consumer relinks; the C and Fortran libraries stay at `SOVERSION 0` and the C ABI is unaffected (it rejects `ndim == 0` outright). `doc/abi.md`'s and `abi_version.hpp`'s history tables, which had gone stale at ABI 9 and 10 respectively, are filled in to 12.

## v10.34.0 (2026-09-08)

**Geometry guardrails.** A trained surrogate answers any mesh it is given, and the answer for a part unlike anything it saw is finite, plausible and wrong — worse than an error, because nothing downstream flags it. This release is the missing check: fit a description of the *shapes* a model was trained on, and score a new one against it. Pure Python over `compute_stats`, `compute_quality` and `extract_surface`, which already produce every descriptor — no C++, ABI, WASM or binding change. Closes roadmap section 1's geometry-guardrails bullet, which is removed. See [`doc/geometry_guardrails.md`](doc/geometry_guardrails.md).

- **New `geometry_descriptors(mesh)` and `GeometryGuard`** (`fit`/`score`/`check`, JSON round-trip). Sixteen descriptors from the three existing operations: extents and centroid, counts as logarithms, area, volume, inverted count, the outer surface's area and facet count, and the scaled-Jacobian and aspect-ratio summaries.
- **The descriptors are deliberately NOT invariant, and that is the design.** The usual instinct is to normalize away position and scale; here that is backwards, because a scaled part is a different part and a model trained on brackets 10 cm across has learnt physics at that scale. Extents, centroid, area and volume enter raw, so a guard fitted on centimetre parts flags a metre one and a displaced part is flagged on its centroid. Pinned by a test asserting that scaling a mesh by two multiplies its descriptor area by four and its volume by eight — which a bounding-box-normalized implementation would report unchanged.
- **A diagonal Gaussian, and a threshold that means something.** Each descriptor is standardized against the training mean and standard deviation and the score is the root mean square of those z-values — deliberately the simplest thing that works, since a full covariance over sixteen descriptors needs far more cases than a training set has. The threshold is `margin` (default 1.5) times the **worst training score**, so "out" means further from the training set than anything in it was, by a stated factor, rather than a probability nobody calibrated; every training score is kept so a caller can re-threshold without re-fitting. A standard deviation is floored relative to its own mean: `num_inverted` is *exactly* zero across a well-formed training set, and without the floor any mesh that differs would score infinite instead of large-and-finite.
- **A descriptor that does not apply is `NaN`, never zero and never omitted** — a point cloud has no surface, and scoring a fabricated zero against a training mean would flag it for the wrong reason. Those are excluded from the score and listed as `missing`. What comes back names the worst descriptors, so a flag is actionable.
- **Wired in three places, all advisory.** A top-level `Guard` spec block fits on the training split and stores the guard **in the model card**, so it travels with the checkpoint `mark_best` copies; `predict`/`predict_mesh`/`predict_file` score every input, add a `guard` block to the report row and warn naming the worst descriptor — **the prediction is still made and written**, because a model cannot refuse to answer and the honest thing is to answer and say loudly that the input is unlike its training set. `dataset_health` gains a per-entry `guard` and a manifest-level `out_of_distribution` list, kept **separate from `bad_entries`**: an unusual shape is a fact about the dataset, not a defect in the mesh.
- **`guard-fit` and `guard-check` CLI verbs and `guard_fit`/`guard_check` MCP tools** (the registry goes 76 → 78). `guard-check` accepts a fitted guard *or* a model card carrying one, and without a guard the tool reports the raw descriptors.

## v10.33.0 (2026-09-08)

**Temporal windows, rollout, and dataset augmentation.** A steady-state surrogate maps one mesh to one answer; a transient one maps a short history to the next state, and nothing here built that history. This release adds the windows, the rollout evaluation that measures whether a transient model survives more than one step, and the per-epoch augmentation wrapper. It also fixes the one thing the augmentation depends on: `transform(rotate_vector_data=True)` rotated `point_data` only, so a rotated mesh carried an unrotated per-cell velocity. Closes roadmap section 1's temporal-windowing and augmentation bullets, both removed. See [`doc/physicsnemo.md`](doc/physicsnemo.md#temporal-windows-and-rollout).

- **Breaking:** **`transform(rotate_vector_data=True)` now rotates `cell_data` too**, in both engines — float arrays whose trailing dimension is 3 (`R·v`) or 9 (`R·A·Rᵀ`), at either location; an integer array is never rotated, so a material tag is safe. A vector living on a cell rotates exactly as one living on a point does, and the old behaviour was silently wrong: nothing about an array says which frame it is in. Output changes only for a mesh that carries such an array *and* passes the flag, which defaults off. The C API, Fortran, Julia, R and WASM inherit it with no signature change.
- **New `make_window`, `window_samples`, `iter_windows`, `window_stats`** — windows of K states, **oldest first**, in three schemes: `single_step` (each window paired with the state that followed, `T − K` samples), `one_shot` (the first window paired with the last state, one sample) and `time_conditional` (the first window plus a normalized time channel, so error does not compound). `make_window` is the single owner of the layout, and the recorded `x_columns` name each column's age (`T@-2`, `T@-1`, `T@-0`) so a stored contract catches a reversed ordering rather than leaving it to a convention. The streaming invariant holds: at most K *feature matrices* and one mesh alive, each step read exactly once, pinned by a read-counting test.
- **New `rollout(predict_fn, series, history_size=)` → `Rollout`** — seeded with K true states, then fed its own predictions, reporting the per-step RMSE. `predict_fn` is **injected**, which is what keeps the module torch-free: the caller owns the framework, normalization and device. Pinned by two models whose behaviour is known exactly — a perfect one (zero error throughout) and persistence, whose error grows 1, 2, 3, 4 under a real rollout and stays flat at 1 under teacher forcing, which is the distinction the test exists to make.
- **New `Augmentation(rotation=, scale=, translation=, seed=)`** and an `augmentation=`/`epoch=` pair on `iter_samples` and `make_dataset`. Every draw is a pure function of `(seed, epoch, index)` — no global generator, nothing carried between calls — so a run is reproducible and two processes of a distributed job agree. Scaling is isotropic on purpose: an anisotropic scale changes what a vector field means. **A paired sample gets one draw, replayed on the target**, since augmenting the two independently would teach the model that a part rotates between one step and the next.
- **A top-level `Augmentation` spec block**, refused by name for the `srresnet` family (a grid sampled on a fixed lattice would have its own coverage changed by rotating the mesh under it), written back only when set, applied to the **train** split only and recorded in the model card. The dataset's `set_epoch` is called between epochs through a capability check, so the shared epoch loop needs no family branch.
- Scope, stated: there is deliberately **no** new `TrainSpec` model family and no torch `Dataset` for windows — no shipped family consumes one, and unused surface with no consumer is what the spec's strict unknown-key refusal exists to prevent.

## v10.32.0 (2026-09-08)

**Single-mesh inference.** `predict` needed a `DatasetManifest`, a split and an entry; there was no way to point a trained checkpoint at one arbitrary mesh file that was never catalogued anywhere, and no CLI verb for prediction at all. This release is that path — `predict_mesh`, `predict_file`, a `predict` verb and a `predict_file` MCP tool — built by extracting the per-mesh body already inside `predict`'s loop rather than by adding a second inference path. Pure Python; no C++, ABI, WASM or binding change. Closes roadmap section 1's single-mesh-inference bullet, which is removed. See [`doc/physicsnemo.md`](doc/physicsnemo.md#single-mesh-inference).

- **New `predict_file(checkpoint, input_path, output_path, time_step=, target_path=, device=)` → a report row**, and **`predict_mesh(checkpoint, mesh, target_mesh=, device=)` → `(mesh, row)`** for the in-memory form. Everything the prediction needs comes from the checkpoint's own **model card** — which family wrote it (so a grid checkpoint needs no different call), the sample options, the read options, the column contract and both normalizations — so nothing consults a manifest. `time_step` picks a step of a multi-step input; `target_path` supplies the paired mesh a t→t+n or coarse/fine checkpoint compares against.
- **`predict` and `predict_grid` are now loops over that same body**, with the checkpoint loaded once per call rather than per entry. Behaviour-preserving by construction and pinned as such: the gated end-to-end test asserts `predict_file` returns the *same* `rmse`, `max_error` and `num_rows` for a case as `predict` does through the manifest, and the same prediction arrays.
- **A mesh carrying no truth now predicts anyway.** With the card's target fields absent from the input — the ordinary situation for a mesh that was never part of a dataset — the `<column>_pred` arrays are written, no `<column>_error` arrays are, and `rmse`/`max_error` come back `None`. Without this rule `graph_sample` would quietly take `y` from the input's own step and report an error of a prediction against itself. **`predict` inherits it**: an entry whose meshes lack the target field now predicts with a `None` rmse instead of raising.
- **A `predict` CLI verb** (`meshioplusplus predict model.mdlus in.vtu out.vtu`), which fails by name and returns 1 when the frameworks are absent rather than surfacing a traceback, and a **`predict_file` MCP tool** (gated `physicsnemo`; the registry goes 75 → 76), whose checkpoint, input, target and output all pass through the path sandbox.

## v10.31.0 (2026-09-08)

**Proximity graphs.** `edge_index` builds a graph out of a mesh's connectivity, which is exactly the wrong graph for a particle state: a smoothed-particle or discrete-element cloud has positions and no cells, and what links two particles is the interaction radius. This release is that path — radius and k-nearest-neighbour neighbourhoods with a periodic minimum-image mode, world edges alongside the mesh edges, and bistride multiscale coarsening. Pure numpy over the bucket grid `interpolate` and `merge` already use, vectorized because a proximity graph queries every point rather than a few; no SciPy, no KD-tree, and no C++, ABI, WASM or binding change. Narrows roadmap section 1 to its remaining bullets by closing the proximity-graph one, which is removed. See [`doc/proximity_graphs.md`](doc/proximity_graphs.md).

- **New `proximity_graph(mesh_or_points, method=, radius=, max_neighbors=, box_size=, kind=, undirected=)` → `(2, E)` int64.** `"radius"` links every pair closer than the cutoff — the physical rule, whose degree varies with local density; `"knn"` links each point to its nearest `max_neighbors` and symmetrizes, so degree is near-constant and a sparse region still has edges. Both are exact and neither is seeded. The output goes through the **same canonicalization `edge_index` uses**, factored out into one owner, so a mesh graph and a proximity graph over the same pair set are byte-identical — pinned by a recorded hash of `edge_index` before and after the refactor.
- **A periodic `box_size` turns on the minimum-image convention** in the search *and* in `edge_vectors`, so a pair either side of a boundary is linked and the vector between them is short: two points at `x = 0.01` and `x = 0.99` in a unit box are 0.02 apart, not 0.98. Unwrapped positions are taken modulo the box. A radius past half the smallest box side is **refused by name** — a pair then has two images at the same distance and "the" minimum image does not exist. The periodic lattice divides each axis into a whole number of cells at least a radius wide rather than cells of exactly one radius: sizing them at the radius leaves a short partial cell at the seam, and two points either side of it can be within the cutoff while sitting two buckets apart, which a single-offset neighbourhood never looks at. That was a real bug, found by the brute-force parity test on a box whose sides are not multiples of the radius.
- **The lattice is an accelerator and never part of the answer.** Every candidate is compared under one total order, so the bucket size cannot change the output — which is what let the k-nearest sizing be retuned on measurement alone (33.6 s → 20.5 s at 200k points and k=16, identical edges), and what a test pins across a sixteen-fold range of it. Below a couple of thousand points the exact `O(N²)` route runs instead, and it is the oracle the grid path is checked against on every run. Measured: 50k points in 1.3 s (radius, 3.3 M edges) and 4.5 s (k=16); 200k in 6.4 s and 23.0 s.
- **New `edge_vectors(points, edge_index, box_size=)`** — displacement then norm, the stable PhysicsNeMo convention `graph_sample` already emitted, now with a periodic mode and one owner instead of two transcriptions.
- **New `bistride_hierarchy(edge_index, positions, num_levels=)` → `BistrideHierarchy`**, the multiscale pooling a deep graph network needs so a signal crosses the mesh in logarithmically many message-passing steps. Each level two-colours every connected component by breadth-first depth from the node nearest its centroid, keeps the smaller class, and joins survivors within two hops. Two details are load-bearing and both are pinned: the seed is the centroid-nearest node, so the classes are the geometric checkerboard rather than an artefact of node numbering; and the join is over the adjacency **plus self-loops**, without which a survivor loses the reach of the neighbours that were dropped. The published 16-node path reference reproduces exactly.
- **`graph_sample` gained `proximity=` and `world_edges=`.** `proximity` replaces the mesh edges (the particle case); `world_edges` adds a second set as **separate** `world_edge_index`/`world_edge_attr` arrays, because PyTorch Geometric increments any attribute whose name contains `index` when it batches, while a concatenated set could not be split apart afterwards. **Breaking-adjacent:** `graph_sample_version` moves 2 → 3, so a schema stored under an earlier release now compares unequal — the drift guard working, since a checkpoint trained on a proximity graph must not be replayed on a mesh one. `edge_stats` takes the same two arguments and goes through the **same shared edge builder**, since statistics gathered over a different graph than training uses normalize the wrong thing with nothing downstream to report it.
- **`TrainSpec` gained `Graph.Proximity`**, validated when the document is read rather than an epoch later, and written back only when set so an unchanged spec's document is byte-for-byte what it was. There is deliberately no `Graph.WorldEdges` and no bistride block: no shipped model family reads a second edge set, and a key a run would silently ignore is what the strict unknown-key refusal exists to prevent.
- **A `proximity-graph` CLI verb and a `proximity_graph` MCP tool** (the registry goes 74 → 75), both writing the graph as `line` cells with a `degree` array and reporting the degree spread and isolated count — the numbers that say whether the radius was well chosen. `edge_vectors`/`bistride_hierarchy`/`BistrideHierarchy` are exempted from the tool parity guard as in-memory values.

## v10.30.0 (2026-09-08)

**Point-cloud budgets.** Nothing in the library reduced a large surface cloud to the fixed token count a Transolver-, FLARE- or DoMINO-shaped model takes; `feature_matrix` produced the node table and stopped there. This release is the budgeting: a selection as a value, three ways to make one, and a mesh carrying only the selected points. Pure Python over existing machinery — no C++, ABI, WASM or binding change. Closes roadmap section 1's point-cloud bullet, which is removed. See [`doc/point_budgets.md`](doc/point_budgets.md).

- **New `select_points(mesh, count, method=, seed=, start=, bounds=)` → `PointBudget`**, whose `indices` are kept **in selection order**: for farthest-point sampling every prefix of the selection is the selection at that smaller budget, so one budget at 8192 serves 4096 and 2048 by slicing (`budget.prefix(n)`, pinned by a test). `take(values)` gathers the selected rows of any per-point array and `to_dict`/`from_dict` round-trip the budget through JSON, so it can live in a model card. The recommended token table is composition, `budget.take(feature_matrix(mesh).matrix)`, which keeps that function's column contract intact — there is deliberately no `feature_matrix(budget=...)` parameter.
- **Three methods with measured costs.** `"farthest"` is exact greedy farthest-point sampling, `O(N·count)` — 50k points / 1024 in 0.10 s, 200k / 2048 in 1.1 s, 1M / 4096 in 22.6 s on one core (the inner loop runs over three contiguous coordinate arrays with in-place ufuncs, six times faster than a row-wise `(N, 3)` reduction and identical in result). `"grid"` quantizes the cloud onto a coarse `GridSpec` lattice, keeps the point nearest each occupied cell's centre and farthest-point samples over those representatives, `O(N + count²)` — the same three sizes in 0.04 s, 0.15 s and 0.8 s, the scalable choice above a few hundred thousand points. `"random"` is the baseline. Coverage is measured by fill distance, the largest gap a budget leaves unseen: on the 56k-point Stanford bunny at 2048 points, 2.7 (`farthest`), 3.2 (`grid`) and 13.8 (`random`) length units.
- **Seeding follows the library's one convention** — a keyword-only `seed` fed to a locally constructed `random.Random` — and reaches the two deterministic methods only through `start=None`, which draws the first point; ties break on the lowest index so a selection is a function of the input alone. `bounds` restricts the candidates to a box while the indices stay indices into the whole mesh. A duplicated point is never selected twice: a cloud with fewer distinct positions than the budget fails by name rather than padding.
- **New `subsample_points(mesh, budget_or_count, record_ids=)`** — the selected points, their `point_data`, `field_data` and every Point region remapped, with a single `vertex` block so `.vtu`/`.vtp`/`.ply` all accept the cloud; cells, `cell_data` and Cell/Side regions are dropped with one warning each naming what went. `record_ids` attaches `budget:original_point_id`. `feature_matrix` on the subsampled mesh has the same columns as on the source and equals the gathered table row for row, pinned by a test.
- **A `subsample` CLI verb and a `subsample` MCP tool** (the registry goes 73 → 74); `PointBudget`/`select_points` are consciously exempted from the tool parity guard as in-memory values. The [PhysicsNeMo models page](doc/physicsnemo/models.md), which still described the grid data path as "on the roadmap" two releases after it shipped, now names what exists for each data shape.

## v10.29.0 (2026-09-04)

**Superresolution end to end.** `TrainSpec` grows a second model family, `predict` dispatches on the model card, the dashboard gains a model selector, and a GPU-executed worked example beats the trilinear baseline 19x on RMSE and 93x on the power spectrum. Closes roadmap section 1's grid bullet, which is removed. Python and browser only; no C++, WASM or binding change. See [`doc/physicsnemo.md`](doc/physicsnemo.md#superresolution-the-srresnet-family).

- **New `srresnet` family in `TrainSpec`** — `Model.Name: "srresnet"` with `ScalingFactor`/`ConvLayerSize`/`ResidBlocks`/kernels/`ActivationFn`, and a `Grid` block taking the same lattice vocabulary as `GridSpec.from_mesh`. **Cross-family strictness**: a hyperparameter meant for the other family, the wrong block, a `ScalingFactor` outside {2, 4, 8}, or `Grid.Squeeze` on an SRResNet (which is `Conv3d` throughout) each raise by name. A silently dropped key is how a run ends up training a model nobody asked for. `spec_to_dict` emits only the family's own blocks, so the MeshGraphNet document is unchanged.
- **A grid run needs `torch` and `nvidia-physicsnemo` but not `torch_geometric`.** PyG exists to batch ragged graphs and a dense `(C, D, H, W)` tensor is not one, so demanding it would refuse a runnable job over a dependency whose prebuilt wheels routinely lag torch. The grid dataset is a plain `torch.utils.data.Dataset` in the new `physicsnemo/_torch.py`; `_frameworks` and `_require_training_frameworks` take the model name.
- **The grid model card records the layout as a literal string** (`"channels_first_zyx"`) alongside both grid specs, because a checkpoint that does not say which axis is which produces finite, plausible, transposed numbers and nothing downstream can tell. `predict` reads the card and dispatches on it, writing `<field>_pred`/`_true`/`_error` into an ordinary `.vtu` — so the dashboard's prediction preview works on a superresolution run with no browser change. Rows gain `coverage` and **`spectrum_rel_l2`**, the relative L2 between the predicted and true azimuthally averaged spectra.
- **Fixed: the input normalizer was not as wide as `x`.** It was built field-by-field while the model was sized from the recorded columns, so with `Graph.Regions` on the two disagreed. With one field numpy **broadcast** the single value across every column — no error, and the region one-hots silently normalized by the field's own statistics; with several fields it was a shape crash. It is now built column by column from the recorded sources, with a one-hot contributing mean 0 / std 1, and has a regression test.
- **`normalizers_from_card` treats a missing `edge_normalization` as empty** — a convolutional model has no edges — and the trainer's epoch loop is now shared by both families, with only the forward pass differing. The MeshGraphNet path is unchanged, which its own end-to-end test confirms.
- **MCP and the dashboard**: `train_start` gains `model_name` and the grid parameters (with the parity-guarded server mirror); `train_list` lifts each family's own hyperparameters and reports `None` for the other's; the launch form gains a model selector that swaps which block it posts, and refuses a malformed resolution before starting anything.
- **New `example/physicsnemo/superresolution.py`**, executed for real: 60 cases of a multi-scale 3-D field, 150 epochs in 149 s on one GPU. Test-split RMSE 0.0081 against the baseline's 0.1556, and `spectrum_rel_l2` 0.0027 against 0.2493. That the two ratios differ so much is the point — the baseline gets the large scales roughly right and loses the small ones, which a pointwise error under-reports by design.

## v10.28.0 (2026-09-04)

The **dataset side of superresolution**: a manifest entry can now describe a paired coarse/fine series, and the PhysicsNeMo adapter can turn one into the grid pair a convolutional model trains on. Pure Python — no C++, WASM or binding change. Narrows roadmap section 1's first bullet to its last item. See [`doc/datasets.md`](doc/datasets.md#paired-cases) and [`doc/physicsnemo.md`](doc/physicsnemo.md#grid-samples).

- **`DatasetEntry` gains an optional `Target`** — a second source of exactly the same shape as `Source`, validated by the same code (parameterized on the block name, so a `Target` failure names `Target`). An entry without one is **self-supervised**, and that is the ordinary case rather than a fallback: a superresolution dataset is usually one high-resolution solve per case with the coarse input made by sampling it less finely. Absent, every existing manifest serializes byte-identically — pinned by the browser twin's byte-parity fixture, regenerated from a real Python `save()`.
- **The pairing is checked, and the second check is the one that matters.** Equal step counts catches the obvious mismatch; **agreeing times** catches the silent one, where a coarse run written at a different output interval yields two plans of the same length whose steps are at different instants, so index pairing would train the model to map one moment onto another. Times are compared only where both sides carry a real one, and an explicit `Times` list always overrides `TimeFrom` — declaring times is taken to mean them.
- **New `GridSpec.upscale_samples` / `sample_scaling_factor`**, and this is the release's load-bearing detail: a convolutional upsampler multiplies **sample** counts, not cells. `SRResNet(scaling_factor=2)` maps `(B,C,5,5,5)` to `(B,C,10,10,10)`, while `upscale(2)` on a 4×4×4-cell grid gives 9×9×9 points — right for resampling, where all 125 coarse points nest, and wrong for a model pair, where it surfaces as a shape error deep inside a loss. Both preserve the box exactly. Pinned by a gated test that runs a real SRResNet at s=2 and s=4 and asserts the output shape equals the pair's target.
- **New `grid_sample_pair` / `iter_grid_samples` / `grid_stats`** in `meshioplusplus.physicsnemo`, mirroring `graph_sample`/`iter_samples`/`field_stats` and honouring the same streaming invariant. `grid_stats` is deliberately *not* a mode of `field_stats`: a grid's statistics include the fill wherever the lattice reaches outside the mesh, so normalizing a grid with node stats would be silently wrong. `squeeze_grid` gained `reduce="mean"` and `expand_grid` a `size=`, for the 2-D-operator thin-axis idiom.
- **Surfaces**: `dataset add --target` (with its own `--target-format`/`--target-times`/`--target-time-from`/`--target-sort`); `dataset_add`'s `target_*` MCP parameters; `dataset_health` reporting `target_steps`/`pairing_error`, with a broken pair counting as a bad entry; and the browser's strict manifest twin plus its two health twins. **The MCP path sandbox now walks the target plan** — a `Target` is client input inside the document, so skipping it would have left the whole sandbox open through one extra key.
- `DatasetManifest._with` now derives its field list from the dataclass instead of enumerating it by hand. It runs on every `save()`, so a field missing from that list was silently dropped from the document — a bug this release would otherwise have shipped.

## v10.27.0 (2026-09-04)

The **mesh-to-grid data path** a convolutional or superresolution model needs: sample a mesh's fields onto a regular lattice, resample the lattice, scatter a prediction back onto a mesh, and measure whether the result carries the right small-scale content. Pure Python over existing machinery — no C++, WASM or binding change. Narrows roadmap section 1's first bullet. See [`doc/grids.md`](doc/grids.md).

- **New `sample_grid` / `scatter_grid` / `interpolate_grid` / `resample_grid`**, over the new `GridSpec` (a lattice as a serializable value) and `GridArray` (values plus the channel contract). Sampling is `interpolate(..., "barycentric")` onto the lattice's own points, so it inherits that operation's exactness for a linear field — measured at 1.8e-15 through the whole coarse-grid-to-upsample-to-fine-mesh chain — and it passes the lattice as a **cell-less** target, so a 128³ grid costs points but no connectivity. The channel contract is `feature_matrix`'s, so a multi-component array expands to `v_0`/`v_1`/`v_2` by the one rule the pandas export already uses, and `scatter_grid` inverts it so an array's *shape* survives the round trip as well as its values.
- **The layout is `(C, D, H, W)` — channels first, then z, y, x**, which is `Conv3d`'s and VTK ImageData's, and what `grid`'s x-fastest numbering already produces. `spec.dims` is world-order cell counts and `spec.shape` is tensor-order point counts; they are deliberately different words, and every schema records `"layout": "channels_first_zyx"` verbatim, because a transposed grid trains and predicts plausible numbers in the wrong places.
- **Trilinear is its own implementation rather than a third `interpolate` method**, and the difference is measured, not asserted: on a unit cell carrying `u = x·y`, `interpolate_grid` reproduces the field **exactly** (it is trilinear) while barycentric interpolation over the cell's simplex decomposition gives twice the value at the centre. Both are exact for a linear field, so a test using only one would have seen nothing.
- **New `power_spectrum`**, azimuthally averaged, whose power sums to `mean(field**2)` exactly — Parseval holds only because every mode is binned including those past the isotropic Nyquist, so `counts` is reported rather than the tail truncated. An anisotropic lattice raises rather than averaging something not rotationally meaningful. The rotation-invariance test is the one that discriminates a shell average from per-axis binning; a plausible per-axis implementation disagrees on it by 7e-4 against an assertion of 1e-15.
- **`coverage`** — the fraction of lattice points inside the source — is reported on every sample, because a grid around a concave domain is mostly fill and a model trained on it learns the fill. It is free when `extrapolate` is off (a constant probe channel rides the same interpolation call, exact because barycentric weights sum to one) and costs one extra pass when it is on.
- **`squeeze_grid`/`expand_grid`** for the 2-D-operator thin-axis idiom, taking a **world** axis. A lattice always has at least two points per axis, so `index` is required whenever the axis is longer than one rather than silently keeping the first plane.
- **Four CLI verbs** (`grid-sample`, `grid-resample`, `grid-scatter`, `grid-spectrum`, sharing `voxelize`'s six-field lattice vocabulary) and **four MCP tools** (`grid_sample`, `grid_scatter`, `grid_resample`, `grid_power_spectrum`; the registry goes 69 → 73). Cache grids as `.vti`, which stores the lattice as origin/spacing/extent so the spec is recovered exactly; a grid written to `.vtu` is a lossy round trip for this purpose and reading one back as a grid fails by name. `cell_data` is refused in both directions, pointing at `data to-point`.

## v10.26.0 (2026-09-04)

Documentation only. Adds a fourteen-page **PhysicsNeMo concepts** section explaining the framework a physics-ML pipeline sits on top of, and opens a new roadmap section for the data paths meshio++ does not yet have. No code, no ABI change, no binding change.

- **New `doc/physicsnemo/` section, and a new "Physics ML" sidebar group** holding it alongside the existing ML, dataset-manifest, adapter and dashboard pages, so the concepts, the adapter and the workflow read as one story. The pages cover what a `Module` and a `.mdlus` checkpoint are and why a model card is the thing that stops you silently misusing one; the 25 architecture families as a chart keyed on the shape of one sample; the layers and the GPU functionals you can call with no model at all; datapipes and the four contracts that bite; the simplices-only mesh type and what tessellating into it costs; the three different things "residual" means; diffusion, metrics and ONNX; uncertainty and whether an error bar is honest; the active-learning loop; where the time in a pipeline actually goes; the two kinds of parallelism; the companion packages; and the 2.2 surface with its upgrade traps.
- **Every page closes with what meshio++ actually supplies**, named against real operations rather than claimed in general — and, where it supplies nothing, says so. Two of those are worth repeating: the mesh page states that `convert_cells(simplexify)` splits conformingly because its hexahedron diagonal is fixed rather than chosen per cell, and that the *provenance map* which would let a prediction on a tetrahedron be written back onto its parent hexahedron does not exist; and the uncertainty page states that a geometry guardrail is the gap this library is unusually well placed to fill, since `compute_stats`, `compute_quality` and `extract_surface` already produce every descriptor it needs.
- **Twelve figures**, ten adapted and two newly drawn (the architecture decision chart and the active-learning loop, both hand-authored SVG rather than a new Mermaid or math dependency for four diagrams). Every figure was rendered and measured: a script checks each `<text>` element against its own box and the canvas, and no label overflows either by more than a few pixels. Figures now get a card treatment in both themes.
- **New roadmap section 1, "Physics-ML data paths beyond graphs"** (the previous sections 1–6 renumber to 2–7). It names the Kratos `PhysicsNeMoApplication` as the reference implementation to port each item from, and lists nine gaps with the grid/superresolution path first — mesh-to-lattice sampling and trilinear scatter-back, coarse/fine dataset pairing, an `srresnet` family in `TrainSpec`, and an azimuthally averaged power spectrum. Reconnaissance for that first item is recorded in the section rather than left to be redone: `interpolate(..., "barycentric")` onto a simplexified `grid` already reproduces a linear field at every lattice point to within 2e-15, and it accepts a cell-less target, so the sampler never materializes hexahedron connectivity.
...
- **New `meshioplusplus.physicsnemo.run_training` / `predict`**, and `python -m meshioplusplus.physicsnemo.train --spec spec.json`. A run is described by a **training spec** — a hand-editable PascalCase settings document (`Manifest`, `Fields`, `TargetFields`, splits, hyperparameters, `Model`, `Graph`), strictly parsed like a dataset manifest — and everything it writes lands in one run directory: `metrics.jsonl` (one row per epoch), `progress.json`, the normalization stats in PhysicsNeMo's own key convention, and `checkpoints/` (periodic `save_checkpoint` state, plus `best.mdlus` and `final.mdlus`). **Every `.mdlus` carries a model card** recording the field names, the column contract and the normalization it was trained under; `predict` reads that card and refuses by name on feature drift. SIGTERM finishes the epoch, writes `final.*` and exits 143, so a stopped run leaves a usable checkpoint. The pure half (`physicsnemo/_train.py`: spec, layout, metrics, card) is stdlib+numpy on Python 3.8 and runs in the default CI matrix; `physicsnemo/train.py` is the only module importing torch, PyTorch Geometric and PhysicsNeMo.
- **The model is sized from the recorded feature schema**, not from `len(Fields)` — with region one-hots the two differ, and the example's old script would have built the wrong first layer.
- **New job manager** (`meshioplusplus/mcp/_jobs.py`) and **ten `train_*` MCP tools**: `train_defaults`, `train_start`, `train_status`, `train_metrics`, `train_log`, `train_list`, `train_stop`, `train_checkpoints`, `train_mark_best`, `train_predict`. The manager never imports torch — it spawns the trainer as a subprocess in its own process group, derives status from the process and the trainer's own files (so a run survives a server restart), and tails the log by byte offset. `train_start`/`train_predict` are gated on `torch_geometric` + `nvidia-physicsnemo` and check **before** spawning.
- **Dataset manager:** a manifest the server can see gains a *Training* section — fields, targets, splits and hyperparameters, then **Start training** — and a run panel that follows the job: a log-scaled loss chart (hand-rolled SVG, no new dependency; hover crosshair, legend and direct labels), progress and ETA, a stop control, the checkpoint list with downloads and *mark best*, and the live log with a follow toggle. A finished run raises a browser notification; **Runs…** reopens any past run, so closing the tab does not lose one.
- `example/physicsnemo/train.py` and `infer.py` are now thin wrappers over the shipped trainer and cannot drift from it; the committed results are from a fresh GPU run (100 epochs over 160 graphs in 50.3 s, validation MSE 7.9×10⁻¹ → 1.6×10⁻⁴, mean test RMSE 0.0027).

## v10.23.0 (2026-09-04)

The MCP server gains an **HTTP front-end** — the local companion process the browser dataset manager talks to — and two dataset tools, closing roadmap section 1's server-side health-summary bullet and the "companion process" half of its training-launch bullet. See `doc/dashboard.md` and `doc/mcp.md`.

- **`meshioplusplus-mcp --http`** (new `[dashboard]` extra: the `mcp` SDK plus Starlette and uvicorn, not in `[all]`) serves, from one process, MCP over streamable HTTP at `/mcp` for agents (SSE on an SDK without it) and a JSON API for the dataset manager: `GET /api/health`, `POST /api/tools/<name>` (every registry tool through the same sandbox and strict-JSON sanitizer; a tool failure stays a `200` `{"error", "error_type"}` payload) and a sandboxed `GET /api/files?path=` download. Loopback bind and a per-start bearer token by default; `--host`, `--port`, `--token`/`--no-token`, `--allow-origin` (loopback origins and the hosted docs site are always admitted; Chrome's Private Network Access preflight is answered) and `--runs-dir`. Without the extra the flag fails by name. `src/python/meshioplusplus/mcp/_http.py` is the only module importing Starlette/uvicorn.
- **New tools `dataset_find` and `dataset_health`.** `dataset_find` walks a sandboxed directory for manifests and reports each file's SHA-256 (the identity a browser-side card binds to, since the File System Access API never reveals a path); `dataset_health` is the server-side producer of the dashboard's health summary (`data_info` NaN/Inf over the data arrays with `quality:*` excluded, `compute_quality` inverted/degenerate counts and worst scaled Jacobian, arrays present, split balance, fields missing across entries, bad entries), one mesh alive at a time, every entry's resolved paths containment-checked. Both live in the pure layer (`_health.py` is stdlib + meshioplusplus, Python 3.8) and are tested in the default matrix.
- **`meshioplusplus.mcp._tools.call_tool(name, kwargs)`** is the one dispatch path (the guard moved out of `_server.py`); `create_server` takes a `host` so FastMCP's DNS-rebinding protection matches a non-loopback bind.
- **Dataset manager:** a *Companion process* panel (URL + token, remembered in IndexedDB and retried on the next visit); cards are bound to the server's manifests by content hash (`on server` chip, **Scan (server)** on the card and in the curation view — the server's summary fills the same badges and entry scans), and manifests only the server knows appear as server-only cards. Everything server-dependent stays hidden until the health probe succeeds, so a page with no server is unchanged.
- Test surface: `tests/python/test_mcp_http.py` (Starlette `TestClient`: auth, dispatch and error shapes, CORS allow-list, PNA, file downloads — the `mcp` CI job installs the extra plus `httpx`), `tool_dataset_health`/`tool_dataset_find`/`call_tool` in `test_mcp.py`, an `api.test.mjs` unit suite and a Playwright spec against a same-origin routed fake (service workers blocked: the COOP/COEP worker would answer the fetch first).
- Docs: `doc/mcp.md` and the README stated 54 and 45 tools against 57 registered; both now say the real count (59).
...
- **Multi-step series round-trip in both directions.** Steps live in the `.post.res` headers; `read_metadata` now reports them as `time_values` through a header-only scan that skips every `Values` body without tokenising it, and `gid` joins the sequence layer's step-capable formats. Fan-out (`out_{step}.vtu`), fan-in (N meshes → one multi-step file), `TimeSeries` and `info --fast` step counts all work. **This closed an unlisted gap, not a roadmap item**: the reader had always honoured `time_step`, but metadata never opened the results sibling, so it reported *one* step whatever the file held — `gid` could be read at step N but never asked how many steps it had, which made it invisible to the whole sequence layer.
- **`Group`/`OnGroup` support, for what it actually is.** A `Group` wraps the `MESH` blocks of one mesh serving a consecutive run of steps, so GiD can swap meshes as the step changes (re-meshing / adaptive analyses). Reading now resolves the requested step to its owning group and uses only that group's meshes, results and **nodes** — each group carries its own coordinates. Previously the wrapper was skipped and every group's blocks were merged into one garbled mesh while `OnGroup` results were dropped outright; no test had a multi-group file, so the bug was latent. Writing emits a group only when a step's mesh *differs* from the previous one's, so a fixed-mesh series emits none and keeps the single-step output shape.
- The multi-step writer is deliberately **not** a public class: `write_gid_series` is a free function that *pulls* steps and is driven by `sequence_to_timeseries`. The transient surface meshio++ exposes is the sequence layer, which already carries every binding, so this needed no `GidTimeSeriesWriter` with its own C/Fortran/Julia/R/WASM surfaces — only one pybind function, since Python's sequence engine is a separate pure-Python implementation rather than a shim over the C++ one.
