...
**The ParaView index formats: `.pvtu` / `.pvtp` (a partitioned dataset) and `.pvd` (a time-indexed collection), in both engines, with `vtkGhostType` halos.** Roadmap §1.1 is narrowed to its three remaining edges rather than closed (see the last bullet). `MESHIOPLUSPLUS_ABI_VERSION` stays 14: every installed-header change is a brand-new header (`doc/abi_reviews.md`).

- **New formats `pvtu`, `pvtp` and `pvd`**, each a Python reference plus a C++ core the shim prefers and falls back from through `core_declined`, registered in every registry table, sniffed, and reachable from the C API, Fortran, Julia, R and WASM through registry dispatch with no binding change. All three write a *sibling directory* named after the index's stem holding zero-padded pieces (`case/case_0000.vtu`), with relative, XML-escaped `Source=`/`file=` paths that are always read against the index's own directory.
- **`.pvtu` / `.pvtp`** declare `PPoints`, `PPointData` and `PCellData` (name, type, component count) and name one piece per *part*, where `.vtm` carves by cell *block*. `write` carves a mesh by the integer `cell_data` array `part_key` (default `partition:part`, what `partition_labels` produces), else writes one piece; `write_pieces` takes already-carved pieces, so `partition`'s output goes straight in, halo layers included. **Every piece must declare identical arrays, checked before anything is created**, so a refusal leaves no directory and no index; the error names the array, the piece and both declarations. A part that owns no cells is still written (skipping it would renumber the rest). `read` merges without welding, one `piece_<i>` cell region per piece, or `piece=k` keeps one (negative counts from the end, out of range names the count), through the `ReadOptions::mPiece` that VTKHDF introduced; a one-piece file reads as that piece. `partition` → `.pvtu` → read-merge → `clean(weld=True)` returns the original mesh up to point ordering, which is the roadmap's own "done when", in tests for both engines.
- **Ghosts.** A `partition:ghost` array (0 owned, `L` reached at layer `L`) becomes `vtkGhostType` `UInt8` on cells (`DUPLICATECELL` for any layer) and on points (`DUPLICATEPOINT` when no owned cell of the piece uses the point) and sets the index's `GhostLevel` to the deepest layer; `partition:ghost` itself is kept too, since `vtkGhostType` collapses layer 2 onto layer 1. A `vtkGhostType` the caller already supplied is passed through (`REFINEDCELL`/`HIDDENCELL` survive), nothing is fabricated when neither array exists, and the caller's meshes are never mutated. Reading **keeps** ghost cells by default, and `ghosts="drop"` removes every cell with any ghost bit set and the points only they used, per piece before merging (`PvtuReadOptions::mGhosts` in C++). VTK's `vtkXMLPUnstructuredGridReader` recognises the array as its ghost array with the right flagged count.
- **`.pvd`** has two axes: `timestep=` selects the step (`time_step=`, `ReadOptions::mTimeStep`) and `part=` the piece within it (`piece=`). Steps are the distinct `timestep` values ascending whatever their order or spacing; a missing `timestep` or `part` is 0; parts of a step merge with one region per entry (`name=`, else `group/part_<p>`, else `part_<p>`); a step that is a single entry reads as that file does, so `.pvd` → `.pvtu` → `.vtu` nests with no special case and keeps the `.pvtu`'s own regions. The chosen step's time is attached as `field_data["meshio:time"]`. `read_pvd_metadata` reports **every step's time from the index alone** (no piece opened for `time_values`, `fell_back_to_full_read = false`). `.pvd` joins both sequence predicates (`seq_format_may_have_steps` / `_TIME_CAPABLE_READERS`, and `sequence_write_supports_time` / `_SERIES_WRITERS`), so `read_sequence`, `TimeSeries` and `write_sequence(path.pvd, steps)` work, through a new `PvdSeriesWriter` (C++, behind the native sequence driver) and `pvd.SeriesWriter` (Python) that stream one mesh at a time and **rewrite the index after every step**, so a killed run still leaves a collection ParaView opens. A plain `write("x.pvd", mesh)` is a one-step collection timed by `meshio:time`. Times are written as the shortest exact spelling, locale-independent; non-finite times are refused.
- **The partition command writes an index.** `partition IN OUT.pvtu -n 4 --ghost-layers 1` (both CLIs; also `-o pvtu`) with no `{part}` token writes one index over every part instead of one file per part, which is the only route that keeps a halo; a `{part}` token still wins. The MCP `partition` tool does the same for a `name_template` ending in `.pvtu`/`.pvtp` without `{part}` (returning `index`), and now **refuses any other template without `{part}`**, which used to overwrite every part with the last. The MCP `compression` codecs reach the three formats. `doc/cli.md` no longer says `--ghost-layers` is "reserved, only 0".
- **Sniffing fixed.** The `vtu`/`vtp` checks were bare substrings, and `PUnstructuredGrid` contains `UnstructuredGrid`. The indexes are now matched on the quoted `type=` value before the serial types (a bare `Collection` would also match `vtkPartitionedDataSetCollection`), and a parallel image, structured or rectilinear index is refused by name instead of being mistaken for its serial twin. C++ and Python twins agree.
- **Robustness.** Piece paths with spaces and XML entities are read as written; an absolute path from another machine that does not exist here is an error naming the attribute and the path, deliberately *not* searched for elsewhere (a fallback could read the wrong file); `header_type`/`byte_order` may differ between index and pieces (each piece's own header is authoritative); a parallel index does not nest (a `.pvtu` names `.vtu`/`.vtp` only), so no cycle is expressible; the three names are in `_MULTIFILE_FORMATS`, so a buffer is refused by name.
- **Recorded limitations.** `field_data` is not carried: the `.vtu` writer emits no `<FieldData>` and the C++ `.vtu` reader does not read it (the Python reference reader does) — a pre-existing property of `.vtu`, found while testing, that this change does not fix. The Python reference carves, derives ghosts and drops ghosts for rectangular blocks only and raises `NotImplementedError` naming the core for polygon/polyhedron blocks, which the C++ core handles (tested).
- **Tests.** `test_pvtu.py` (105, including the `partition` command and polyhedron cases) and `test_pvd.py` (53) run every behavioural test against both engines, the core one under strict-core (sabotaging `_core.pvd_read` was confirmed to fail every `[core]` case); `test_pvtu_vtk.py` (16, skipped without `vtk`) has VTK 9.7's `vtkXMLPUnstructuredGridReader`/`vtkXMLPPolyDataReader` read every file either engine writes and both engines read what `vtkXMLPUnstructuredGridWriter`/`vtkXMLPPolyDataWriter` write (ASCII and binary, `Float32` points, `version="0.1"`); `tests/cpp/test_pvtu.cpp` and `test_pvd.cpp` (43) test the core directly, including the sequence driver's fan-in and fan-out. The two anti-drift gates in `test_sequence.cpp` sweep every registered writer and reader, so they cover the new formats without an edit. `PvtuReadOptions` and `PvdSeriesWriter` are pinned in `tests/cpp/test_abi_layout.cpp` from birth.
- **`ReadOptions` did not grow.** The ghost policy is a `PvtuReadOptions` struct passed as a defaulted trailing parameter of `read_pvtu`/`read_pvtp`/`read_pvd`: a new `ReadOptions` member would be a Tier A layout change that also reaches `PipelineInput`, `Pipeline`, `SequenceInput` and `SequencePipeline`, forcing ABI 15 in a minor release.
- **Not verified in this change.** **ParaView itself was not run**: vanilla VTK has no collection reader (`vtkPVDReader` ships with ParaView), so `.pvd` is checked by structure and by every file it names opening in VTK's readers, never as a ParaView time slider. The R and WASM bindings, the Fortran and Julia runtimes and `tests/wasm/smoke.mjs` were not built or run (no R, Emscripten, Julia or Fortran toolchain was exercised, and no WASM smoke step was added); the new sources were compile-checked under the NATIVE and KRATOS mesh backends but their tests were run under MESHIO only. The C++ tests were linked against the already-built core objects rather than through a fresh `build/configure.sh --tests` tree.
- **Roadmap §1.1 is narrowed, not closed.** Three deliberate edges remain: a `ghosts` switch on the generic read surfaces (CLIs, MCP, the flat bindings) needs a `ReadOptions` member and so an ABI 15; a `TimeValue` field-data array as a file's time needs the C++ `.vtu` reader to read `<FieldData>`; and a `.pvd` of `.pvtu` still has to be opened in ParaView and the version recorded.
