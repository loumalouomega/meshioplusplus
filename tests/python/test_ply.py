...
def test_ply(mesh, binary, tmp_path):
    def writer(*args, **kwargs):
        return meshioplusplus.ply.write(*args, binary=binary, **kwargs)

    for k, c in enumerate(mesh.cells):
        mesh.cells[k] = meshioplusplus.CellBlock(c.type, c.data.astype(np.int32))

    helpers.write_read(tmp_path, writer, meshioplusplus.ply.read, mesh, 1.0e-12)
...
def test_no_cells(binary):
    ...
    mesh = meshioplusplus.Mesh(vertices, [])
    file = io.BytesIO()
