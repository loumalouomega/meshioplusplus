...
| `warp3d_ssy.out`, `.wnfr00001`, `.wefe00001`, `.wnbd00001`, `.webs00001` | WARP3D (neutral file and Patran 2.5 result files: nodal reactions and element strains as text, nodal displacements and element stresses as Fortran binary) | WARP3D `pat2exii_test/`, trimmed by `tools/gen_patran_fixtures.py` to the first 40 elements and their nodes (the kept packets and records copied unchanged) | same |
...
`tests/python/test_patran.py` reads each with both engines and checks the cell orientation and, for `tahoe_square.pat`, the named components (packet 21), and for `warp3d_ssy` the constraints (packet 08) and the result files.
