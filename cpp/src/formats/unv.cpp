...
bool meshio_descriptor(const std::string& rT, int& rDesc, bool& rBeam) {
    static const std::unordered_map<std::string, std::pair<int, bool>> m = {
        ...
        {"hexahedron", {115, false}}, {"hexahedron20", {116, false}}};
    auto it = m.find(rT);
    if (it == m.end())
        return false;
    ...
    while (i < n) {
        ...
        if (ds == 2411 || ds == 781) {
            ...
            while (k < end) {
                ...
                groups[git->second].mRows.push_back(std::move(conn));
                groups[git->second].mPid.push_back(pid);
            }
        } else if (ds == 2467 || ds == 2477 || ds == 2414 || ds == 55 || ds == 56 || ds == 57) {
            // groups / fields need the Python reference implementation
            throw ReadError("UNV: dataset " + std::to_string(ds) + " handled by Python fallback");
        }
