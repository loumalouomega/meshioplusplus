...
const std::vector<int>* write_reorder(const std::string& type) {
    ...
    if (type == "triangle6") return &tria6;
    if (type == "tetra10") return &tet10;
    if (type == "quad9") return &quad9;
    if (type == "wedge15") return &wedge15;
    return nullptr;
}
...
std::string keyword_of(const std::string& line) {
    std::size_t a = 0, b = line.size();
    while (a < b && (line[a] == '$' || std::isspace(static_cast<unsigned char>(line[a])))) ++a;
    while (b > a && (line[b - 1] == '$' || std::isspace(static_cast<unsigned char>(line[b - 1])))) --b;
    return upper(line.substr(a, b - a));
}
...
Mesh read_permas(const std::string& path) {
    ...
    while (pos < n) {
        ...
        if (kw.rfind("COOR", 0) == 0) {
            ...
            if (eq == std::string::npos) throw ReadError("PERMAS: $ELEMENT without TYPE=");
            std::string etype = upper(split_ws(kw.substr(eq + 1)).empty()
                                          ? std::string()
            ...
            for (std::size_t r = 0; r < rows.size(); ++r)
                for (std::size_t j = 0; j < k; ++j) dp[r * k + j] = rows[r][j];
    ...
    mesh.points = NDArray(DType::Float64,
                          {static_cast<std::size_t>(npoints), ncoord});
        ...
        for (std::size_t r = 0; r < nc; ++r) {
            ...
            if (reorder) {
                ...
                for (std::size_t j = 0; j < ncols; ++j)
                    f << " " << (detail::read_int(cb.data, r * ncols + j) + 1);
            }
            f << "\n";
        }
    }
