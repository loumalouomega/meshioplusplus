...
bool is_float_token(const std::string& s) {
...
std::string tecplot_to_meshio(const std::string& z) {
    std::string u = upper(z);
    if (u == "LINESEG" || u == "FELINESEG") return "line";
    if (u == "TRIANGLE" || u == "FETRIANGLE") return "triangle";
    if (u == "QUADRILATERAL" || u == "FEQUADRILATERAL") return "quad";
    if (u == "TETRAHEDRON" || u == "FETETRAHEDRON") return "tetra";
    if (u == "BRICK" || u == "FEBRICK") return "hexahedron";
    return "";
}
std::string meshio_to_tecplot(const std::string& m) {
    if (m == "line") return "FELINESEG";
    if (m == "triangle") return "FETRIANGLE";
    if (m == "quad") return "FEQUADRILATERAL";
    if (m == "tetra") return "FETETRAHEDRON";
    if (m == "pyramid" || m == "wedge" || m == "hexahedron") return "FEBRICK";
    return "";
}
const std::vector<int>& tecplot_order(const std::string& m) {
...
Mesh read_tecplot(const std::string& path) {
    ...
    for (; i < lines.size(); ++i) {
        ...
        if (u.rfind("VARIABLES", 0) == 0) {
            ...
            while (i + 1 < lines.size() && strip(lines[i + 1])[0] == '"') joined += " " + lines[++i];
            ...
            while (p < rhs.size()) {
                if (rhs[p] == '"') {
                    ...
                    std::size_t q = p;
                    while (q < rhs.size() && !std::isspace((unsigned char)rhs[q]) && rhs[q] != ',') ++q;
            ...
            for (std::size_t k = 0; k + 1 < tk.size(); ++k) {
                ...
                if (key == "NODES" || key == "N" || key == "ELEMENTS" || key == "E" ||
                    key == "DATAPACKING" || key == "ZONETYPE" || key == "F" ||
                    key == "ET" || key == "NV")
                    zone[key] = tk[k + 1];
            }
            break;
    ...
    if (variables.empty()) throw ReadError("Tecplot: no VARIABLES");

    auto getz = [&](const char* a, const char* b) -> std::string {
        if (zone.count(a)) return zone[a];
        if (zone.count(b)) return zone[b];
        return "";
    };
    std::size_t num_nodes = std::stoull(getz("NODES", "N"));
    std::size_t num_cells = std::stoull(getz("ELEMENTS", "E"));
    std::string fmt, ztype;
    if (zone.count("F")) { fmt = upper(zone["F"]); ztype = zone.count("ET") ? zone["ET"] : ""; }
    else { fmt = "FE" + upper(getz("DATAPACKING", "")); ztype = getz("ZONETYPE", ""); }
    ...
    if (feblock) {
        if (zone.count("NV")) {
            ...
            for (std::size_t k = nv; k < variables.size(); ++k) cell_centered[k] = 1;
        } else if (!varloc.empty()) {
            std::string vc = varloc.substr(1, varloc.size() - 2);  // strip ()
            for (const auto& entry : [&] {
                     std::vector<std::string> es; std::string cur;
                     for (char c : vc) { if (c == ',') { es.push_back(cur); cur.clear(); } else cur += c; }
                     if (!cur.empty()) es.push_back(cur); return es; }()) {
                std::size_t eq = entry.find('=');
                if (eq == std::string::npos) continue;
                std::string rng = entry.substr(0, eq), loc = upper(entry.substr(eq + 1));
                if (loc != "CELLCENTERED") continue;
                ...
                if (dash == std::string::npos) {
                    ...
                    for (int k = a; k <= b; ++k) cell_centered[k - 1] = 1;
                }
            }
        }
    }
    ...
    while (flat.size() < want && li < lines.size()) {
        for (const auto& t : tokens(lines[li])) flat.push_back(std::strtod(t.c_str(), nullptr));
        ++li;
    }
    ...
    if (feblock) {
        ...
        for (std::size_t k = 0; k < nv; ++k) cols[k].resize(num_nodes);
        for (std::size_t r = 0; r < num_nodes; ++r)
            for (std::size_t k = 0; k < nv; ++k) cols[k][r] = flat[r * nv + k];
    }

    // Cells.
    std::string mtype = tecplot_to_meshio(ztype);
    if (mtype.empty()) throw ReadError("Tecplot: unsupported zone type " + ztype);
    std::size_t nn;
    if (mtype == "line") nn = 2; else if (mtype == "triangle") nn = 3;
    else if (mtype == "quad" || mtype == "tetra") nn = 4; else nn = 8;
    NDArray celldata(DType::Int64, {num_cells, nn});
    std::int64_t* cp = celldata.as<std::int64_t>();
    for (std::size_t c = 0; c < num_cells; ++c) {
        auto t = tokens(lines.at(li++));
        for (std::size_t j = 0; j < nn; ++j) cp[c * nn + j] = std::strtoll(t[j].c_str(), nullptr, 10) - 1;
    }
    ...
    for (std::size_t k = 0; k < variables.size(); ++k) {
        ...
        if (v == "X") xi = (int)k; else if (v == "Y") yi = (int)k; else if (v == "Z") zi = (int)k;
    }
    std::size_t ndim = (zi >= 0) ? 3 : 2;
    mesh.points = NDArray(DType::Float64, {num_nodes, ndim});
    double* pp = mesh.points.as<double>();
    for (std::size_t r = 0; r < num_nodes; ++r) {
        pp[r * ndim + 0] = cols[xi][r];
        pp[r * ndim + 1] = cols[yi][r];
        if (zi >= 0) pp[r * ndim + 2] = cols[zi][r];
    }
    for (std::size_t k = 0; k < variables.size(); ++k) {
        if ((int)k == xi || (int)k == yi || (int)k == zi) continue;
        ...
        if (cell_centered[k]) {
            ...
            mesh.cell_data.emplace(variables[k], std::move(blk));
        } else {
            mesh.point_data.emplace(variables[k], std::move(arr));
        }
    }
    mesh.cells.emplace_back(mtype, std::move(celldata));
    return mesh;
}
...
void write_tecplot(const std::string& path, const Mesh& mesh) {
    ...
    for (std::size_t i = 0; i < mesh.cells.size(); ++i) {
        if (!meshio_to_tecplot(mesh.cells[i].type).empty()) {
            blocks.push_back(i);
            types.insert(mesh.cells[i].type);
        }
    }
    if (types.size() != 1)
    ...
    for (std::size_t b : blocks) num_cells += mesh.cells[b].num_cells();
    ...
    auto push_point_col = [&](std::size_t comp) {
        ...
        for (std::size_t r = 0; r < num_nodes; ++r) col[r] = detail::read_double(mesh.points, r * dim + comp);
    ...
    for (const auto& kv : mesh.point_data) {
        ...
        for (std::size_t c = 0; c < ncomp; ++c) {
            ...
            std::vector<double> col(num_nodes);
            for (std::size_t r = 0; r < num_nodes; ++r) col[r] = detail::read_double(v, r * ncomp + c);
            data.push_back(std::move(col));
            varrange0 += 1;
        }
    }
    ...
    if (have_cell_data && varrange0 <= varrange1) {
        os << ",\n";
        std::string r = (varrange0 == varrange1) ? std::to_string(varrange0)
    ...
    for (std::size_t b : blocks) {
        const CellBlock& cb = mesh.cells[b];
        ...
        for (std::size_t r = 0; r < cb.num_cells(); ++r) {
            for (std::size_t j = 0; j < order.size(); ++j) {
                ...
                if (src >= k) src = k - 1;
                os << (detail::read_int(cb.data, r * k + src) + 1) << (j + 1 == order.size() ? '\n' : ' ');
            }
        }
    }
}
