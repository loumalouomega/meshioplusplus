...
const std::unordered_map<std::string, std::vector<int>>& m2n_pmap() {
    static const std::unordered_map<std::string, std::vector<int>> m = [] {
        ...
        for (const auto& kv : n2m_pmap()) {
            ...
            for (std::size_t i = 0; i < p.size(); ++i) inv[p[i]] = static_cast<int>(i);
...
int topo_dim(const std::string& type) {
...
struct LineCursor {
    ...
    std::string next_real(bool& is_eof) {
    ...
    std::string next_count() {
        bool eof = false;
        return next_real(eof);
    }
};

struct RawBlock {
    std::string type;
    std::vector<std::vector<std::int64_t>> rows;  // meshio node order, 0-based
    std::vector<std::int64_t> index;
};

void read_cells(LineCursor& c, const std::string& section,
                std::vector<RawBlock>& blocks) {
    int dim, pi0, i_index, fixed_nump = -1;
    if (section == "pointelements") {
        dim = 0; pi0 = 0; i_index = 1; fixed_nump = 1;
    } else if (section.rfind("edgesegments", 0) == 0) {
        dim = 1; pi0 = 2; i_index = 0; fixed_nump = 2;
    } else if (section.rfind("surfaceelements", 0) == 0) {
        dim = 2; pi0 = 5; i_index = 1;
    } else if (section == "volumeelements") {
        dim = 3; pi0 = 2; i_index = 0;
    } else {
        throw ReadError("Netgen: unknown cell section '" + section + "'");
    }

    std::int64_t num_cells = std::strtoll(c.next_count().c_str(), nullptr, 10);
    const auto& tmap = netgen_type(dim);

    for (std::int64_t k = 0; k < num_cells; ++k) {
        bool eof = false;
        std::string line = c.next_real(eof);
        if (eof) throw ReadError("Netgen: unexpected end of file in " + section);
        std::vector<std::string> data = split_ws(line);

        int nump = fixed_nump;
        if (dim == 2) nump = static_cast<int>(std::strtoll(data[4].c_str(), nullptr, 10));
        else if (dim == 3) nump = static_cast<int>(std::strtoll(data[1].c_str(), nullptr, 10));

        std::int64_t index = std::strtoll(data[i_index].c_str(), nullptr, 10);
        auto tit = tmap.find(nump);
        if (tit == tmap.end())
            throw ReadError("Netgen: unsupported element with " +
                            std::to_string(nump) + " nodes");
        ...
        blocks.back().rows.push_back(std::move(pi));
        blocks.back().index.push_back(index);
    }
}
...
Mesh read_netgen(const std::string& path) {
    if (path.size() >= 7 && path.compare(path.size() - 7, 7, ".vol.gz") == 0)
    ...
    bool eof = false;
    std::string line = c.next_real(eof);
    if (line != "mesh3d") throw ReadError("Not a valid Netgen mesh");
    ...
    while (true) {
        line = c.next_real(eof);
        if (eof) break;
        if (line == "dimension") {
            dimension = static_cast<int>(std::strtoll(c.next_count().c_str(), nullptr, 10));
        } else if (line == "geomtype") {
            c.next_count();  // value; ignored
        } else if (line == "points") {
            num_points = std::strtoll(c.next_count().c_str(), nullptr, 10);
            raw_points.resize(static_cast<std::size_t>(num_points) * 3, 0.0);
            for (std::int64_t i = 0; i < num_points; ++i) {
                std::string pl = c.next_real(eof);
                if (eof) throw ReadError("Netgen: unexpected EOF in points");
        ...
        } else if (line == "pointelements" || line == "edgesegments" ||
                   line == "edgesegmentsgi" || line == "surfaceelements" ||
                   line == "surfaceelementsgi" || line == "surfaceelementsuv" ||
                   line == "volumeelements") {
            ...
            throw ReadError("Netgen: token '" + line +
    ...
    for (std::int64_t i = 0; i < num_points; ++i)
        for (int j = 0; j < dimension; ++j) pp[i * dimension + j] = raw_points[i * 3 + j];

    std::vector<NDArray> index_blocks;
    for (auto& b : blocks) {
        const std::vector<int>& pmap = n2m_pmap().at(b.type);
        ...
        for (std::size_t r = 0; r < n; ++r)
            for (std::size_t j = 0; j < k; ++j)
                dp[r * k + j] = b.rows[r][pmap[j]] - 1;
        mesh.cells.emplace_back(b.type, std::move(data));

        NDArray idx(DType::Int64, {n});
        for (std::size_t r = 0; r < n; ++r) idx.as<std::int64_t>()[r] = b.index[r];
        index_blocks.push_back(std::move(idx));
    }
    mesh.cell_data.emplace("netgen:index", std::move(index_blocks));
...
void write_block(std::ostream& os, const CellBlock& cb, const NDArray* index) {
    if (cb.num_cells() == 0) return;
    ...
    for (std::size_t r = 0; r < n; ++r) {
        ...
        for (auto v : pre) cols.push_back(v);
        for (int j = 0; j < np; ++j)
            cols.push_back(detail::read_int(cb.data, r * np + pmap[j]) + 1);
        for (auto v : post) cols.push_back(v);
        if (index) cols[i_index] = detail::read_int(*index, r);

        for (std::size_t j = 0; j < cols.size(); ++j)
            os << cols[j] << (j + 1 == cols.size() ? '\n' : ' ');
    }
}

}  // namespace

void write_netgen(const std::string& path, const Mesh& mesh,
                  const std::string& float_fmt) {
    std::ofstream f(path, std::ios::binary);
    if (!f) throw WriteError("Could not open file for writing: " + path);

    const int dimension =
        mesh.points.shape().size() >= 2 ? static_cast<int>(mesh.points.shape()[1]) : 3;

    // Pick the single integer cell index, preferring "netgen:index".
    const std::vector<NDArray>* cells_index = nullptr;
    auto it = mesh.cell_data.find("netgen:index");
    ...
    auto index_for = [&](std::size_t ci) -> const NDArray* {
        if (!cells_index || ci >= cells_index->size()) return nullptr;
    ...
    for (const auto& cb : mesh.cells) {
        ...
        if (d >= 0 && d <= 3) per_dim[d] += static_cast<std::int64_t>(cb.num_cells());
    }
    ...
    for (std::size_t ci = 0; ci < mesh.cells.size(); ++ci)
        if (topo_dim(mesh.cells[ci].type) == 2)
            write_block(f, mesh.cells[ci], index_for(ci));

    f << "\n#  matnr      np      p1      p2      p3      p4\n";
    f << "volumeelements\n" << per_dim[3] << "\n";
    for (std::size_t ci = 0; ci < mesh.cells.size(); ++ci)
        if (topo_dim(mesh.cells[ci].type) == 3)
            write_block(f, mesh.cells[ci], index_for(ci));
    ...
    f << "edgesegmentsgi2\n" << per_dim[1] << "\n";
    for (std::size_t ci = 0; ci < mesh.cells.size(); ++ci)
        if (topo_dim(mesh.cells[ci].type) == 1)
            write_block(f, mesh.cells[ci], index_for(ci));
    ...
    for (std::size_t i = 0; i < npts; ++i) {
        for (int j = 0; j < 3; ++j) {
            double v = (j < dimension) ? detail::read_double(mesh.points, i * dimension + j)
                                       : 0.0;
    ...
    f << "pointelements\n" << per_dim[0] << "\n";
    for (std::size_t ci = 0; ci < mesh.cells.size(); ++ci)
        if (topo_dim(mesh.cells[ci].type) == 0)
            write_block(f, mesh.cells[ci], index_for(ci));
