...
TEST(Parallel, BackendName) {
    std::string name = meshio::parallel_backend_name();
...
TEST(Parallel, EmptyAndSingle) {
    ...
    meshio::parallel_for(0, [&](std::size_t) { ++count; });
    EXPECT_EQ(count.load(), 0);
    meshio::parallel_for(1, [&](std::size_t) { ++count; });
...
TEST(Parallel, ScatterSmallAndLarge) {
    ...
    for (std::size_t n : {std::size_t{17}, std::size_t{100000}}) {
        ...
        meshio::parallel_for(n, [&](std::size_t i) {
            out[i] = static_cast<std::int64_t>(i) * 3;
        });
...
TEST(Parallel, EveryIndexExactlyOnce) {
    ...
    meshio::parallel_for(n, [&](std::size_t i) { ++hits[i]; });
    for (std::size_t i = 0; i < n; ++i) ASSERT_EQ(hits[i].load(), 1) << "i=" << i;
}
...
TEST(Parallel, CustomGrain) {
    ...
    meshio::parallel_for(
        n, [&](std::size_t i) { out[i] = 1; }, /*grain=*/64);
