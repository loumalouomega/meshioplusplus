#pragma once
//
// meshio::log — minimal, header-only logging built on C++20 std::format and
// std::source_location.
//
//   meshio::log::warn("MED: orientation for '{}' not implemented", type);
//
// * Format strings are compile-time checked (std::format_string).
// * Every message carries its call site (file:line) automatically.
// * Runtime filtering via the MESHIO_LOG_LEVEL environment variable:
...
inline Level threshold() {
    static const Level lvl = [] {
        const char* env = std::getenv("MESHIO_LOG_LEVEL");
