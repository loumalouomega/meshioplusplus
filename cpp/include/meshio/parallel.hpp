#pragma once
//
// meshio::parallel_for — a minimal parallel loop with a compile-time-selected
// backend, in the spirit of the C++17 parallel STL:
//
//     meshio::parallel_for(n, [&](std::size_t i) { out[i] = f(in[i]); });
//
// The backend is chosen at *configure* time via the CMake option
// MESHIO_PARALLEL_BACKEND (SEQ | STL | OPENMP | TBB), which defines exactly
// one of:
//
//     MESHIO_PARALLEL_SEQ      plain sequential loop (always correct)
//     MESHIO_PARALLEL_STL      C++17 std::execution::par (default; on
//                              libstdc++ this requires linking TBB — CMake
//                              probes for it and falls back to SEQ)
//     MESHIO_PARALLEL_OPENMP   #pragma omp parallel for
//     MESHIO_PARALLEL_TBB      tbb::parallel_for
//
// Adding a new backend (Kokkos, HPX, ...) is a self-contained change:
//   1. add a MESHIO_PARALLEL_<NAME> branch to the CMake backend block,
...
#if defined(MESHIO_PARALLEL_STL)
#include <algorithm>
#include <execution>
#include <thread>
#include <vector>
#elif defined(MESHIO_PARALLEL_TBB)
...
#if defined(MESHIO_PARALLEL_STL)
    return "stl";
#elif defined(MESHIO_PARALLEL_OPENMP)
    return "openmp";
#elif defined(MESHIO_PARALLEL_TBB)
    return "tbb";
#else
...
#pragma omp parallel for schedule(static)
    ...
    exc.rethrow_if_any();
#else  // MESHIO_PARALLEL_SEQ (and the safe default)
    (void)grain;
