...
/** Wrapper functions the viewer calls; see wasm/src/index.mjs. */
...
/**
 * Fail the build if the installed meshio++ package is not the one in wasm/.
 *
 ...
 * changed, so npm skips it. A viewer built after a `wasm/` rebuild therefore
...
function checkWasmPackage() {
    return {
        ...
        async buildStart() {
            ...
            if (!b) {
                this.error(
                    `${built} does not exist -- wasm/dist/ is gitignored, so the package must ` +
