...
- **Optional deps** (`CMakeLists.txt`): `MESHIOPLUSPLUS_WITH_HDF5`/`_NETCDF`/`_ZLIB` options → `MESHIOPLUSPLUS_HAS_*` compile definitions and `_core.__has_hdf5__`/`__has_netcdf__` flags. `#ifdef`-guarded sources become empty TUs when off. Shared HDF5 helpers in `cpp/include/meshioplusplus/detail/hdf5_util.hpp`.
...
- **Parallelism** (`cpp/include/meshioplusplus/parallel.hpp`): `meshioplusplus::parallel_for(n, f, grain=2048)` with a compile-time backend selected by the CMake option `MESHIOPLUSPLUS_PARALLEL_BACKEND` = `SEQ|STL|OPENMP|TBB` (default STL; probed at configure time — on libstdc++ the STL backend needs TBB, and falls back to SEQ with a warning when unusable; MSVC needs nothing). Adding a backend (Kokkos, …) = one CMake branch + one `#elif` in `parallel.hpp`. Iterations must be independent; the first exception is captured and rethrown after the join; `n <= grain` runs sequentially. Used in the hot loops of med/gmsh/vtu/vtk/ply/su2/abaqus/ansysinp/flac3d/openfoam and `detail/vtu_binary.hpp` (base64, zlib blocks). Hoist per-element dtype switches with `detail::dispatch_dtype(dt, []<class T>(){…})` (`value_io.hpp`) before parallelizing a loop.
