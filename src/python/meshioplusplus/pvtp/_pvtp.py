"""VTK XML parallel index ``.pvtp``, the pure-Python reference.

A ``<PPolyData>`` index holds the *declarations* (``PPoints``, ``PPointData``,
``PCellData``) and one ``<Piece Source=>`` per part; every piece is a
standalone ``.vtp`` file. The machinery is shared with the other index
