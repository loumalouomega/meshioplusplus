...
from ._pvtp import read as _py_read
from ._pvtp import write as _py_write
from ._pvtp import write_pieces as _py_write_pieces
...
def read(filename, points_only=False, arrays=None, piece=None, ghosts="keep"):
    """Read a VTK XML parallel index (PPolyData): every piece merged into one mesh
    ...
    if not is_buffer(filename, "r"):
        try:
            return _core.pvtp_read(str(filename), points_only, arrays, piece, ghosts)
        except Exception as exc:
            if not core_declined(exc, "pvtp", "read", filename):
                raise
...
def write(
    ...
    """Write a mesh as a VTK XML parallel index plus one ``.vtp`` piece per part.
    ...
    if _cpp_codec_ok(compression, header_type) and not is_buffer(filename, "w"):
        try:
            _core.pvtp_write_codec(
                str(filename), mesh, binary, _CPP_CODECS[compression], part_key
            )
            return
        except Exception as exc:
            if not core_declined(exc, "pvtp", "write", filename):
                raise
    return _py_write(
        filename,
        mesh,
...
register_format("pvtp", [".pvtp"], read, {"pvtp": write})
