...
def _ensure_med_families(mesh):
    ...
    has_cell_tags = (
        hasattr(mesh, "cell_tags") and mesh.cell_tags and "cell_tags" in mesh.cell_data
    )
    ...
    if not has_cell_tags and (mesh.cell_sets or gmsh_physical is not None):
        ...
        cell_fam_arrays = [np.zeros(len(cb.data), dtype=np.int32) for cb in mesh.cells]
        ...
        for block_idx in range(n_blocks):
            ...
            for local_i in range(n_cells_in_block):
                ...
                cell_fam_arrays[block_idx][local_i] = combo_to_fam_cell[combo]
...
def read(filename):
    ...
    mesh_description = (
        mesh.attrs.get("DES", b"").decode("latin-1").strip().rstrip("\x00")
    )
    mesh_unit_time = mesh.attrs.get("UNT", b"").decode("latin-1").strip().rstrip("\x00")
    mesh_unit_coords = (
        mesh.attrs.get("UNI", b"").decode("latin-1").strip().rstrip("\x00")
    )
    ...
    if (
        "FAS" in mesh
    ):  # first check for FAS in the mesh, then in the root group, since some MED files have FAS only in the root group
        fas = mesh["FAS"]
...
def write(filename, mesh, med_version="4.1.0", **kwargs):
    ...
    med_mesh.attrs.create(
        "UNI", np.bytes_(uni.encode("latin-1")) if uni else numpy_void_str
    )
    ...
    for cell_type, cells_list in cells_by_type.items():
        ...
        if cell_type in ("polygon", "polygon2"):
            ...
            merged_cells = _med_cells_for_write(
                cell_type, merged_cells
            )  # meshio -> MED
    ...
    for base_name, entries in cell_groups.items():
        ...
        for i, (idx, pdt_orig, cell_type, data) in enumerate(entries):
            ...
            tracker.notify(
                "MED_CELL", med_to_geo_type.get(med_type, med_type), step_name
            )
