...
def _write_med_multi(filename, meshes, mesh_names=None, med_version="4.1.0", **kwargs):
    ...
    for mesh, name in zip(meshes, mesh_names):
        ...
        med_mesh.attrs.create("NOM", np.bytes_("".join(f"{n:<16}" for n in axis_names)))
        ...
        if "point_tags" in mesh.point_data:
            fam = nodes_group.create_dataset("FAM", data=mesh.point_data["point_tags"])
...
def _read_single_mesh(f, name):
    ...
    description = (
        mesh_grp.attrs.get("DES", b"").decode("latin-1").strip().rstrip("\x00")
    )
    unit_time = mesh_grp.attrs.get("UNT", b"").decode("latin-1").strip().rstrip("\x00")
    unit_coords = (
        mesh_grp.attrs.get("UNI", b"").decode("latin-1").strip().rstrip("\x00")
    )
    ...
    for med_cell_type, med_cell_type_group in med_cells.items():
        ...
        if "FAM" in med_cell_type_group:
            cell_data.setdefault("cell_tags", []).append(med_cell_type_group["FAM"][()])
